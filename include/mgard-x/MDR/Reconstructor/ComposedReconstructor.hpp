...
namespace MDR {
    // a decomposition-based scientific data reconstructor: inverse operator of composed refactor
    template<typename HandleType, mgard_x::DIM D, typename T_data, typename T_bitplane, class Decomposer, class Interleaver, class Encoder, class Compressor, class SizeInterpreter, class ErrorEstimator, class Retriever>
    ...
    public:
        ...
        T_data * reconstruct(double tolerance){
            ...
            uint8_t target_level = level_error_bounds.size() - 1;
            std::vector<std::vector<double>>& level_errors = level_squared_errors;
            if(std::is_base_of<mgard_x::MDR::MaxErrorEstimator<T_data>, ErrorEstimator>::value){
                std::cout << "ErrorEstimator is base of MaxErrorEstimator, computing absolute error" << std::endl;
                mgard_x::MDR::MaxErrorCollector<T_data> collector = mgard_x::MDR::MaxErrorCollector<T_data>();
                for(int i=0; i<=target_level; i++){
                    auto collected_error = collector.collect_level_error(NULL, 0, level_squared_errors[i].size(), level_error_bounds[i]);
                    level_abs_errors.push_back(collected_error);
                }
            ...
            else if(std::is_base_of<mgard_x::MDR::SquaredErrorEstimator<T_data>, ErrorEstimator>::value){
                std::cout << "ErrorEstimator is base of SquaredErrorEstimator, using level squared error directly" << std::endl;
            }
            else{
                std::cerr << "Customized error estimator not supported yet" << std::endl;
        ...
        T_data * progressive_reconstruct(double tolerance){
            ...
            mgard_x::LevelwiseCalcNDKernel<D, T_data, ADD, mgard_x::CUDA>().Execute(handle.shapes_h[0], 
                                                      handle.shapes_d[0],
                                                      mgard_x::SubArray<D, T_data, mgard_x::CUDA>(curr_data_array),
                                                      mgard_x::SubArray<D, T_data, mgard_x::CUDA>(data_array),
                                                      0);
        ...
        const std::vector<mgard_x::SIZE>& get_dimensions(){
            return dimensions;
        }
    ...
    private:
        bool reconstruct(uint8_t target_level, const std::vector<uint8_t>& prev_level_num_bitplanes, int queue_idx, bool progressive=true){
            ...
            std::vector<std::vector<mgard_x::Array<1, mgard_x::Byte, mgard_x::CUDA>>> compressed_bitplanes;
            for (int level_idx = 0; level_idx < target_level + 1; level_idx++){
              compressed_bitplanes.push_back(std::vector<mgard_x::Array<1, mgard_x::Byte, mgard_x::CUDA>>());
              ...
              for (int bitplane_idx = 0; bitplane_idx < num_bitplanes; bitplane_idx++) {
                mgard_x::SIZE size = level_sizes[level_idx][prev_level_num_bitplanes[level_idx] + bitplane_idx];
                // printf("level: %d, bitplane_idx: %d, size: %u\n", level_idx, bitplane_idx, size);
                compressed_bitplanes[level_idx].push_back(mgard_x::Array<1, mgard_x::Byte, mgard_x::CUDA>({size}));
            ...
            //     // printf("level: %d, bitplane_idx: %d, size: %u\n", level_idx, bitplane_idx, size);
            //     // compressed_bitplanes[level_idx].push_back(mgard_x::Array<1, mgard_x::Byte>({size}));
            //     compressed_bitplanes[level_idx][bitplane_idx].loadData(level_components[level_idx][bitplane_idx]);
            //   }
            // }
            ...
            mgard_x::Array<1, T_data, mgard_x::CUDA> * levels_array = new mgard_x::Array<1, T_data, mgard_x::CUDA>[target_level + 1];
            mgard_x::SubArray<1, T_data, mgard_x::CUDA> * levels_data = new mgard_x::SubArray<1, T_data, mgard_x::CUDA>[target_level + 1];

            for (int level_idx=0; level_idx < target_level + 1; level_idx++) {
              timer.start();
              // compressor.decompress_level(level_components[i], level_sizes[i], prev_level_num_bitplanes[i], level_num_bitplanes[i] - prev_level_num_bitplanes[i], stopping_indices[i]);
              mgard_x::SIZE num_bitplanes = level_num_bitplanes[level_idx] - prev_level_num_bitplanes[level_idx];
              mgard_x::Array<2, T_bitplane, mgard_x::CUDA> encoded_bitplanes({num_bitplanes, encoder.buffer_size(level_num_elems[level_idx])});
              ...
              levels_data[level_idx] = mgard_x::SubArray<1, T_data, mgard_x::CUDA>(levels_array[level_idx]);
              compressor.decompress_release();
              timer.end();
              timer.print("Decoding");            
            }
            ...
            timer.start();
            interleaver.reposition(levels_data, mgard_x::SubArray<D, T_data, mgard_x::CUDA>(data_array), queue_idx);
            handle.sync(queue_idx);
            timer.end();
            timer.print("Reposition");            

            timer.start();
            decomposer.recompose(mgard_x::SubArray<D, T_data, mgard_x::CUDA>(data_array), target_level, queue_idx);
            handle.sync(queue_idx);
            timer.end();
            timer.print("Recomposing");     
        ...
        // std::vector<std::vector<mgard_x::Array<1, mgard_x::Byte>>> compressed_bitplanes;
