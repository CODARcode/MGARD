...
template <typename T, typename T_fp, typename T_bitplane, typename T_error, SIZE B, typename DeviceType>
        ...
        { //option 1: one bit-plane per thread + put sign after most significant bit
          ...
          if (local_idx < num_bitplanes) { // each thread reponsibles for one bit-plane
            ...
            for (SIZE i = 0; i < B; i++) {
              ...
              if (debug) {
                printf("encode fp[%u] %u first bit %u\n", i, sm_fix_point[i], sm_first_bit_pos[i]);
                for (int j = 0; j < B; j++) {
  ...
  public:
    ...
    void Execute(SIZE n,
      ...
      for(int i=0; i<num_bitplanes+1; i++){
        ...
        //                             level_errors_temp[i], 2*(- (int)num_bitplanes + exp));
            ...
            if (!(*flags(gloabl_data_idx))) {
              ...
              if (fp_data) {
                if (sizeof(T_fp) == sizeof(uint32_t)) {
                  ...
                  first_bit_pos = __ffsll(__brevll(fp_data)) - (64 - num_bitplanes) - 1;
                }
              }
  ...
  public:
                                                                           ...
                                                                           int queue_idx) 
    {
      ...
      FunctorType functor(n, starting_bitplane, num_bitplanes, exp, encoded_bitplanes, flags, signs, v);
        SIZE total_thread_z = 1;
        SIZE total_thread_y = 1;
        SIZE total_thread_x = n;
        SIZE tbx, tby, tbz, gridx, gridy, gridz;
    ...
    public:
        ...
        std::vector<uint8_t *> encode(T_data const * data, SIZE n, int32_t exp, uint8_t num_bitplanes, std::vector<SIZE>& stream_sizes, std::vector<double>& level_errors) const {
            ...
            SIZE bitplane_max_length_per_block = ((B * 2)-1)/(sizeof(T_bitplane)*8)+1;
            ...
            for(int i=0; i<num_bitplanes; i++){
                ...
                streams2.push_back((uint8_t *) malloc(bitplane_max_length_total*sizeof(uint8_t)));
                memcpy(streams2[i], encoded_bitplanes + i * bitplane_max_length_total, bitplane_max_length_total*sizeof(uint8_t)); 
            }
            return streams2;
            ...
            for(int i=0; i<level_errors.size(); i++){
              // printf("cpu %f <- %f exp %d\n", ldexp(level_errors[i], 2*(- (int)num_bitplanes + exp)),
