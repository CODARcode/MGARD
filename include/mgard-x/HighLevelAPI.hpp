...
void
domain_decompose(T * data, std::vector<T*> &decomposed_data, std::vector<SIZE> shape, DIM domain_decomposed_dim, SIZE domain_decomposed_size) {
  // {
  ...
  for (SIZE i = 0; i < shape[domain_decomposed_dim]; i += domain_decomposed_size) {
    T *chunck_data;
    ...
    MemoryManager<DeviceType>::CopyND(chunck_data, dst_ld, src_ptr, src_ld, n1, n2, 0);
    DeviceRuntime<DeviceType>::SyncQueue(0);
  ...
  SIZE leftover_dim_size = shape[domain_decomposed_dim] % domain_decomposed_size;
  if (leftover_dim_size != 0) {
    ...
    MemoryManager<DeviceType>::CopyND(leftover_data, dst_ld, src_ptr, src_ld, n1, n2, 0);
    DeviceRuntime<DeviceType>::SyncQueue(0);
...
template <DIM D, typename T, typename DeviceType>
void
domain_recompose(std::vector<T*> decomposed_data, T * data, std::vector<SIZE> shape, DIM domain_decomposed_dim, SIZE domain_decomposed_size) {
  std::vector<SIZE> chunck_shape = shape;
  chunck_shape[domain_decomposed_dim] = domain_decomposed_size;
  std::vector<SIZE> rev_shape = shape;
  std::reverse(rev_shape.begin(), rev_shape.end());
  ...
  for (SIZE i = 0; i < shape[domain_decomposed_dim]; i += domain_decomposed_size) {
    ...
    //           << "n1:" << n1 << " "
    //           << "n2:" << n2 << "\n";
    MemoryManager<DeviceType>::CopyND(src_ptr, src_ld, chunck_data, dst_ld, n1, n2, 0);
    DeviceRuntime<DeviceType>::SyncQueue(0);
  ...
  if (leftover_dim_size != 0) {
    ...
    MemoryManager<DeviceType>::CopyND(src_ptr, src_ld, leftover_data, dst_ld, n1, n2, 0);
    DeviceRuntime<DeviceType>::SyncQueue(0);
  }
...
template <DIM D, typename T, typename DeviceType>
T calc_norm_decomposed(std::vector<T*> decomposed_data, T s, std::vector<SIZE> shape, DIM domain_decomposed_dim, SIZE domain_decomposed_size) {
  T norm;
  ...
  for (SIZE i = 0; i < shape[domain_decomposed_dim]; i += domain_decomposed_size) {
    ...
    if (s == std::numeric_limits<T>::infinity()) {
      DeviceCollective<DeviceType>::AbsMax(elem_per_chunk, chunck_in_subarray, norm_subarray, 0);
      DeviceRuntime<DeviceType>::SyncQueue(0);
      norm = std::max(norm, norm_array.getDataHost()[0]);
    } else {
      DeviceCollective<DeviceType>::SquareSum(elem_per_chunk, chunck_in_subarray, norm_subarray, 0);
      DeviceRuntime<DeviceType>::SyncQueue(0);
  ...
  if (leftover_dim_size != 0) {
    ...
    if (s == std::numeric_limits<T>::infinity()) {
      DeviceCollective<DeviceType>::AbsMax(elem_leftover, leftover_subarray, norm_subarray, 0);
      DeviceRuntime<DeviceType>::SyncQueue(0);
      norm = std::max(norm, norm_array.getDataHost()[0]);
    } else {
      DeviceCollective<DeviceType>::SquareSum(elem_leftover, leftover_subarray, norm_subarray, 0);
      DeviceRuntime<DeviceType>::SyncQueue(0);
      norm += norm_array.getDataHost()[0];
    }
  }
...
void compress(std::vector<SIZE> shape, T tol, T s, enum error_bound_type type,
  ...
  if (!hierarchy.domain_decomposed) {
    ...
    Array<1, Byte, DeviceType> lossless_compressed_array =
        compress<D, T, DeviceType>(hierarchy, in_array, type, tol, s, norm, config);
    ...
    Array<1, unsigned char, DeviceType> compressed_array({(SIZE)compressed_size});
    ...
    if (config.timing) {
      std::cout << log::log_info << "Insufficient device memory for compressing the whole dataset at once. "
                << "Orignial domain is decomposed into " << hierarchy.hierarchy_chunck.size() << " sub-domains\n";
    }
    std::vector<T*> decomposed_data;
    domain_decompose<D, T, DeviceType>((T*)original_data, decomposed_data, shape, 
    ...
    T norm = 1;
    if (type == error_bound_type::REL) {
      norm = calc_norm_decomposed<D, T, DeviceType>(decomposed_data, s, shape, hierarchy.domain_decomposed_dim, hierarchy.domain_decomposed_size);
      if (config.timing) {
        if (s == std::numeric_limits<T>::infinity()) {
    ...
    for (SIZE i = 0; i < hierarchy.hierarchy_chunck.size(); i++) {
      if (config.timing) {
        ...
        for (DIM d = 0; d < D; d++) std::cout << hierarchy.hierarchy_chunck[i].shape_org[d] << " ";
        std::cout << ")\n";
      }
      Array<D, T, DeviceType> chunck_in_array(hierarchy.hierarchy_chunck[i].shape_org);
      chunck_in_array.loadData((const T *)decomposed_data[i]);
      // PrintSubarray("chunck_in_array", SubArray(chunck_in_array));
      Array<1, Byte, DeviceType> lossless_compressed_array =
          compress<D, T, DeviceType>(hierarchy.hierarchy_chunck[i], chunck_in_array, type, tol, s, norm, config);
      lossless_compressed_size.push_back(lossless_compressed_array.getShape()[0]);
      Byte * temp = NULL;
      MemoryManager<DeviceType>::MallocHost(temp, lossless_compressed_array.getShape()[0], 0);
      // std::cout << "temp: " << temp
                // << "lossless_compressed_array.get_dv(): " << lossless_compressed_array.get_dv() ;
                // << "lossless_compressed_array.getShape()[0]: " << lossless_compressed_array.getShape()[0] << "\n" << std::flush;
      // temp = new Byte[lossless_compressed_array.getShape()[0]];
      MemoryManager<DeviceType>::Copy1D(temp, lossless_compressed_array.get_dv(), lossless_compressed_array.getShape()[0], 0);
      DeviceRuntime<DeviceType>::SyncQueue(0);
    ...
    byte_offset = 0;
    Serialize<SERIALIZED_TYPE, DeviceType>((Byte*)compressed_data, serizalied_meta, metadata_size, byte_offset);
    for (uint32_t i = 0; i < hierarchy.hierarchy_chunck.size(); i++) {
      Serialize<SIZE, DeviceType>((Byte*)compressed_data, lossless_compressed_size.data() + i, 1, byte_offset);
      align_byte_offset<uint64_t>(byte_offset);
      Serialize<Byte, DeviceType>((Byte*)compressed_data, lossless_compressed_data[i], lossless_compressed_size[i], byte_offset);
    }
    free(serizalied_meta);
    for (SIZE i = 0; i < hierarchy.hierarchy_chunck.size(); i++) {
...
void decompress(std::vector<SIZE> shape, const void *compressed_data,
  ...
  if (!infer_domain_decomposed(compressed_data, compressed_size)) {
    ...
    Array<1, unsigned char, DeviceType> lossless_compressed_array({lossless_size});
    ...
    Array<D, T, DeviceType> out_array = decompress<D, T, DeviceType>(hierarchy, lossless_compressed_array, m.ebtype, m.tol, m.s, m.norm, config);
    ...
    // Deserialize<uint32_t, DeviceType>((Byte*)compressed_data, metadata_size_prt, 1, byte_offset);
    // printf("decompress metadata_size: %u\n", metadata_size);
    // SERIALIZED_TYPE * serizalied_meta = (SERIALIZED_TYPE *)std::malloc(metadata_size);
    // byte_offset = 0;
    // Deserialize<SERIALIZED_TYPE, DeviceType>((Byte*)compressed_data, serizalied_meta, metadata_size, byte_offset);
    ...
    Hierarchy<D, T, DeviceType> hierarchy(shape, m.domain_decomposed_dim, m.domain_decomposed_size, config.uniform_coord_mode);

    if (config.timing) {
      std::cout << log::log_info << "The original domain was decomposed into " 
                << hierarchy.hierarchy_chunck.size() << " sub-domains during compression.\n";
    }
    ...
    for (uint32_t i = 0; i < hierarchy.hierarchy_chunck.size(); i++) {
      SIZE temp_size;
      Byte * temp_data;
      Deserialize<SIZE, DeviceType>((Byte*)compressed_data, &temp_size, 1, byte_offset);
      MemoryManager<DeviceType>::MallocHost(temp_data, temp_size, 0);
      DeviceRuntime<DeviceType>::SyncQueue(0);
      align_byte_offset<uint64_t>(byte_offset);
      Deserialize<Byte, DeviceType>((Byte*)compressed_data, temp_data, temp_size, byte_offset);
    ...
    for (uint32_t i = 0; i < hierarchy.hierarchy_chunck.size(); i++) {
      ...
      Array<1, Byte, DeviceType> lossless_compressed_array({lossless_compressed_size[i]});
      lossless_compressed_array.loadData(lossless_compressed_data[i]);
      Array<D, T, DeviceType> out_array = decompress<D, T, DeviceType>(hierarchy.hierarchy_chunck[i], 
                                        lossless_compressed_array, m.ebtype, m.tol, m.s, m.norm, config);

      SIZE linearized_width = 1;
      for (DIM d = 1; d < D; d++) linearized_width *= out_array.getShape()[d];
      T * decompressed_data_chunck = NULL;
      MemoryManager<DeviceType>::MallocHost(decompressed_data_chunck, out_array.getShape()[0] * linearized_width, 0);
      MemoryManager<DeviceType>::CopyND(decompressed_data_chunck, out_array.getShape()[0],
                                        out_array.get_dv(), out_array.get_ldvs_h()[0],
                                        out_array.getShape()[0], linearized_width, 0);
      DeviceRuntime<DeviceType>::SyncQueue(0);
    ...
    domain_recompose<D, T, DeviceType>(decomposed_data, (T*)decompressed_data, shape, 
                      hierarchy.domain_decomposed_dim, hierarchy.domain_decomposed_size);
...
void decompress(std::vector<SIZE> shape, const void *compressed_data,
  ...
  if (!infer_domain_decomposed(compressed_data, compressed_size)) {
    ...
    Array<D, T, DeviceType> out_array = decompress<D, T, DeviceType>(hierarchy, compressed_array, m.ebtype, m.tol, m.s, m.norm, config);
    ...
    Hierarchy<D, T, DeviceType> hierarchy(shape, m.domain_decomposed_dim, m.domain_decomposed_size, coords);
