...
ZstdCompress(SubArray<1, C, DeviceType> &input_data, int compressionLevel) {
  ...
  size_t const actual_out_size =
      ZSTD_compress(out_data + sizeof(size_t), estimated_out_size, in_data, input_count * sizeof(C), compressionLevel);
...
ZstdDecompress(SubArray<1, Byte, DeviceType> &input_data) {
  ...
  ZSTD_decompress(out_data, actual_out_count, in_data + sizeof(size_t), input_count - sizeof(size_t));
