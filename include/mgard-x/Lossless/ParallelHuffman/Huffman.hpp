...
void HuffmanCompress(SubArray<1, Q, DeviceType> &dprimary_subarray,
  ...
  Histogram<Q, unsigned int, DeviceType>(dprimary_subarray,
                                         workspace.freq_subarray, primary_count,
                                         dict_size, queue_idx);
  ...
  MemoryManager<DeviceType>().Copy1D(
      dH_bit_meta, workspace.huff_bitwidths_subarray.data(), nchunk, queue_idx);
...
void HuffmanDecompress(SubArray<1, Byte, DeviceType> compressed_data,
  ...
  DeserializeArray<LENGTH>(compressed_data, outlier_idx, outlier_count,
                           byte_offset, true);
  DeserializeArray<S>(compressed_data, outlier, outlier_count, byte_offset,
                      true);
  outlier_idx_subarray =
      SubArray<1, LENGTH, DeviceType>({(SIZE)outlier_count}, outlier_idx);
  outlier_subarray = SubArray<1, S, DeviceType>({(SIZE)outlier_count}, outlier);
  ...
  Decode<Q, H, DeviceType>(ddata_subarray, huffmeta_subarray, primary_subarray,
                           ...
                           decodebook_subarray, decodebook_size, queue_idx);
  DeviceRuntime<DeviceType>::SyncQueue(queue_idx);
  if (log::level & log::TIME) {
    timer.end();
...
public:
  ...
  static size_t EstimateMemoryFootprint(SIZE primary_count, SIZE dict_size,
    ...
    return HuffmanWorkspace<Q, S, H, DeviceType>::estimate_size(
