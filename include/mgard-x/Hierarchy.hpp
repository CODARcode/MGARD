...
template <DIM D, typename T, typename DeviceType>
void Hierarchy<D, T, DeviceType>::init(std::vector<SIZE> shape, std::vector<T *> coords) {
...
template <DIM D, typename T, typename DeviceType> void Hierarchy<D, T, DeviceType>::destroy() {
...
template <DIM D, typename T, typename DeviceType>
std::vector<T *>
Hierarchy<D, T, DeviceType>::create_uniform_coords(std::vector<SIZE> shape, int mode) {
...
template <typename T>
void printShape(std::string name, std::vector<T> shape) {
...
template <DIM D, typename T, typename DeviceType>
size_t Hierarchy<D, T, DeviceType>::estimate_memory_usgae(std::vector<SIZE> shape) {
  ...
  hierarchy_space += D * (nlevel+1) * roundup(max_dim * sizeof(T), pitch_size); // volume
  for (DIM d = 0; d < D; d++) {
  ...
  estimate_memory_usgae = std::max(estimate_memory_usgae,
                                       hierarchy_space + input_space + norm_workspace);

  size_t decomposition_workspace = roundup(shape[D-1] * sizeof(T), pitch_size);
  for (DIM d = 0; d < D-1; d++) {
  ...
  size_t quantization_workspace = sizeof(QUANTIZED_INT) * total_elem + // quantized 
  ...
  size_t huffman_workspace = sizeof(QUANTIZED_INT) * total_elem; // fix-length encoding
...
template <DIM D, typename T, typename DeviceType>
bool Hierarchy<D, T, DeviceType>::need_domain_decomposition(std::vector<SIZE> shape) {
  // std::cout << log::log_info << "Estimated device memory usage/available " 
  //                            << (double)estimate_memory_usgae(shape)/1e9
  //                            << "GB/" << DeviceRuntime<DeviceType>::GetAvailableMemory()/1e9
  //                            << "GB.\n";
  return estimate_memory_usgae(shape) >= DeviceRuntime<DeviceType>::GetAvailableMemory();
}


template <DIM D, typename T, typename DeviceType>
void
Hierarchy<D, T, DeviceType>::domain_decomposition_strategy(std::vector<SIZE> shape) {
...
template <DIM D, typename T, typename DeviceType>
void
Hierarchy<D, T, DeviceType>::domain_decompose(std::vector<SIZE> shape, int uniform_coord_mode) {
  if (domain_decomposed_size < 3) {
    std::cerr << log::log_err << "need domain decomposition with reduce dimension.\n"
              << "This feature is not implemented. Please contact Jieyang Chen (chenj3@ornl.gov).\n";
    exit(-1);
  }
  ...
  for (SIZE i = 0; i < shape[domain_decomposed_dim]; i += chunck_shape[domain_decomposed_dim]) {
    // printShape("Decomposed domain " + std::to_string(hierarchy_chunck.size()), chunck_shape);
    hierarchy_chunck.push_back(Hierarchy<D, T, DeviceType>(chunck_shape, uniform_coord_mode));
  }

  SIZE leftover_dim_size = shape[domain_decomposed_dim] % chunck_shape[domain_decomposed_dim];
  if (leftover_dim_size != 0) {
    std::vector<SIZE> leftover_shape = shape;
    leftover_shape[domain_decomposed_dim] = leftover_dim_size;
    // printShape("Decomposed domain " + std::to_string(hierarchy_chunck.size()), leftover_shape);
    hierarchy_chunck.push_back(Hierarchy<D, T, DeviceType>(leftover_shape, uniform_coord_mode));
  }
}

template <DIM D, typename T, typename DeviceType>
void
Hierarchy<D, T, DeviceType>::domain_decompose(std::vector<SIZE> shape, std::vector<T *> &coords) {
  if (domain_decomposed_size < 3) {
    std::cerr << log::log_err << "need domain decomposition with reduce dimension.\n"
              << "This feature is not implemented. Please contact Jieyang Chen (chenj3@ornl.gov).\n";
    exit(-1);
  ...
  for (SIZE i = 0; i < shape[domain_decomposed_dim]; i += chunck_shape[domain_decomposed_dim]) {
    ...
    MemoryManager<DeviceType>::Copy1D(decompose_dim_coord, coords[domain_decomposed_dim] + i, chunck_shape[domain_decomposed_dim], 0);
    DeviceRuntime<DeviceType>::SyncQueue(0);
    for (SIZE j = 0; j < chunck_shape[domain_decomposed_dim]; j++) decompose_dim_coord[j] -= decompose_dim_coord[0];
    chunck_coords[domain_decomposed_dim] = decompose_dim_coord;
    hierarchy_chunck.push_back(Hierarchy<D, T, DeviceType>(chunck_shape, chunck_coords));
  ...
  if (leftover_dim_size != 0) {
    ...
    MemoryManager<DeviceType>::Copy1D(decompose_dim_coord, coords[domain_decomposed_dim] + (shape[domain_decomposed_dim] - leftover_dim_size), 
                          leftover_dim_size, 0);
    DeviceRuntime<DeviceType>::SyncQueue(0);
    for (SIZE j = 0; j < leftover_dim_size; j++) decompose_dim_coord[j] -= decompose_dim_coord[0];
    leftover_coords[domain_decomposed_dim] = decompose_dim_coord;
    hierarchy_chunck.push_back(Hierarchy<D, T, DeviceType>(leftover_shape, leftover_coords));
    delete [] decompose_dim_coord;
  }
}

// This constructor is for internal use only
template <DIM D, typename T, typename DeviceType> Hierarchy<D, T, DeviceType>::Hierarchy() {}

template <DIM D, typename T, typename DeviceType>
Hierarchy<D, T, DeviceType>::Hierarchy(std::vector<SIZE> shape, int uniform_coord_mode) {
...
template <DIM D, typename T, typename DeviceType>
Hierarchy<D, T, DeviceType>::Hierarchy(std::vector<SIZE> shape, std::vector<T *> coords) {
  if (!need_domain_decomposition(shape)) {
    // Config config;
    shape_org = shape;
    std::reverse(shape.begin(), shape.end());
...
template <DIM D, typename T, typename DeviceType>
Hierarchy<D, T, DeviceType>::Hierarchy(std::vector<SIZE> shape, DIM domain_decomposed_dim, SIZE domain_decomposed_size, int uniform_coord_mode) {
  // std::cout << log::log_info << "Domain decomposition was used during compression\n";
...
template <DIM D, typename T, typename DeviceType>
Hierarchy<D, T, DeviceType>::Hierarchy(std::vector<SIZE> shape, DIM domain_decomposed_dim, SIZE domain_decomposed_size, std::vector<T *> coords) {
  // std::cout << log::log_info << "Domain decomposition was used during compression\n";
  this->domain_decomposed_dim = domain_decomposed_dim;
  this->domain_decomposed_size = domain_decomposed_size;
  domain_decompose(shape, coords);
...
// template <DIM D, typename T, typename DeviceType>
// Hierarchy<D, T, DeviceType>::Hierarchy(std::vector<SIZE> shape, Config config, int uniform_coord_mode) {
//   if (!need_domain_decomposition(shape)) {
//     shape_org = shape;
//     std::reverse(shape.begin(), shape.end());
//     std::vector<T *> coords = create_uniform_coords(shape, uniform_coord_mode);
...
//     domain_decomposed = true;
//     DIM decompose_dim;
//     std::vector<SIZE> chunck_shape = domain_decomposition(shape, decompose_dim);
//     for (SIZE i = 0; i < shape[decompose_dim]; i += chunck_shape[decompose_dim]) {
//       hierarchy_chunck.push_back(Hierarchy<D, T, DeviceType>(chunck_shape));
//     }
//     SIZE leftover_dim_size = shape[decompose_dim] % chunck_shape[decompose_dim];
//     if (leftover_dim_size != 0) {
//       std::vector<SIZE> leftover_shape = shape;
//       leftover_shape[decompose_dim] = leftover_dim_size;
//       hierarchy_chunck.push_back(Hierarchy<D, T, DeviceType>(leftover_shape));
//     }
//   }
// }
...
//       MemoryManager<DeviceType>::Copy1D(decompose_dim_coord, coords[decompose_dim] + i, chunck_shape[decompose_dim], 0);
//       DeviceRuntime<DeviceType>::SyncQueue(0);
//       for (SIZE j = 0; j < chunck_shape[decompose_dim]; j++) decompose_dim_coord[j] -= decompose_dim_coord[0];
//       chunck_coords[decompose_dim] = decompose_dim_coord;
//       hierarchy_chunck.push_back(Hierarchy<D, T, DeviceType>(chunck_shape, chunck_coords, config));
//       delete [] decompose_dim_coord;
//     }
//     SIZE leftover_dim_size = shape[decompose_dim] % chunck_shape[decompose_dim];
//     if (leftover_dim_size != 0) {
...
//       std::vector<T *> leftover_coords = coords;
//       T * decompose_dim_coord = new T[leftover_dim_size];
//       MemoryManager<DeviceType>::Copy1D(decompose_dim_coord, coords[decompose_dim] + (shape[decompose_dim] - leftover_dim_size), 
//                             leftover_dim_size, 0);
//       DeviceRuntime<DeviceType>::SyncQueue(0);
//       for (SIZE j = 0; j < leftover_dim_size; j++) decompose_dim_coord[j] -= decompose_dim_coord[0];
//       leftover_coords[decompose_dim] = decompose_dim_coord;
//       hierarchy_chunck.push_back(Hierarchy<D, T, DeviceType>(leftover_shape, leftover_coords, config));
//       delete [] decompose_dim_coord;
//     }
//   }
// }
