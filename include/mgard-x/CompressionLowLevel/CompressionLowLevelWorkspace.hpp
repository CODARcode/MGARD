...
public:
  ...
  void allocate(Hierarchy<D, T, DeviceType> &hierarchy, Config &config,
    ...
    outlier_idx_array = Array<1, LENGTH, DeviceType>(
        {(SIZE)(hierarchy.total_num_elems() * estimated_outlier_ratio)});
    ...
    if (config.reorder != 0) {
      quantized_linearized_array = Array<1, QUANTIZED_INT, DeviceType>(
          {(SIZE)hierarchy.total_num_elems()});
    }
    pre_allocated = true;
  }
  ...
  void move(const CompressionLowLevelWorkspace<D, T, DeviceType> &workspace) {
    ...
    quantized_linearized_array =
        std::move(workspace.quantized_linearized_array);
  }
