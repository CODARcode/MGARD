...
public:
  ...
  uint8_t compress_level(std::vector<uint8_t *> &streams,
    ...
    for (int i = 0; i < streams.size(); i++) {
      ...
      free(streams[i]);
      // std::cout << compressed_size << " " << stream_sizes[i] << " " <<
      // stream_sizes[i] * 1.0 / compressed_size << std::endl; skip the first
    ...
    int latter_start_index =
        (stopping_index < latter_index) ? latter_index : stopping_index + 1;
  ...
  void decompress_level(std::vector<const uint8_t *> &streams,
    ...
    for (int i = 0; i < num_bitplanes; i++) {
      ...
      if ((bitplane_index <= stopping_index) ||
          (bitplane_index >= latter_index)) {
        uint8_t *decompressed = NULL;
        auto decompressed_size = ZSTD::decompress(
            streams[i], stream_sizes[bitplane_index], &decompressed);
  ...
  ~AdaptiveLevelCompressor() { decompress_release(); }
