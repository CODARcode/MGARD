...
inline void ErrorAsyncCheck(hipError_t code, TaskType &task,
  ...
  if (code != hipSuccess) {
    std::cout << 
              log::log_err << hipGetErrorString(code) 
...
template <typename TaskType>
inline void ErrorSyncCheck(hipError_t code, TaskType &task,
                      bool abort = true) {
  if (code != hipSuccess) {
    std::cout << log::log_err << hipGetErrorString(code) 
              <<" while executing "<< task.GetFunctorName().c_str()
              <<" with HIP (Sync-check)\n";
    if (abort) exit(code);
  }
...
class DeviceRuntime<HIP> {
  ...
  MGARDX_CONT static hipStream_t 
...
struct EncodeSignBits<T, METHOD, HIP>{
  MGARDX_EXEC 
...
template <typename T, OPTION METHOD>
struct DecodeSignBits<T, METHOD, HIP>{
  MGARDX_EXEC 
...
template <typename TaskType>
class DeviceAdapter<TaskType, HIP> {
public:
  MGARDX_CONT
  DeviceAdapter(){};
...
template <>
class DeviceCollective<HIP>{
public:
  MGARDX_CONT
  DeviceCollective(){};

  template <typename T> MGARDX_CONT static
  void Sum(SIZE n, SubArray<1, T, HIP>& v, SubArray<1, T, HIP>& result, int queue_idx) {
  ...
  template <typename T> MGARDX_CONT static
  void AbsMax(SIZE n, SubArray<1, T, HIP>& v, SubArray<1, T, HIP>& result, int queue_idx) {
    void     *d_temp_storage = NULL;
    size_t   temp_storage_bytes = 0;
    AbsMaxOp absMaxOp;
    hipStream_t stream = DeviceRuntime<HIP>::GetQueue(queue_idx);
 ...
 template <typename T> MGARDX_CONT static
  void ScanSumInclusive(SIZE n, SubArray<1, T, HIP>& v, SubArray<1, T, HIP>& result, int queue_idx) {
  ...
  template <typename T> MGARDX_CONT static
  void ScanSumExclusive(SIZE n, SubArray<1, T, HIP>& v, SubArray<1, T, HIP>& result, int queue_idx) {
    Byte     *d_temp_storage = NULL;
    size_t   temp_storage_bytes = 0;
    hipStream_t stream = DeviceRuntime<HIP>::GetQueue(queue_idx);
    bool debug = DeviceRuntime<HIP>::SyncAllKernelsAndCheckErrors;
  ...
  template <typename T> MGARDX_CONT static
  void ScanSumExtended(SIZE n, SubArray<1, T, HIP>& v, SubArray<1, T, HIP>& result, int queue_idx) {
    Byte     *d_temp_storage = NULL;
    size_t   temp_storage_bytes = 0;
    hipStream_t stream = DeviceRuntime<HIP>::GetQueue(queue_idx);
    bool debug = DeviceRuntime<HIP>::SyncAllKernelsAndCheckErrors;
  ...
  void SortByKey(SIZE n, SubArray<1, KeyT, HIP>& keys, SubArray<1, ValueT, HIP>& values, 
                 int queue_idx) {
    void     *d_temp_storage = NULL;
    size_t   temp_storage_bytes = 0;
    hipStream_t stream = DeviceRuntime<HIP>::GetQueue(queue_idx);
    ...
    MemoryManager<HIP>().Copy1D(values.data(), out_values.get_dv(), n, queue_idx);
    DeviceRuntime<HIP>::SyncQueue(queue_idx);
    MemoryManager<HIP>().Free(d_temp_storage);
  }
