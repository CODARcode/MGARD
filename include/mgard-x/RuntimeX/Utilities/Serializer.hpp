...
void Serialize(Byte *serialize_ptr, T * data_ptr, SIZE count, SIZE &byte_offset) {
  using Mem = MemoryManager<DeviceType>;
  align_byte_offset<T>(byte_offset);
  Mem::Copy1D(serialize_ptr + byte_offset, (Byte*)data_ptr, count * sizeof(T), 0);
  byte_offset += count * sizeof(T);
  DeviceRuntime<DeviceType>::SyncQueue(0);
}
...
template <typename T, typename DeviceType>
void Deserialize(Byte * serialize_ptr, T * data_ptr, SIZE count, SIZE &byte_offset) {
  using Mem = MemoryManager<DeviceType>;
  align_byte_offset<T>(byte_offset);
  Mem::Copy1D((Byte*)data_ptr, serialize_ptr + byte_offset, count * sizeof(T), 0);
  byte_offset += count * sizeof(T);
  DeviceRuntime<DeviceType>::SyncQueue(0);
}
