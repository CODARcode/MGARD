...
constexpr int LWPK_CONFIG[5][7][3] = {{{1, 1, 8}, {1, 1, 16}, {1, 1, 32}, {1, 1, 64}, {1, 1, 128}, {1, 1, 256}, {1, 1, 512}},
                                  {{1, 4, 4}, {1, 4, 8}, {1, 4, 16}, {1, 4, 32}, {1, 4, 64}, {1, 4, 128}, {1, 4, 256}},
                                  {{4, 4, 4}, {4, 4, 8}, {4, 4, 16}, {4, 4, 32}, {4, 4, 64}, {4, 4, 64},  {4, 4, 64}},
                                  {{4, 4, 4}, {4, 4, 8}, {4, 4, 16}, {4, 4, 32}, {4, 4, 64}, {4, 4, 64},  {4, 4, 64}},
                                  {{4, 4, 4}, {4, 4, 8}, {4, 4, 16}, {4, 4, 32}, {4, 4, 64}, {4, 4, 64},  {4, 4, 64}}};
...
MGARDX_CONT void 
FillAutoTunerTable(std::string kernel_name, int precision_idx, int range_l, int config) {
  ...
  string intput_dir_path = curr_dir_path + "/../../../../src/mgard-x/RuntimeX/AutoTuners";
  ...
  std::string input_file = intput_dir_path + "/AutoTuner" + device_type_string + extension;
  ...
  std::string new_string1 = std::regex_replace(buffer.str(), e1, replace1_string);
  // std::cout << "********************new file: " << new_string1 << "\n";

  std::regex e2(regex2_string, std::regex_constants::ECMAScript);
  std::string new_string2 = std::regex_replace(new_string1, e2, replace2_string);
  // std::cout << "********************new file2: " << new_string2 << "\n";

  std::regex e3(regex3_string, std::regex_constants::ECMAScript);
  std::string new_string3 = std::regex_replace(new_string2, e3, replace3_string);
