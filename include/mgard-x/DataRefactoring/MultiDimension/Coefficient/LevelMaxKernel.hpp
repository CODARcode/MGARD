#ifndef MGARD_X_CALC_LEVEL_MAX_KERNEL_HPP
#define MGARD_X_CALC_LEVEL_MAX_KERNEL_HPP
...
template <DIM D, typename T, int R, int C, int F,
          typename DeviceType>
class LevelMaxFunctor : public Functor<DeviceType> {
public:
  MGARDX_CONT
  LevelMaxFunctor() {}
  MGARDX_CONT
  LevelMaxFunctor(SubArray<1, SIZE, DeviceType> ranges, SIZE l_target,
...
class LevelMaxKernel : public AutoTuner<DeviceType> {
public:
  MGARDX_CONT
  LevelMaxKernel() : AutoTuner<DeviceType>() {}

  template <SIZE R, SIZE C, SIZE F>
  MGARDX_CONT
      Task<LevelMaxFunctor<D, T, R, C, F, DeviceType>>
      GenTask(SIZE l_target,
              SubArray<1, SIZE, DeviceType> ranges,
              SubArray<D, T, DeviceType> v, SubArray<1, T, DeviceType> *level_v,
              int queue_idx) {
    using FunctorType =
        LevelMaxFunctor<D, T, R, C, F, DeviceType>;
    FunctorType functor(ranges, l_target, v, level_v);
    SIZE tbx, tby, tbz, gridx, gridy, gridz;
    size_t sm_size = functor.shared_memory_size();
    int total_thread_z = v.getShape(2);
    int total_thread_y = v.getShape(1);
    int total_thread_x = v.getShape(0);
  ...
  MGARDX_CONT
  void Execute(SIZE l_target,
               SubArray<1, SIZE, DeviceType> ranges,
               SubArray<D, T, DeviceType> v,
               SubArray<1, T, DeviceType> level_max, int queue_idx) {

    Array<1, T, DeviceType> *level_v_array = new Array<1, T, DeviceType>[l_target + 1];
    ...
    for (SIZE l = 0; l < l_target + 1; l++) {
      ...
      level_v_array[l] = Array<1, T, DeviceType>({level_size - last_level_size});
      level_v[l] = SubArray(level_v_array[l]);
...
#define LLK(CONFIG)                                                            \
    if (config == CONFIG || AutoTuner<DeviceType>::ProfileKernels) {             \
      ...
      TaskType task =                                                            \
          GenTask<R, C, F>(l_target, ranges, v, d_level_v, queue_idx);    \
...
#undef LLK
    ...
    for (SIZE l = 0; l < l_target + 1; l++) {
      DeviceCollective<DeviceType>::AbsMax(level_v[l].getShape(0), level_v[l],
                                    result, queue_idx);
      MemoryManager<DeviceType>::Copy1D(level_max(l_target-l), result.data(), 1, queue_idx);
    }
  }
};
