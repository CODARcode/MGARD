...
namespace MDR {
    ...
    private:
        bool refactor(uint8_t target_level, uint8_t num_bitplanes){
            ...
            timer.start();
            // mgard_x::print_matrix(dimensions[2], dimensions[1], dimensions[0], data.data(), dimensions[0], dimensions[1]);
...
namespace MDR {
    // a decomposition-based scientific data refactor: compose a refactor using decomposer, interleaver, encoder, and error collector
    template<typename HandleType, mgard_x::DIM D, typename T_data, typename T_bitplane, class Decomposer, class Interleaver, class Encoder, class Compressor, class ErrorCollector, class Writer>
    ...
    public:
        ...
        void refactor(T_data const * data_, const std::vector<mgard_x::SIZE>& dims, uint8_t target_level, uint8_t num_bitplanes){
        ...
        void write_metadata() const {
            mgard_x::SIZE metadata_size = sizeof(uint8_t) + mgard_x::MDR::get_size(dimensions) // dimensions
                            + sizeof(uint8_t) + mgard_x::MDR::get_size(level_error_bounds) + mgard_x::MDR::get_size(level_squared_errors) + mgard_x::MDR::get_size(level_sizes) // level information
                            + mgard_x::MDR::get_size(stopping_indices) + mgard_x::MDR::get_size(level_num);
            uint8_t * metadata = (uint8_t *) malloc(metadata_size);
            uint8_t * metadata_pos = metadata;
            *(metadata_pos ++) = (uint8_t) dimensions.size();
    ...
    private:
        bool refactor(mgard_x::SIZE target_level, mgard_x::SIZE num_bitplanes, int queue_idx) {
          ...
          for(int level_idx = 0; level_idx < target_level + 1; level_idx ++){
            levels_array[level_idx] = mgard_x::Array<1, T_data, mgard_x::CUDA>({level_num_elems[level_idx]});
          ...
          for(int level_idx = 0; level_idx < target_level + 1; level_idx++){
            ...
            timer.start();
            mgard_x::Array<1, T_error, mgard_x::CUDA> level_errors_array({num_bitplanes+1});
            ...
            timer.start();
            std::vector<mgard_x::Array<1, mgard_x::Byte, mgard_x::CUDA>> compressed_encoded_bitplanes;
          ...
          for (int level_idx = 0; level_idx < target_level + 1; level_idx++){
            ...
            for (int bitplane_idx = 0; bitplane_idx < num_bitplanes; bitplane_idx++) {
              mgard_x::Byte * bitplane = compressed_bitplanes[level_idx][bitplane_idx].getDataHost();
