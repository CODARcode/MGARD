...
#include "linear_quantization.h"

namespace mgard_cuda {

template <typename T>
void calc_quantizers(T * quantizers, int D, T norm, T tol, T s, int l_target, bool reciprocal) {
  ...
  for (int l = 0; l < l_target+1; l++) {
    ...
    level_eb *= c;
    if (reciprocal) quantizers[l] = 1.0f / quantizers[l];
  }
...
__global__ void _levelwise_linear_quantize(int * shapes, int l_target, T * quantizers,
                      T *dv, int * ldvs, int *dwork, int * ldws,
                      bool prep_huffmam, int dict_size, 
                      int * shape,
  ...
  for (int d = 0; d < D; d++) {
    ...
    for (int l = 0; l < l_target+1; l++) {
      int bit = (idx[d] >= shapes_sm[(l_target+2) * d + l]) && (idx[d] < shapes_sm[(l_target+2) * d + l+1]);
      l_bit += bit << l;
  ...
  bool in_range = true;
  for (int d = 0; d < D; d++) {
    if (idx[d] >= shapes_sm[(l_target+2) * d + l_target+1]) in_range = false;
  }

  // printf("idx %llu, level: %d, in_range: %d idx[0]: shape_sm: %d\n", get_idx<D>(shape_sm, idx), level, in_range, shapes_sm[(l_target+2) * 0 + l_target+1]);
  ...
  if (level >= 0 && level <= l_target && in_range) {
    ...
    if (prep_huffmam) {
      ...
      if (quantized_data >= 0 && quantized_data < dict_size) {
        ...
        size_t i = atomicAdd((unsigned long long int*)outlier_count, (unsigned long long int)1);
...
void levelwise_linear_quantize_adaptive_launcher(
  ...
  cudaMemcpyAsyncHelper(handle, handle.quantizers, quantizers, sizeof(T)*(l_target+1), H2D,
                        queue_idx);
  // printf("norm: %f, tol: %f, s: %f, dict_size: %d\n", m.norm, m.tol, m.s, m.dict_size);
  ...
  for (int d = 3; d < D; d++) { gridx *= handle.dofs[d][0]; }
  ...
  _levelwise_linear_quantize<T, D, R, C, F><<<blockPerGrid, threadsPerBlock, sm_size,
                   *(cudaStream_t *)handle.get(queue_idx)>>>(
                    shapes, l_target, handle.quantizers, 
                    dv, ldvs, 
                    dwork, ldws,
                    prep_huffmam, m.dict_size, 
                    shape,
...
void levelwise_linear_quantize(mgard_cuda_handle<T, D> &handle, 
                     ...
                     size_t * outlier_count, unsigned int * outlier_idx, int * outliers,
                    int queue_idx) {
#define QUANTIZE(R, C, F) {levelwise_linear_quantize_adaptive_launcher<T, D, R, C, F>(\
                                handle, shapes, l_target,\
                                m, dv, ldvs, dwork, ldws,\
                                prep_huffmam,\
                                shape,\
                                outlier_count, outlier_idx, outliers, queue_idx);}

  if (D >= 3) { QUANTIZE(4, 4, 16) }
  if (D == 2) { QUANTIZE(1, 4, 32) }
  if (D == 1) { QUANTIZE(1, 1, 64) }
...
template <typename T, int D, int R, int C, int F>
__global__ void _levelwise_linear_dequantize(int * shapes, int l_target, T * quantizers,
                      int *dv, int * ldvs, T *dwork, int * ldws, int dict_size,
                      size_t outlier_count, unsigned int * outlier_idx, int * outliers) {

  size_t threadId =  (threadIdx.z * (blockDim.x * blockDim.y)) + (threadIdx.y * blockDim.x) + threadIdx.x;
  size_t blockId =  (blockIdx.z * (gridDim.x * gridDim.y)) + (blockIdx.y * gridDim.x) + blockIdx.x;
  ...
  if (level >= 0 && level <= l_target && in_range) {
    ...
    //       quantized_data, level, quantizers_sm[level], get_idx<D>(ldws, idx), quantizers_sm[level] * (T)quantized_data);
  ...
  //   //       outliter, level, quantizers_sm[level], get_idx<D>(ldws, idx), quantizers_sm[level] * (T)outliter);
...
template <typename T, int D, int R, int C, int F>
__global__ void _levelwise_linear_dequantize_outliers(int * shapes, int l_target, T * quantizers,
                      int *dv, int * ldvs, T *dwork, int * ldws, int dict_size,
                      size_t outlier_count, unsigned int * outlier_idx, int * outliers) {
...
void levelwise_linear_dequantize_adaptive_launcher(
  ...
  _levelwise_linear_dequantize<T, D, R, C, F><<<blockPerGrid, threadsPerBlock, sm_size,
                   *(cudaStream_t *)handle.get(queue_idx)>>>(
                    shapes, l_target, handle.quantizers,
                    dv, ldvs, 
                    dwork, ldws,
                    m.dict_size,
                    outlier_count, outlier_idx, outliers);
  _levelwise_linear_dequantize_outliers<T, D, R, C, F><<<blockPerGrid, threadsPerBlock, sm_size,
                   *(cudaStream_t *)handle.get(queue_idx)>>>(
                    shapes, l_target, handle.quantizers,
                    dv, ldvs, 
                    dwork, ldws,
...
void levelwise_linear_dequantize(mgard_cuda_handle<T, D> &handle, 
                    ...
                    size_t outlier_count, unsigned int * outlier_idx, int * outliers,
                    int queue_idx) {
#define DEQUANTIZE(R, C, F) {levelwise_linear_dequantize_adaptive_launcher<T, D, R, C, F>(\
                                handle, shapes, l_target,\
                                m, dv, ldvs, dwork, ldws,\
                                outlier_count, outlier_idx, outliers, queue_idx);}

  if (D >= 3) { DEQUANTIZE(4, 4, 16) }
  if (D == 2) { DEQUANTIZE(1, 4, 32) }
  if (D == 1) { DEQUANTIZE(1, 1, 64) }
