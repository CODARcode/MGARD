...
#include "grid_processing_kernel.h"

namespace mgard_cuda {

template <typename T, int D_GLOBAL, int D_LOCAL, int R, int C, int F,
          bool INTERPOLATION, bool CALC_COEFF, int TYPE>
__global__ void _gpk_reo(int * shape, int * shape_c,
                         int * ldvs, int * ldws,
                         int unprocessed_n, int * unprocessed_dims, 
                         ...
                         T *dwrcf, int lddwrcf1, int lddwrcf2) 
{
  ...
  //     threadIdx.x == 0 && threadIdx.y == 0 && threadIdx.z == 0) debug = false;
  ...
  for (int d = 0; d < D_GLOBAL; d ++) idx[d] = 0;
  ...
  if (D_LOCAL < 3) { nr = 1; nr_c = 1; }
  if (D_LOCAL < 2) { nc = 1; nc_c = 1; }
  ...
  for (int d = 0; d < D_GLOBAL; d++) {
    if (D_LOCAL == 3 && d != curr_dim_r && d != curr_dim_c && d != curr_dim_f) {
      ...
      bidx /= shape_sm[d];
      if (idx[d] >= shape_c_sm[d]) in_next = false;
    }
    ...
    for (int k = 0; k < unprocessed_n; k++) {
      if (t == unprocessed_dims_sm[k] && 
          (shape_sm[t] % 2 == 1 && idx[t] % 2 == 1 ||
           shape_sm[t] % 2 == 0 && idx[t] % 2 == 1 && idx[t] != shape_sm[t] - 1)) {
        skip = 1;
      }
    }
  }
...
template <typename T, int D_GLOBAL, int D_LOCAL, int R, int C, int F, 
          bool INTERPOLATION, bool CALC_COEFF, int TYPE>
void gpk_reo_adaptive_launcher(mgard_cuda_handle<T, D_GLOBAL> &handle, 
  ...
  _gpk_reo<T, D_GLOBAL, D_LOCAL,  R/2, C/2, F/2,
           INTERPOLATION, CALC_COEFF, TYPE><<<blockPerGrid, threadsPerBlock, sm_size, 
               *(cudaStream_t *)handle.get(queue_idx)>>>( 
                            ...
                            thrust::raw_pointer_cast(unprocessed_dims.data()), 
...
template <typename T, int D_GLOBAL, int D_LOCAL, bool INTERPOLATION, bool CALC_COEFF, int TYPE>
...
#define GPK(R, C, F){\
        gpk_reo_adaptive_launcher<T, D_GLOBAL, D_LOCAL, R, C, F, INTERPOLATION, CALC_COEFF, TYPE>(\
                                              handle,\
                                              shape, shape_c, ldvs, ldws,\
                                              unprocessed_dims,\
                                              curr_dim_r, curr_dim_c, curr_dim_f,\
                                              dratio_r, dratio_c, dratio_f,\
                                              dv, lddv1, lddv2,\
...
template <typename T, int D_GLOBAL, int D_LOCAL, int R, int C, int F, 
          bool INTERPOLATION, bool CALC_COEFF, int TYPE>
void gpk_reo_adaptive_launcher(mgard_cuda_handle<T, D_GLOBAL> &handle, 
                              int * shape_h, int * shape_d, int * shape_c_d, 
                              int * ldvs, int * ldws,
                              int unprocessed_n, int * unprocessed_dims,
                              int curr_dim_r, int curr_dim_c, int curr_dim_f,  
                              T *dratio_r, T *dratio_c, T *dratio_f, 
...
#define GPK(R, C, F){\
        gpk_reo_adaptive_launcher<T, D_GLOBAL, D_LOCAL, R, C, F, INTERPOLATION, CALC_COEFF, TYPE>(\
                                              handle,\
                                              shape_h, shape_d, shape_c_d, ldvs, ldws,\
                                              unprocessed_n, unprocessed_dims,\
                                              curr_dim_r, curr_dim_c, curr_dim_f,\
                                              dratio_r, dratio_c, dratio_f,\
                                              dv, lddv1, lddv2,\
...
if (D_LOCAL == 3) {
  ...
  if (profile || config == 3) { GPK(1, 1, 16) }
  if (profile || config == 2) { GPK(1, 1, 8) }
  if (profile || config == 1) { GPK(1, 1, 4) }
  if (profile || config == 0) { GPK(1, 1, 2) }
}
#undef GPK
}
...
__global__ void _gpk_rev(int * shape, int * shape_c,
                         int * ldvs, int * ldws,
                         int unprocessed_n, int * unprocessed_dims, 
                         int curr_dim_r, int curr_dim_c, int curr_dim_f,  
                         T *dratio_r, T *dratio_c, T *dratio_f, 
                         ...
                         int nvr, int nvc, int nvf) 
{
  ...
  bool debug2 = false;
  if (blockIdx.x == 0 && blockIdx.y ==0 && blockIdx.z == 0) debug2 = false;
  ...
  for (int d = 0; d < D_GLOBAL; d++) {
    if (D_LOCAL == 3 && d != curr_dim_r && d != curr_dim_c && d != curr_dim_f) {
      ...
      if ((shape_sm[d] % 2 == 1 && idx[d] % 2 != 0) ||
           shape_sm[d] % 2 == 0 && (idx[d] % 2 != 0 && idx[d] != shape_sm[d] - 1)) in_next = false;
    }
    if (D_LOCAL == 2 && d != curr_dim_c && d != curr_dim_f) {
      idx[d] = bidx % shape_sm[d];
      bidx /= shape_sm[d];
  ...
  if (TYPE == 1 && nf % 2 == 0) unpadding_f -= 1;
...
template <typename T, int D_GLOBAL, int D_LOCAL, int R, int C, int F,
          bool INTERPOLATION, bool COEFF_RESTORE, int TYPE>
void gpk_rev_adaptive_launcher(mgard_cuda_handle<T, D_GLOBAL> &handle,
                              thrust::device_vector<int> shape, 
                              thrust::device_vector<int> shape_c, 
                              thrust::device_vector<int> ldvs, 
                              thrust::device_vector<int> ldws,
  ...
  _gpk_rev<T, D_GLOBAL, D_LOCAL, R/2, C/2, F/2,
           INTERPOLATION, COEFF_RESTORE, TYPE><<<blockPerGrid, threadsPerBlock, sm_size, 
               *(cudaStream_t *)handle.get(queue_idx)>>>(
                            thrust::raw_pointer_cast(shape.data()), 
                            thrust::raw_pointer_cast(shape_c.data()), 
                            thrust::raw_pointer_cast(ldvs.data()), 
...
template <typename T, int D_GLOBAL, int D_LOCAL, bool INTERPOLATION, bool COEFF_RESTORE, int TYPE>
...
#define GPK(R, C, F) {gpk_rev_adaptive_launcher<T, D_GLOBAL, D_LOCAL, R, C, F, INTERPOLATION, COEFF_RESTORE, TYPE>(\
                            handle,\
                            shape, shape_c, ldvs, ldws,\
                            unprocessed_dims,\
                            curr_dim_r, curr_dim_c, curr_dim_f,\
