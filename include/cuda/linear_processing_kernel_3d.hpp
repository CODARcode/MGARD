...
#include "linear_processing_kernel.h"
namespace mgard_cuda {

template <typename T, int R, int C, int F>
__global__ void _lpk_reo_1_3d(int nr, int nc, int nf, int nf_c,
                              ...
                              T *dw, int lddw1, int lddw2) 
{
  ...
  // bool debug2 = false;
  // if (blockIdx.z == gridDim.z-1 && blockIdx.y == 1 && blockIdx.x == 16) debug2 = false;
  ...
  int actual_F = F;
  if(nf_c - blockId * blockDim.x < F) { actual_F = nf_c - blockId * blockDim.x; }


  // if (nf_c % 2 == 1){ 
  //   if(nf_c-1 - blockId * blockDim.x < F) { actual_F = nf_c - 1 - blockId * blockDim.x; }
  // } else {
  //   if(nf_c - blockId * blockDim.x < F) { actual_F = nf_c - blockId * blockDim.x; }
  // }
  ...
  if (r_gl < nr && c_gl < nc && f_gl < nf_c) {
    if (r_gl < zero_r && c_gl < zero_c && f_gl < zero_f) {
      ...
      // if (debug) printf("load left vsm[%d]<-dv1[%d, %d, %d]: %f\n", f_sm * 2 + 2, r_gl, c_gl, f_gl, dv1[get_idx(lddv11, lddv12, r_gl, c_gl, f_gl)]);
    ...
    if (f_sm == actual_F - 1) {
      if (r_gl < zero_r && c_gl < zero_c && f_gl < zero_f) {
        ...
        if (f_gl+1 < nf_c) {
          // if (debug) printf("load left+1 vsm[%d]: %f\n", actual_F * 2 + 2, dv1[get_idx(lddv11, lddv12, r_gl, c_gl, f_gl + 1)]);
    ...
    if (!PADDING) {
      if(nf_c % 2 != 0) {
        if (f_gl - 1 >= 0 && f_gl - 1 < nf_c - 1) {
          // if (debug) printf("load right vsm[%d]: %f <- %d %d %d\n", f_sm * 2 + 1, dv2[get_idx(lddv21, lddv22, r_gl, c_gl, f_gl - 1)], r_gl, c_gl, f_gl - 1);
        ...
        if (f_gl < nf_c - 1) {
          // if (debug) printf("load right vsm[%d]: %f <- %d %d %d\n", f_sm * 2 + 3, dv2[get_idx(lddv21, lddv22, r_gl, c_gl, f_gl)], r_gl, c_gl, f_gl);
    ...
    if (f_sm == actual_F - 1) {
      ...
      if (!PADDING) {
        if(nf_c % 2 != 0) {
          if (f_gl < nf_c - 1) {
            // if (debug) printf("load right+1 vsm[%d]: %f <- %d %d %d\n", actual_F * 2 + 1, dv2[get_idx(lddv21, lddv22, r_gl, c_gl, f_gl)], r_gl, c_gl, f_gl);
          ...
          if (f_gl - actual_F >= 0) {
            // if (debug) printf("load right-1 vsm[1]: %f <- %d %d %d\n", dv2[get_idx(lddv21, lddv22, r_gl, c_gl, f_gl - actual_F)], r_gl, c_gl, f_gl - actual_F);
  ...
  if (r_sm == 0 && c_sm == 0 && f_sm < actual_F) {
    ...
    if (blockId * F * 2 + actual_F + f_sm < nf - 2) {
      dist_f_sm[2 + actual_F + f_sm] = ddist_f[blockId * F * 2 + actual_F + f_sm];
      ratio_f_sm[2 + actual_F + f_sm] = dratio_f[blockId * F * 2 + actual_F + f_sm];
    } else {
  ...
  if (r_gl < nr && c_gl < nc && f_gl < nf_c) {  
    ...
    //       mass_trans(c, d, e, (T)0.0, (T)0.0, h1, h2, (T)0.0, (T)0.0, r1, r2, (T)0.0, (T)0.0);
    // }
...
void lpk_reo_1_3d_adaptive_launcher(mgard_cuda_handle<T, D> &handle, 
  ...
  _lpk_reo_1_3d<T, R, C, F><<<blockPerGrid, threadsPerBlock, sm_size, 
                                   *(cudaStream_t *)handle.get(queue_idx)>>>( nr, nc, nf, nf_c,
...
#define LPK(R, C, F) { lpk_reo_1_3d_adaptive_launcher<T, D, R, C, F>(handle,\
                               nr, nc, nf, nf_c,\
                               zero_r, zero_c, zero_f,\
                               ddist_f, dratio_f,\
                               dv1, lddv11, lddv12,\
                               dv2, lddv21, lddv22,\
                               dw, lddw1, lddw2, queue_idx); }
...
#endif
  if (D == 3) {
    if (profile || config == 6) { LPK(2, 2, 128) }
    if (profile || config == 5) { LPK(2, 2, 64) }
    if (profile || config == 4) { LPK(2, 2, 32) }
    if (profile || config == 3) { LPK(4, 4, 16) }
    if (profile || config == 2) { LPK(8, 8, 8) }
    if (profile || config == 1) { LPK(4, 4, 4) }
    if (profile || config == 0) { LPK(2, 2, 2) }
  } else if (D == 2) {
    if (profile || config == 6) { LPK(1, 2, 128) }
    if (profile || config == 5) { LPK(1, 2, 64) }
    if (profile || config == 4) { LPK(1, 2, 32) }
    if (profile || config == 3) { LPK(1, 4, 16) }
    if (profile || config == 2) { LPK(1, 8, 8) }
    if (profile || config == 1) { LPK(1, 4, 4) }
    if (profile || config == 0) { LPK(1, 2, 4) }
  } else if (D == 1) {
    if (profile || config == 6) { LPK(1, 1, 128) }
    if (profile || config == 5) { LPK(1, 1, 64) }
    if (profile || config == 4) { LPK(1, 1, 32) }
    if (profile || config == 3) { LPK(1, 1, 16) }
    if (profile || config == 2) { LPK(1, 1, 8) }
    if (profile || config == 1) { LPK(1, 1, 8) }
    if (profile || config == 0) { LPK(1, 1, 8) }
  }
#undef LPK
...
template <typename T, int R, int C, int F>
__global__ void _lpk_reo_2_3d(int nr, int nc, int nf_c, int nc_c,
  ...
  int actual_C = C;
  if(nc_c - blockIdx.y * blockDim.y < C) { actual_C = nc_c - blockIdx.y * blockDim.y; }

  // if (nc_c % 2 == 1){ 
  //   if(nc_c-1 - blockIdx.y * blockDim.y < C) { actual_C = nc_c - 1 - blockIdx.y * blockDim.y; }
  // } else {
  //   if(nc_c - blockIdx.y * blockDim.y < C) { actual_C = nc_c - blockIdx.y * blockDim.y; }
  // }
  ...
  if (r_gl < nr && c_gl < nc_c && f_gl < nf_c) {
    // if (debug) printf("load up vsm[%d]: %f <- %d %d %d\n", c_sm * 2 + 2, dv1[get_idx(lddv11, lddv12, r_gl, c_gl, f_gl)], r_gl, c_gl, f_gl);
    v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm * 2 + 2, f_sm)] =
              dv1[get_idx(lddv11, lddv12, r_gl, c_gl, f_gl)];

    if (c_sm == actual_C - 1) {
      if (c_gl+1 < nc_c) {
        // if (debug) printf("load up+1 vsm[%d]: %f <- %d %d %d\n", actual_C * 2 + 2, dv1[get_idx(lddv11, lddv12, r_gl, blockId * C + actual_C, f_gl)], r_gl, blockId * C + actual_C, f_gl);
        //c_gl+1 == blockId * C + C
    ...
    if (c_sm == 0) {
      if (c_gl-1 >= 0) {
        // if (debug) printf("load up-1 vsm[0]: %f <- %d %d %d\n", dv1[get_idx(lddv11, lddv12, r_gl, c_gl-1, f_gl)], r_gl, c_gl-1, f_gl);
    ...
    if (!PADDING) {
      if (c_gl < nc_c - 1) {
        // if (debug) printf("load down vsm[%d]: %f <- %d %d %d\n", c_sm * 2 + 3, dv2[get_idx(lddv11, lddv12, r_gl, c_gl, f_gl)], r_gl, c_gl, f_gl);
    ...
    if (c_gl-1 >= 0 && (PADDING && c_gl-1 < nc_c - 2 || !PADDING && c_gl-1 < nc_c - 1)) {
      if (c_sm == 0) {
        // if (debug) printf("PADDING: %d, c_gl-1: %d nc_c-2: %d\n", PADDING, c_gl-1, nc_c - 2);
        // if (debug) printf("load down-1 vsm[1]: %f <- %d %d %d\n", dv2[get_idx(lddv11, lddv12, r_gl, c_gl-1, f_gl)], r_gl, c_gl-1, f_gl);
  ...
  if (r_sm == 0 && c_sm == 0 && f_sm < actual_C) {
    ...
    if (blockId * C * 2 + actual_C + f_sm < nc - 1) {
      dist_c_sm[2 + actual_C + f_sm] = ddist_c[blockId * C * 2 + actual_C + f_sm]; 
      ratio_c_sm[2 + actual_C + f_sm] = dratio_c[blockId * C * 2 + actual_C + f_sm];
    } else {
...
template <typename T, int D, int R, int C, int F>
void lpk_reo_2_3d_adaptive_launcher(mgard_cuda_handle<T, D> &handle, 
  ...
  _lpk_reo_2_3d<T, R, C, F><<<blockPerGrid, threadsPerBlock, sm_size, 
                                   *(cudaStream_t *)handle.get(queue_idx)>>>( nr, nc, nf_c, nc_c,
                                                ddist_c, dratio_c, 
                                                dv1, lddv11, lddv12, 
                                                dv2, lddv21, lddv22, 
                                                dw, lddw1, lddw2); 
...
#define LPK(R, C, F) {lpk_reo_2_3d_adaptive_launcher<T, D, R, C, F>(handle,\
                               nr, nc, nf_c, nc_c,\
                               ddist_c, dratio_c,\
                               dv1, lddv11, lddv12,\
                               dv2, lddv21, lddv22,\
...
template <typename T, int R, int C, int F>
__global__ void _lpk_reo_3_3d(int nr, int nc_c, int nf_c, int nr_c,
  ...
  int actual_R = R;
  if(nr_c - blockIdx.z * blockDim.z < R) { actual_R = nr_c - blockIdx.z * blockDim.z; }
  // if (nr_c % 2 == 1){ 
  //   if(nr_c-1 - blockIdx.z * blockDim.z < R) { actual_R = nr_c - 1 - blockIdx.z * blockDim.z; }
  // } else {
  //   if(nr_c - blockIdx.z * blockDim.z < R) { actual_R = nr_c - blockIdx.z * blockDim.z; }
  // }
 ...
 // if (debug) printf("RCF: %d %d %d\n", R, C, F);
  if (r_gl < nr_c && c_gl < nc_c && f_gl < nf_c) {
    // if (debug) printf("load front vsm[%d]: %f <- %d %d %d\n", r_sm * 2 + 2, dv1[get_idx(lddv11, lddv12, r_gl, c_gl, f_gl)], r_gl, c_gl, f_gl);
    v_sm[get_idx(ldsm1, ldsm2, r_sm * 2 + 2, c_sm, f_sm)] =
              dv1[get_idx(lddv11, lddv12, r_gl, c_gl, f_gl)];

    if (r_sm == actual_R - 1) {
      if (r_gl+1 < nr_c) {
        // if (debug) printf("load front+1 vsm[%d]: %f <- %d %d %d\n", actual_R * 2 + 2, dv1[get_idx(lddv11, lddv12, blockId * R + actual_R, c_gl, f_gl)], blockId * R + actual_R, c_gl, f_gl);
    ...
    if (r_sm == 0) {
      if (r_gl-1 >= 0) {
        // if (debug) printf("load front-1 vsm[0]: %f <- %d %d %d\n", dv1[get_idx(lddv11, lddv12, r_gl-1, c_gl, f_gl)], r_gl-1, c_gl, f_gl);
    ...
    if (!PADDING) {
      if (r_gl < nr_c - 1) {
        // if (debug) printf("load back vsm[%d]: %f <- %d %d %d\n", r_sm * 2 + 3, dv2[get_idx(lddv11, lddv12, r_gl, c_gl, f_gl)], r_gl, c_gl, f_gl);
    ...
    if (r_gl-1 >= 0 && (PADDING && r_gl-1 < nr_c - 2 || !PADDING && r_gl-1 < nr_c - 1)) {
    // if (blockId > 0) {
      if (r_sm == 0) {
        // if (debug) printf("load back-1 vsm[1]: %f <- %d %d %d\n", dv2[get_idx(lddv11, lddv12, r_gl-1, c_gl, f_gl)], r_gl-1, c_gl, f_gl);
  ...
  if (r_sm == 0 && c_sm == 0 && f_sm < actual_R) {
    if (blockId * R * 2 + f_sm < nr - 1) { 
      ...
      // if (debug2 ) printf("load dist 1 [%d]: %f [%d]\n", 2 + f_sm, dist_r_sm[2 + f_sm], blockId * R * 2 + f_sm);
      ratio_r_sm[2 + f_sm] = dratio_r[blockId * R * 2 + f_sm];
      // if (debug2 )printf("load ratio 1 [%d]: %f [%d]\n", 2 + f_sm, ratio_r_sm[2 + f_sm], blockId * R * 2 + f_sm);
    } else {
      dist_r_sm[2 + f_sm] = 0.0;
      ratio_r_sm[2 + f_sm] = 0.0;
    } 
    if (blockId * R * 2 + actual_R + f_sm < nr - 2) {
      dist_r_sm[2 + actual_R + f_sm] = ddist_r[blockId * R * 2 + actual_R + f_sm];
      // if (debug2 )printf("load dist 2 [%d]: %f [%d]\n", 2 + actual_R + f_sm, dist_r_sm[2 + actual_R + f_sm], blockId * R * 2 + actual_R + f_sm);
      ratio_r_sm[2 + actual_R + f_sm] = dratio_r[blockId * R * 2 + actual_R + f_sm];
      // if (debug2 )printf("load ratio 2 [%d]: %f [%d]\n", 2 + actual_R + f_sm, ratio_r_sm[2 + actual_R + f_sm], blockId * R * 2 + actual_R + f_sm);
    } else {
  ...
  if (blockId > 0) {
    if (f_sm < 2) {
      ...
      // if (debug2 )printf("load dist -1 [%d]: %f [%d]\n", f_sm, dist_r_sm[f_sm], blockId * R * 2 - 2 + f_sm);
      ratio_r_sm[f_sm] = dratio_r[blockId * R * 2 - 2 + f_sm];
      // if (debug2 )printf("load ratio -1 [%d]: %f [%d]\n", f_sm, ratio_r_sm[f_sm], blockId * R * 2 - 2 + f_sm);
    } 
  } else {
    if (f_sm < 2) {
      dist_r_sm[f_sm] = 0.0;
  ...
  if (r_gl < nr_c && c_gl < nc_c && f_gl < nf_c) {
    ...
    // if (debug) printf("store[%d %d %d] %f (%f)\n", r_gl, c_gl, f_gl, mass_trans(a, b, c, d, e, h1, h2, h3, h4, r1, r2, r3, r4),
    //                 mass_trans(a, b, c, (T)0.0, (T)0.0, h1, (T)0.0, (T)0.0, h4, r1, r2, (T)0.0, (T)0.0));
    // // printf("%d %d %d\n", r_gl, c_gl, f_gl);
    //if (blockId * R + R == nr-1) {
      //if (r_gl+1 == nr_c - 1) {
      // if (r_gl+1 == nr_c - 1) {
...
template <typename T, int D, int R, int C, int F>
void lpk_reo_3_3d_adaptive_launcher(mgard_cuda_handle<T, D> &handle, 
  ...
  _lpk_reo_3_3d<T, R, C, F><<<blockPerGrid, threadsPerBlock, sm_size, 
                                   *(cudaStream_t *)handle.get(queue_idx)>>>( nr, nc_c, nf_c, nr_c,
                                                ddist_r, dratio_r, 
                                                dv1, lddv11, lddv12, 
                                                dv2, lddv21, lddv22, 
                                                dw, lddw1, lddw2); 
...
#define LPK(R, C, F) {lpk_reo_3_3d_adaptive_launcher<T, D, R, C, F>(handle,\
                               nr, nc_c, nf_c, nr_c,\
                               ddist_r, dratio_r,\
                               dv1, lddv11, lddv12,\
                               dv2, lddv21, lddv22,\
