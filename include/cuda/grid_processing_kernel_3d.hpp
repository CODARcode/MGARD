...
#include "grid_processing_kernel_3d.h"

namespace mgard_cuda {

template <typename T, int D, int R, int C, int F>
__global__ void _gpk_reo_3d(int nr, int nc, int nf, 
                           int nr_c,  int nc_c, int nf_c,
  ...
  if (nr % 2 == 0) { nr_p = nr + 1; rest_r_p = nr_p - r; }
  if (nc % 2 == 0) { nc_p = nc + 1; rest_c_p = nc_p - c; } 
  if (nf % 2 == 0) { nf_p = nf + 1; rest_f_p = nf_p - f; } 
  ...
  //if (!skip) 
  {
    ...
    //    case 2.b: block size >= rest (NO need to load extra, but need padding);
    ...
    if (r_sm < rest_r && c_sm < rest_c && f_sm < rest_f) {
      ...
      //     dv[get_idx(lddv1, lddv2, r_gl, c_gl, f_gl)], r_sm, c_sm, f_sm, nr, nc, nf);

      // asm volatile("membar.cta;");
      // start = clock64() - start; 
      // printf("[load main] block id %d,%d,%d elapsed %lu\n", blockIdx.z, blockIdx.y, blockIdx.x, start);
      // start = clock64(); 
      ...
      if (r_sm == 0) {
        if (rest_r > R * 2) {
          ...
          //   dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl, f_gl)], r_sm_ex, c_sm, f_sm);
        } else if (nr % 2 == 0){
          // if (r == 16 && c == 0 && f == 0) {
          //   printf("padding (%d %d %d) %f <- (%f %f %f)\n", rest_r_p - 1, c_sm, f_sm, 
          //         v_sm[get_idx(ldsm1, ldsm2, rest_r - 1, c_sm, f_sm)], rest_r - 1, c_sm, f_sm);
          //   padded = true;
      ...
      if (c_sm == 0) {
        ...
        //   dv[get_idx(lddv1, lddv2, r_gl, c_gl_ex, f_gl)], r_sm, c_sm_ex, f_sm);
        } else if (nc % 2 == 0){
      ...
      if (f_sm == 0) {
        if (rest_f > F * 2) {
          ...
          //   dv[get_idx(lddv1, lddv2, r_gl, c_gl, f_gl_ex)], r_sm, c_sm, f_sm_ex);
        } else if (nf % 2 == 0){
      ...
      if (c_sm == 0 && f_sm == 0) {
        if (rest_c > C * 2 && rest_f > F * 2) {
          ...
          // printf("load-cf[%d %d %d]:%f --> [%d %d %d]\n", r_gl, c_gl_ex, f_gl_ex, 
            // dv[get_idx(lddv1, lddv2, r_gl, c_gl_ex, f_gl_ex)], r_sm, c_sm_ex, f_sm_ex);
        } else if (rest_c <= C * 2 && rest_f <= F * 2 &&
                    nc % 2 == 0 && nf % 2 == 0) {
      ...
      if (r_sm == 0 && f_sm == 0) {
        if (rest_r > R * 2 && rest_f > F * 2) {
          // v_sm[get_idx(ldsm1, ldsm2, r_sm_ex, c_sm, f_sm_ex)] =
          //     dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl, f_gl_ex)];
          // printf("load-rf[%d %d %d]:%f --> [%d %d %d]\n", r_gl_ex, c_gl, f_gl_ex, 
            // dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl, f_gl_ex)], r_sm_ex, c_sm, f_sm_ex);
        } else if (rest_r <= R * 2 && rest_f <= F * 2 &&
                   nr % 2 == 0 && nf % 2 == 0) {
          // printf("padding (%d %d %d) <- (%d %d %d)\n", rest_r_p - 1, c_sm, rest_f_p - 1, rest_r - 1, c_sm, rest_f - 1);
      ...
      if(r_sm == 0 && c_sm == 0) {
        if (rest_r > R * 2 && rest_c > C * 2) {
          // v_sm[get_idx(ldsm1, ldsm2, r_sm_ex, c_sm_ex, f_sm)] =
          //     dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl_ex, f_gl)];
          // printf("load-rc[%d %d %d]:%f --> [%d %d %d]\n", r_gl_ex, c_gl_ex, f_gl, 
            // dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl_ex, f_gl)], r_sm_ex, c_sm_ex, f_sm);
        } else if (rest_r <= R * 2 && rest_c <= C * 2 &&
                    nr % 2 == 0 && nc % 2 == 0) {
          v_sm[get_idx(ldsm1, ldsm2, rest_r_p - 1, rest_c_p - 1, f_sm)] =
              v_sm[get_idx(ldsm1, ldsm2, rest_r - 1, rest_c - 1, f_sm)];
              // printf("padding (%d %d %d) <- (%d %d %d): %f\n", rest_r_p - 1, rest_c_p - 1, f_sm, rest_r - 1, rest_c - 1, f_sm, v_sm[get_idx(ldsm1, ldsm2, rest_r - 1, rest_c - 1, f_sm)]);
        } else if (rest_r > R * 2 && rest_c <= C * 2 &&
                   nc % 2 == 0) {
      ...
      if (r_sm == 0 && c_sm == 0 && f_sm == 0) {
        if (rest_r > R * 2 && rest_c > C * 2 && rest_f > F * 2) {
          ...
          // printf("load-rcf[%d %d %d]:%f --> [%d %d %d]\n", r_gl_ex, c_gl_ex, f_gl_ex, 
            // dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl_ex, f_gl_ex)], r_sm_ex, c_sm_ex, f_sm_ex);
    ...
    //           // print = true;
    //           // printf("(block %d %d %d) %2.2f \n", r,c,f, v_sm[get_idx(ldsm1, ldsm2, i, j, k)]);
    ...
    if (dw && threadId < R * C * F) {
      ...
      if (TYPE == 1) {
        if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
          ...
          // printf("w-store: %d+(%d %d %d) <- %f (%d %d %d)\n", other_offset_w, r_gl, c_gl, f_gl, dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)], r_sm, c_sm, f_sm);
        }
      } else if (TYPE == 2) { 
        f_gl *= 2;
        if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
          ...
          if (!skip) {
            ...
            if (CALC_COEFF) {
              ...
              else { res -= dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)]; }
            }
          }
          dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)] = res;
          // printf("w-store: %d+(%d %d %d) <- %f (%d %d %d)\n", other_offset_w, r_gl, c_gl, f_gl, dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)], r_sm, c_sm, f_sm);
    ...
    int base = 0;
    // printf("TYPE =%d \n", TYPE);
    // printf("%d == %d && %llu >= %d && %llu < %d\n", r + R * 2, nr_p - 1, threadId, base, threadId, base + C * F);
    
    if (dw && r + R * 2 == nr_p - 1 && threadId >= base && threadId < base + C * F) {
      r_sm = R * 2;
    ...
    if (dw && c + C * 2 == nc_p - 1 && threadId >= base && threadId < base + R * F) {
      ...
      //         r_sm, c_sm, f_sm, r_gl, c_gl, f_gl, dwork[get_idx(lddv1, lddv2, r_gl, c_gl, f_gl)]);
    }

    base += R * F;//ROUND_UP_WARP(R * F) * WARP_SIZE;
    // printf("%d %d\n", base,  threadId);
    if (dw && f + F * 2 == nf_p - 1 && threadId >= base && threadId < base + R * C) {
    ...
    // if (TYPE == 2) printf("%d %d, %d, %llu, %d\n",dw == NULL, f + F * 2, nf_p - 1, threadId, C);
        ...
        threadId >= base && threadId < base + C) 
    {
      ...
      // printf("store[%d %d %d]: %f\n", r_sm, c_sm, f_sm, v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)]);
    }
    ...
    if (dwf && threadId >= R * C * F && threadId < R * C * F * 2) 
    {
      ...
      if (TYPE == 1) {
        ...
        // if (nr == 70) printf("f-store: (%d %d %d) <- %f (%d %d %d)\n", r_gl, c_gl, f_gl, v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)], r_sm, c_sm, f_sm);
      // asm volatile("membar.cta;");
      // start = clock64() - start; 
    ...
    // asm volatile("membar.cta;");
    // if (threadId < 256 && blockIdx.z == 0 && blockIdx.y == 0 && blockIdx.x == 0) printf("threadId %d elapsed %lu\n", threadId, end-start);
    if (r + R * 2 == nr_p - 1) 
    {
      // printf("test\n");
      if (threadId < C * F) {
            ...
            if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
              ...
              if (!skip) {
                ...
                if (CALC_COEFF) { // need to test if in_next
                  if (in_next && f_gl < nf_c) { ; } // in_next
                  else { res -= dwf[get_idx(lddwf1, lddwf2, r_gl, c_gl, f_gl)]; }
                }
              }
              dwf[get_idx(lddwf1, lddwf2, r_gl, c_gl, f_gl)] = res;
            }
        ...
        // printf("(%d %d %d) (%d %d %d) %f\n",
      //         r_sm, c_sm, f_sm, r_gl, c_gl, f_gl, v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)]);
        if (dwcf) {
          r_sm = R * 2;
          c_sm = (threadId/F)*2+1;
          f_sm = (threadId%F)*2+1;
      ...
      if (threadId >= R * C * F * 5 && threadId < R * C * F * 5 + F) {
        ...
        if (TYPE == 1) {
          if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
            ...
            dwf[get_idx(lddwf1, lddwf2, r_gl, c_gl, f_gl)] = res;
            // printf("dwf(%d %d %d): %f\n", r_gl, c_gl, f_gl, dwf[get_idx(lddwf1, lddwf2, r_gl, c_gl, f_gl)]);
          }
        } else if (TYPE == 2) {
          f_gl = 2 * f_gl + 1;
          if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
...
template <typename T, int D, int R, int C, int F>
void gpk_reo_3d_adaptive_launcher(mgard_cuda_handle<T, D> &handle, int nr, int nc, int nf,
               T *dratio_r, T *dratio_c, T *dratio_f, 
               T *dv, int lddv1, int lddv2,
               T *dw, int lddw1, int lddw2,
               T *dwf, int lddwf1, int lddwf2,
  ...
  _gpk_reo_3d<T, D, R/2, C/2, F/2><<<blockPerGrid, threadsPerBlock, sm_size, 
               *(cudaStream_t *)handle.get(queue_idx)>>>( nr, nc, nf, 
                            nr_c, nc_c, nf_c, 
...
#define GPK(R, C, F){\
        gpk_reo_3d_adaptive_launcher<T, D, R, C, F>( handle, nr, nc, nf,\
                                              dratio_r, dratio_c, dratio_f,\
                                              dv, lddv1, lddv2,\
                                              dw, lddw1, lddw2,\
                                              dwf, lddwf1, lddwf2,\
...
#endif
if (D == 3) {
  if (profile || config == 6) { GPK(2, 2, 128) }
  if (profile || config == 5) { GPK(2, 2, 64) }
  if (profile || config == 4) { GPK(4, 4, 32) }
  if (profile || config == 3) { GPK(4, 4, 16) }
  if (profile || config == 2) { GPK(4, 4, 8) }
  if (profile || config == 1) { GPK(4, 4, 4) }
  if (profile || config == 0) { GPK(2, 2, 2) }
    // PI_QL(T, 4, 4, 4)
} else if (D == 2){
  if (profile || config == 6) { GPK(1, 2, 128) }
  if (profile || config == 5) { GPK(1, 2, 64) }
  if (profile || config == 4) { GPK(1, 4, 32) }
  if (profile || config == 3) { GPK(1, 4, 16) }
  if (profile || config == 2) { GPK(1, 4, 8) }
  if (profile || config == 1) { GPK(1, 4, 4) }
  if (profile || config == 0) { GPK(1, 2, 4) }
  // PI_QL(T, 1, 4, 4)
} else if (D == 1) {
  if (profile || config == 6) { GPK(1, 1, 128) }
  if (profile || config == 5) { GPK(1, 1, 64) }
  if (profile || config == 4) { GPK(1, 1, 32) }
  if (profile || config == 3) { GPK(1, 1, 16) }
  if (profile || config == 2) { GPK(1, 1, 8) }
  if (profile || config == 1) { GPK(1, 1, 8) }
  if (profile || config == 0) { GPK(1, 1, 8) }
}
#undef GPK
}
...
template <typename T, int D, int R, int C, int F>
__global__ void _gpk_rev_3d(int nr, int nc, int nf, 
                           int nr_c,  int nc_c, int nf_c,
                           T *dratio_r, T *dratio_c, T *dratio_f, 
                           T *dv, int lddv1, int lddv2,
                           T *dw, int lddw1, int lddw2,
                           ...
                           int nvr, int nvc, int nvf) 
{
    ...
    if (TYPE == 1) {
      if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
        ...
        if (!INTERPOLATION && COEFF_RESTORE) {
          ...
          // printf("block: (%d %d %d) thread: (%d %d %d) load0 (%d %d %d): %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
          //               v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)],
          //                 r_gl, c_gl, f_gl);
        }
      }
      ...
      if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
        ...
        if (!skip) {
          ...
          if (COEFF_RESTORE) {
            ...
            if (in_next && f_in_next) { v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)] = 0.0;}
            else { ; }
          }
        }
      }
    ...
    if (TYPE == 1) {
      if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
        ...
        if (!INTERPOLATION && COEFF_RESTORE) {
          ...
          dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)];
          // if (c_gl == nc_c - 1 && f_gl == nf_c-1)
          // printf("block: (%d %d %d) thread: (%d %d %d) load1 (%d %d %d): %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
          //               v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)],
          //                 r_gl, c_gl, f_gl);
        }
      }
    ...
    if (TYPE == 1) {
      if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
        ...
        if (!INTERPOLATION && COEFF_RESTORE) {
          ...
          dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)];
          // if (c_gl == nc_c - 1 && f_gl == nf_c-1)
          // printf("block: (%d %d %d) thread: (%d %d %d) load2 (%d %d %d): %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
          //               v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)],
          //                 r_gl, c_gl, f_gl);
        }
      }
    ...
    if (TYPE == 1) {
      if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
        ...
        if (!INTERPOLATION && COEFF_RESTORE) {
          ...
          dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)];
          // if (c_gl == nc_c - 1 && f_gl == nf_c-1)
          // printf("block: (%d %d %d) thread: (%d %d %d) load3 (%d %d %d): %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
          //               v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)],
          //                 r_gl, c_gl, f_gl);
        }
      }
    ...
    if (TYPE == 1) {
      if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
        ...
        if (!INTERPOLATION && COEFF_RESTORE) {
          ...
          dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)];
          // if (c_gl == nc_c - 1 && f_gl == nf_c-1)
          // printf("block: (%d %d %d) thread: (%d %d %d) load4 (%d %d %d): %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
          //               v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)],
          //                 r_gl, c_gl, f_gl);
        }
      }
    ...
    if (TYPE == 1) {
      if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
        ...
        if (!INTERPOLATION && COEFF_RESTORE) {
          ...
          dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)];
          // if (c_gl == nc_c - 1 && f_gl == nf_c-1)
          // printf("block: (%d %d %d) thread: (%d %d %d) load5 (%d %d %d): %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
          //               v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)],
          //                 r_gl, c_gl, f_gl);
        }
      }
    ...
    if (TYPE == 1) {
      if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
        ...
        if (!INTERPOLATION && COEFF_RESTORE) {
          ...
          dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)];
          // if (c_gl == nc_c - 1 && f_gl == nf_c-1)
          // printf("block: (%d %d %d) thread: (%d %d %d) load6 (%d %d %d): %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
          //               v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)],
          //                 r_gl, c_gl, f_gl);
        }
      }
    ...
    if (TYPE == 1) {
      if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
        ...
        if (!INTERPOLATION && COEFF_RESTORE) {
          ...
          dw[get_idx(lddw1, lddw2, r_gl, c_gl, f_gl)];
          // if (c_gl == nc_c - 1 && f_gl == nf_c-1)
          // printf("block: (%d %d %d) thread: (%d %d %d) load7 (%d %d %d): %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
          //               v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, f_sm)],
          //                 r_gl, c_gl, f_gl);
        }
      }
        ...
        if (!skip) {
          if (INTERPOLATION && COEFF_RESTORE) {
            res += lerp (v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
                         ...
                         ratio_r_sm[r_sm - 1]);   
            // if (c_gl == nc_c-1 && f_gl == nf_c - 1)
            //     printf("block: (%d %d %d) thread: (%d %d %d) calc_coeff0 (%d %d %d): %f <- %f %f\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
            //             res, v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
            //               v_sm[get_idx(ldsm1, ldsm2, r_sm + 1, c_sm, f_sm)]);
          } else if (INTERPOLATION && !COEFF_RESTORE) {
            res = lerp (v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
                        v_sm[get_idx(ldsm1, ldsm2, r_sm + 1, c_sm, f_sm)],
          ...
          if (r_sm < rest_r_p && c_sm < rest_c_p && f_sm < rest_f_p &&
            ...
            if (!skip) {
              if (INTERPOLATION) {
                ...
                //           v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm - 1, f_sm)], r_sm, c_sm - 1, f_sm, 
                //           v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm + 1, f_sm)], r_sm, c_sm + 1, f_sm,
                //           res);
                // }
              }
            ...
            if (!skip) {
              if (INTERPOLATION && COEFF_RESTORE) {
                res += lerp (v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
                             ...
                             ratio_r_sm[r_sm - 1]);
                // if (c_gl == nc_c-1 && f_gl == nf_c - 1)
                // printf("block: (%d %d %d) thread: (%d %d %d) calc_coeff1 (%d %d %d): %f <- %f %f\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
                //         res, v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
                //           v_sm[get_idx(ldsm1, ldsm2, r_sm + 1, c_sm, f_sm)]);
              } else if (INTERPOLATION && !COEFF_RESTORE) {
                res = lerp (v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
            ...
            if (!skip) {
              if (INTERPOLATION && COEFF_RESTORE) {
                res += lerp (v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
                             ...
                             ratio_r_sm[r_sm - 1]);
                // if (c_gl == nc_c-1 && f_gl == nf_c - 1)
                // printf("block: (%d %d %d) thread: (%d %d %d) calc_coeff2 (%d %d %d): %f <- %f %f\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
                //         res, v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
                //           v_sm[get_idx(ldsm1, ldsm2, r_sm + 1, c_sm, f_sm)]);
              } else if (INTERPOLATION && !COEFF_RESTORE) {
                res = lerp (v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
            ...
            if (!skip) {
              if (INTERPOLATION && COEFF_RESTORE) {
                res += lerp (v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
                             ...
                             ratio_r_sm[r_sm - 1]);
                // if (c_gl == nc_c-1 && f_gl == nf_c - 1)
                // printf("block: (%d %d %d) thread: (%d %d %d) calc_coeff3 (%d %d %d): %f <- %f %f\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, c_sm, f_sm,
                //         res, v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
                //           v_sm[get_idx(ldsm1, ldsm2, r_sm + 1, c_sm, f_sm)]);
              } else if (INTERPOLATION && !COEFF_RESTORE) {
                res = lerp (v_sm[get_idx(ldsm1, ldsm2, r_sm - 1, c_sm, f_sm)],
  ...
  if (nr % 2 == 0) unpadding_r -= 1;
  if (nc % 2 == 0) unpadding_c -= 1;
  if (nf % 2 == 0) unpadding_f -= 1;
  ...
  if (r_sm < unpadding_r && c_sm < unpadding_c && f_sm < unpadding_f) {
    ...
    //    case 2.a: block size + 1 >= rest (No need to store extra, but need un-padding first);
    //    case 2.b: block size + 1 < rest (No need to store extra);
    ...
    if (D >= 3 && r_sm == 0) {
      ...
      if (nr % 2 == 0 && R * 2 + 1 >= rest_r_p){
        v_sm[get_idx(ldsm1, ldsm2, rest_r-1, c_sm, f_sm)] =
        v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, c_sm, f_sm)];
        // if ( v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, c_sm, f_sm)] == 71177117) printf("un-padding0 error block: (%d %d %d) thread: (%d %d %d) un-padding (%d %d %d) %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, 
        //   rest_r-1, c_sm, f_sm,
        //     v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, c_sm, f_sm)], rest_r_p-1, c_sm, f_sm);
      }
    }
    ...
    if (D >= 2 && c_sm == 0) {
      ...
      if (nc % 2 == 0 && C * 2 + 1 >= rest_c_p){
        v_sm[get_idx(ldsm1, ldsm2, r_sm, rest_c-1, f_sm)] =
            v_sm[get_idx(ldsm1, ldsm2, r_sm, rest_c_p-1, f_sm)];
        if ( v_sm[get_idx(ldsm1, ldsm2, r_sm, rest_c_p-1, f_sm)] == 71177117) printf("un-padding1 error block: (%d %d %d) thread: (%d %d %d) un-padding (%d %d %d) %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, 
          r_sm, rest_c-1, f_sm,
            v_sm[get_idx(ldsm1, ldsm2, r_sm, rest_c_p-1, f_sm)], r_sm, rest_c_p-1, f_sm);
      }
    }
    ...
    if (D >= 1 && f_sm == 0) {
      ...
      if (nf % 2 == 0 && F * 2 + 1 >= rest_f_p && TYPE == 1){
        v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, rest_f-1)] =
            v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, rest_f_p-1)];
        // if ( v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, rest_f_p-1)] == 71177117) printf("un-padding2 error block: (%d %d %d) thread: (%d %d %d) un-padding (%d %d %d) %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, 
        //   r_sm, c_sm, rest_f-1,
        //     v_sm[get_idx(ldsm1, ldsm2, r_sm, c_sm, rest_f_p-1)], r_sm, c_sm, rest_f_p-1);
      }
    }
    ...
    if (D >= 2 && c_sm == 0 && f_sm == 0) {
      ...
      if (nc % 2 == 0 && nf % 2 == 0 && 
        ...
        v_sm[get_idx(ldsm1, ldsm2, r_sm, rest_c-1, rest_f-1)] =
            v_sm[get_idx(ldsm1, ldsm2, r_sm, rest_c_p-1, rest_f_p-1)];
        // printf("block: (%d %d %d) thread: (%d %d %d) un-padding (%d %d %d) %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_sm, rest_c-1, rest_f-1,
        //     v_sm[get_idx(ldsm1, ldsm2, r_sm, rest_c-1, rest_f-1)], r_sm, rest_c_p-1, rest_f_p-1);
      }
      if (nc % 2 == 0 && nf % 2 != 0 && 
    ...
    if (D >= 3 && r_sm == 0 && f_sm == 0) {
      ...
      if (nr % 2 == 0 && nf % 2 == 0 && 
          R * 2 + 1 >= rest_r_p && F * 2 + 1 >= rest_f_p && TYPE == 1) {
        v_sm[get_idx(ldsm1, ldsm2, rest_r-1, c_sm, rest_f-1)] =
            v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, c_sm, rest_f_p-1)];
        // if ( v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, c_sm, rest_f_p-1)] == 71177117) printf("un-padding3 error block: (%d %d %d) thread: (%d %d %d) un-padding (%d %d %d) %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, 
        //   rest_r-1, c_sm, rest_f-1,
        //     v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, c_sm, rest_f_p-1)], rest_r_p-1, c_sm, rest_f_p-1);
      }
      if (nr % 2 == 0 && nf % 2 != 0 && 
          R * 2 + 1 >= rest_r_p && F * 2 + 1 == rest_f) {
        if (!INTERPOLATION && COEFF_RESTORE) {
          dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl, f_gl_ex)] +=
              v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, c_sm, f_sm_ex)];
        } else {
      ...
      if (nr % 2 != 0 && nf % 2 == 0 && 
          R * 2 + 1 == rest_r && F * 2 + 1 >= rest_f_p && TYPE == 1) {
        if (!INTERPOLATION && COEFF_RESTORE) {
          dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl, f_gl_ex)] +=
              v_sm[get_idx(ldsm1, ldsm2, r_sm_ex, c_sm, rest_f_p-1)];
        } else {
    ...
    if(D >= 3 && r_sm == 0 && c_sm == 0) {
      ...
      if (nr % 2 == 0 && nc % 2 == 0 && 
          R * 2 + 1 >= rest_r_p && C * 2 + 1 >= rest_c_p) {
        v_sm[get_idx(ldsm1, ldsm2, rest_r-1, rest_c-1, f_sm)] =
            v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, rest_c_p-1, f_sm)];
        // if ( v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, rest_c_p-1, f_sm)] == 71177117) printf("un-padding4 error block: (%d %d %d) thread: (%d %d %d) un-padding (%d %d %d) %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, 
        //   rest_r-1, rest_c-1, f_sm,
        //     v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, rest_c_p-1, f_sm)], rest_r_p-1, rest_c_p-1, f_sm);
      }
      if (nr % 2 == 0 && nc % 2 != 0 && 
          R * 2 + 1 >= rest_r_p && C * 2 + 1 == rest_c) {
        if (!INTERPOLATION && COEFF_RESTORE) {
          dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl_ex, f_gl)] +=
              v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, c_sm_ex, f_sm)];
        } else {
    ...
    if (D >= 3 && r_sm == 0 && c_sm == 0 && f_sm == 0) {
      ...
      if (nr % 2 == 0 && nc % 2 == 0 && nf % 2 == 0 &&
        R * 2 + 1 >= rest_r_p && C * 2 + 1 >= rest_c_p && F * 2 + 1 >= rest_f_p && TYPE == 1) {
        v_sm[get_idx(ldsm1, ldsm2, rest_r-1, rest_c-1, rest_f-1)] =
            v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, rest_c_p-1, rest_f_p-1)];

        // printf("block: (%d %d %d) thread: (%d %d %d) un-padding (%d %d %d) %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, rest_r-1, rest_c-1, rest_f-1,
        //     v_sm[get_idx(ldsm1, ldsm2, r_sm, rest_c-1, rest_f-1)], rest_r_p-1, rest_c_p-1, rest_f_p-1);
      }
      if (nr % 2 == 0 && nc % 2 == 0 && nf % 2 != 0 &&
        R * 2 + 1 >= rest_r_p && C * 2 + 1 >= rest_c_p && F * 2 + 1 == rest_f) {
        if (!INTERPOLATION && COEFF_RESTORE) {
          dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl_ex, f_gl_ex)] +=
              v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, rest_c_p-1, f_sm_ex)];
        } else {
      ...
      if (nr % 2 == 0 && nc % 2 != 0 && nf % 2 == 0 &&
        R * 2 + 1 >= rest_r_p && C * 2 + 1 == rest_c && F * 2 + 1 >= rest_f_p && TYPE == 1) {
        if (!INTERPOLATION && COEFF_RESTORE) {
          dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl_ex, f_gl_ex)] +=
              v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, c_sm_ex, rest_f_p-1)];
        } else {
      ...
      if (nr % 2 != 0 && nc % 2 == 0 && nf % 2 == 0 &&
        R * 2 + 1 == rest_r && C * 2 + 1 >= rest_c_p && F * 2 + 1 >= rest_f_p && TYPE == 1) {
        if (!INTERPOLATION && COEFF_RESTORE) {
          dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl_ex, f_gl_ex)] +=
              v_sm[get_idx(ldsm1, ldsm2, r_sm_ex, rest_c_p-1, rest_f_p-1)];
        } else {
      ...
      if (nr % 2 == 0 && nc % 2 != 0 && nf % 2 != 0 &&
        R * 2 + 1 >= rest_r_p && C * 2 + 1 == rest_c && F * 2 + 1 == rest_f) {
        if (!INTERPOLATION && COEFF_RESTORE) {
          dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl_ex, f_gl_ex)] +=
              v_sm[get_idx(ldsm1, ldsm2, rest_r_p-1, c_sm_ex, f_sm_ex)];
        } else {
      ...
      if (nr % 2 != 0 && nc % 2 == 0 && nf % 2 != 0 &&
        R * 2 + 1 == rest_r && C * 2 + 1 >= rest_c_p && F * 2 + 1 == rest_f) {
        if (!INTERPOLATION && COEFF_RESTORE) {
          dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl_ex, f_gl_ex)] +=
              v_sm[get_idx(ldsm1, ldsm2, r_sm_ex, rest_c_p-1, f_sm_ex)];
        } else {
      ...
      if (nr % 2 != 0 && nc % 2 != 0 && nf % 2 == 0 &&
        R * 2 + 1 == rest_r && C * 2 + 1 == rest_c && F * 2 + 1 >= rest_f_p && TYPE == 1) {
        if (!INTERPOLATION && COEFF_RESTORE) {
          dv[get_idx(lddv1, lddv2, r_gl_ex, c_gl_ex, f_gl_ex)] +=
              v_sm[get_idx(ldsm1, ldsm2, r_sm_ex, c_sm_ex, rest_f_p-1)];
        } else {
  ...
  if (r_sm < rest_r && c_sm < rest_c && f_sm < rest_f) {
    if (r_gl >= svr && r_gl < svr+nvr &&
        c_gl >= svc && c_gl < svc+nvc &&
      ...
      if (!INTERPOLATION && COEFF_RESTORE) {
        ...
        //   printf("block: (%d %d %d) thread: (%d %d %d) store (%d %d %d) %f (%d %d %d)\n", blockIdx.z, blockIdx.y, blockIdx.x, threadIdx.z, threadIdx.y, threadIdx.x, r_gl, c_gl, f_gl,
...
template <typename T, int D, int R, int C, int F>
void gpk_rev_3d_adaptive_launcher(mgard_cuda_handle<T, D> &handle, int nr, int nc, int nf,
               T *dratio_r, T *dratio_c, T *dratio_f, 
               T *dv, int lddv1, int lddv2,
               T *dw, int lddw1, int lddw2,
               T *dwf, int lddwf1, int lddwf2,
  ...
  _gpk_rev_3d<T, D, R/2, C/2, F/2><<<blockPerGrid, threadsPerBlock, sm_size, 
               *(cudaStream_t *)handle.get(queue_idx)>>>( nr, nc, nf, 
                            nr_c, nc_c, nf_c, 
                            dratio_r, dratio_c, dratio_f, 
                            dv, lddv1, lddv2, 
...
#define GPK(R, C, F) {gpk_rev_3d_adaptive_launcher<T, D, R, C, F>(\
                            handle, nr, nc, nf,\
                            dratio_r, dratio_c, dratio_f,\
                            dv, lddv1, lddv2,\
                            dw, lddw1, lddw2,\
