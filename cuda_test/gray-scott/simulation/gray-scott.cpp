...
void GrayScott::init_field()
{
    ...
    const int d = 6;
    for (int z = settings.L3 / 2 - d; z < settings.L3 / 2 + d; z++) {
        for (int y = settings.L2 / 2 - d; y < settings.L2 / 2 + d; y++) {
...
void GrayScott::init_mpi()
{
    ...
    size_y = settings.L2 / npy;
    size_z = settings.L3 / npz;

    if (px < settings.L % npx) {
        size_x++;
    }
    if (py < settings.L2 % npy) {
        size_y++;
    }
    if (pz < settings.L3 % npz) {
        size_z++;
    }

    offset_x = (settings.L / npx * px) + std::min(settings.L % npx, px);
    offset_y = (settings.L2 / npy * py) + std::min(settings.L2 % npy, py);
    offset_z = (settings.L3 / npz * pz) + std::min(settings.L3 % npz, pz);
