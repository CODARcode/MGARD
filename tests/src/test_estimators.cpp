...
TEST_CASE("comparison with Python implementation: estimators", "[estimators]") {
  ...
  REQUIRE(mgard::estimator(u_mc, hierarchy, -1.5).unscaled ==
          Approx(14.408831895461581));
  REQUIRE(mgard::estimator(u_mc, hierarchy, -1.0).unscaled ==
          Approx(14.414586335331334));
  REQUIRE(mgard::estimator(u_mc, hierarchy, -0.5).unscaled ==
          Approx(14.454292254136842));
  REQUIRE(mgard::estimator(u_mc, hierarchy, 0.0).unscaled ==
...
TEST_CASE("estimators should track norms", "[estimators]") {
  ...
  for (std::size_t i = 0; i < smoothness_parameters.size(); ++i) {
    ...
    const mgard::SandwichBounds sandwich = mgard::estimator(u_mc, hierarchy, s);
