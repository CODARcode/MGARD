...
/*
    ...
    std::cout << "total number of devices: " << deviceCount << ", rank " << rank << " used " << device_id << "\n"; 
*/
    ...
    size_t num_iter       = (size_t)(std::ceil)((double)gb_elements*sizeof(double)/1024.0/1024.0/1024.0/job_sz/np_size);
    size_t div_nnodes     = (size_t)(std::ceil)((double)nnodes / num_iter);
    size_t iter_nnodes    = (size_t)(std::ceil)((double)div_nnodes / (double)np_size);
//    size_t iter_elements = iter_nnodes * vx * vy * nphi;
    size_t local_nnodes   = (rank==np_size-1) ? (div_nnodes - rank*iter_nnodes) : iter_nnodes;
    size_t local_elements = nphi * vx * local_nnodes * vy;
    size_t lSize          = sizeof(double) * gb_elements; 
    double *in_buff;
    mgard_cuda::cudaMallocHostHelper((void **)&in_buff, sizeof(double) * local_elements);
    if (rank == 0) {
        std::cout <<"total data size: {" << nphi << ", " << vx << ", " << nnodes << ", " << vy << "}, number of iters: " << num_iter << "\n";
    }
    size_t out_size = 0;
    for (size_t iter=0; iter<num_iter; iter++) {
        if (iter == num_iter-1) { 
            iter_nnodes   = (size_t)(std::ceil)(((double)(nnodes - div_nnodes*iter)) / (double)np_size);//local_nnodes - iter_nnodes*iter;
            local_nnodes  = (rank==np_size-1) ? (nnodes - div_nnodes*iter - iter_nnodes * (np_size-1)) : iter_nnodes;
            local_elements = local_nnodes * vx * vy * nphi;
        }
        std::vector<size_t> shape = {nphi, vx, local_nnodes, vy};
        std::cout << "rank " << rank << " read from {0, 0, " << div_nnodes*iter + iter_nnodes*rank << ", 0} for {" << nphi << ", " << vx << ", " << local_nnodes << ", " << vy << "}\n";
        var_i_f_in.SetSelection(adios2::Box<adios2::Dims>({0, 0, div_nnodes*iter + iter_nnodes*rank, 0}, {nphi, vx, local_nnodes, vy}));
...
//        std::cout << "loadData: " << shape[0] << ", " << shape[1] << ", " << shape[2] << ", " << shape[3] << "\n";
...
//        std::cout << "after compression\n"; 
        ...
        memcpy(mgard_out_buff, out_array.getDataHost(), local_elements * sizeof(double));
    ...
    if (rank == 0) {
        std::cout << " CPU to GPU time: " << gpu_in_time << ", compression time: " << gpu_compress_time << ", decompress time: " << gpu_decompress_time << "\n";
    }

    mgard_cuda::cudaFreeHostHelper(in_buff);
    size_t gb_compressed;
    MPI_Allreduce(&out_size, &gb_compressed, 1, MPI_UNSIGNED_LONG, MPI_SUM, MPI_COMM_WORLD);
    if (rank == 0) {
...
//    MPI_Exscan(&out_size, &exscan, 1, MPI_UNSIGNED_LONG, MPI_SUM, MPI_COMM_WORLD);
//    std::cout << "rank " << rank << " compressed size: " << out_size << ", exscan: " << exscan << ", total compressed: " << gb_compressed << "\n";
//    MPI_Gather(&exscan, 1, MPI_UNSIGNED_LONG, scan_counts, 1, MPI_UNSIGNED_LONG, 0, MPI_COMM_WORLD);
//    if (rank == 0) {
//        scan_counts[0] = 0;
//        exscan = 0;
...
//    adios2::Variable<unsigned char> bp_fdata = bpIO.DefineVariable<unsigned char>(
//      "mgard_f", {gb_compressed}, {exscan}, {out_size},  adios2::ConstantDims);
//    writer.Put<unsigned char>(bp_fdata, mgard_compress_buff);
//    if (rank==0) {
//        adios2::Variable<size_t> bp_count = bpIO.DefineVariable<size_t>(
//        "block_start", {(size_t)np_size}, {0}, {(size_t)np_size}, adios2::ConstantDims);
//        writer.Put<size_t>(bp_count, scan_counts);
//    }
//    writer.Close();
