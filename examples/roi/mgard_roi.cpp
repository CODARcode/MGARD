...
int main(int argc, char **argv) {
    ...
    adios2::Variable<float>var_out = writer_io.DefineVariable<float>(var_name, shape, {0, 0, 0}, shape);
    lSize += shape[0] * shape[1] * shape[2];
    float s = 0.0;
    if (ndims==3) {
        float max_v = -1e8, min_v = 1e8;
        std::vector<float> var_in;
        if ((np_size==1) || ((np_size%4 ==0) && (((rank%4==0) && (shape[2]==240)) || (shape[2]==960)))) {
            ...
            var_ad2.SetSelection(adios2::Box<adios2::Dims>({n_t*dim_t, 0, n_c*dim_c}, {dim_t, shape[1], dim_c}));
            reader.Get<float>(var_ad2, var_in, adios2::Mode::Sync);
            reader.PerformGets();
            var_out.SetSelection(adios2::Box<adios2::Dims>({n_t*dim_t, 0, n_c*dim_c}, {dim_t, shape[1], dim_c}));
            ...
            if (rank==0) {
                std::cout << "Variable " << var_name << ", shape = {" << shape[0] << ", " << shape[1] << ", " << shape[2] << "}," ;
                std::cout << "Get {" << dim_t << ", " << shape[1] << ", " << dim_c << "}\n";
                std::cout << "absolute tol: " << abs_tol << ", maxv: " << gb_maxv << ", min_v: " << gb_minv << "\n";
            }
            if ((np_size==1) || ((np_size%4 ==0) && (((rank%4==0) && (shape[2]==240)) || (shape[2]==960)))) {
                const std::array<std::size_t, 3> dims = {dim_t, shape[1], dim_c};
                const mgard::TensorMeshHierarchy<3, float> hierarchy(dims);
                if (rank==0) std::cout << "Lmax: " << hierarchy.L << "\n";
                const mgard::CompressedDataset<3, float> compressed = mgard::compress_roi(hierarchy, var_in.data(), s, abs_tol, thresh, init_bw, bw_ratio, l_th, NULL, false);
            ...
            while (r_step<shape[0]) {
                float max_v = -1e8, min_v = 1e8;
                std::vector<float> var_in;
                var_ad2.SetSelection(adios2::Box<adios2::Dims>({r_step, 0, 0}, {1, shape[1], shape[2]}));
                reader.Get<float>(var_ad2, var_in, adios2::Mode::Sync);
                reader.PerformGets();
                var_out.SetSelection(adios2::Box<adios2::Dims>({r_step, 0, 0}, {1, shape[1], shape[2]}));
                ...
                abs_tol = (gb_maxv-gb_minv) * tol;
                if (rank==0) {
                    std::cout << "Get " << var_name << ", shape = {" << shape[0] << ", " << shape[1] << ", " << shape[2] << "}\n";
                    std::cout << "absolute tol: " << abs_tol << ", maxv: " << gb_maxv << ", min_v: " << gb_minv << "\n";
                }

                const mgard::CompressedDataset<2, float> compressed = mgard::compress_roi(hierarchy, var_in.data(), s, abs_tol, thresh, init_bw, bw_ratio, l_th, NULL, false);
                const mgard::DecompressedDataset<2, float> decompressed = mgard::decompress(compressed);
                writer.Put<float>(var_out, decompressed.data(), adios2::Mode::Sync);
                writer.PerformPuts();
                compressed_size += compressed.size();
                r_step += np_size;
                if (rank==0) std::cout << r_step << "\n";
            } 
        }
    ...
    MPI_Allreduce(&compressed_size, &gb_compressed, 1, MPI_UNSIGNED_LONG, MPI_SUM, MPI_COMM_WORLD);
    if (rank == 0) {
         printf("%s: compression ratio = %.4f\n", var_name.c_str(), ((double)lSize*4) / gb_compressed);
    }
