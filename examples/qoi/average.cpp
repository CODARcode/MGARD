...
int main(int argc, char **argv) {
  ...
  std::cout << "average using decompressed data: "
    << average(hierarchy, decompressed.data()) << ", CR = " << vx*vy*4/(compressed.size()) << std::endl;
  float err = std::abs(average(hierarchy, decompressed.data()) - average(hierarchy, u));
  std::cout << "real error of QoI (average) = " << err << "\n";
  if (err < tol) std::cout << "********** Successful **********\n"; 
  else std::cout << "********** Fail with error preservation **********\n";
  std::free(u);
