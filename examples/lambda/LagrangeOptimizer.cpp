...
double* LagrangeOptimizer::computeLagrangeParameters(
    const double* reconData)
{
    ...
    for (iphi=0; iphi<myPlaneCount; ++iphi) {
        ...
        private (count_unLag, breg_recon)
...
void LagrangeOptimizer::setDataFromCharBufferV1(double* &reconData,
    const double* bufferIn, const char* meshFile)
{
    // size_t bufferOffset = getPQIndexes(bufferIn);
    // FILE* fp = fopen("PqMeshInfo.bin", "rb");
    // fread(&myNumClusters, sizeof(int), 1, fp);
        ...
        const double* bregData)
{
    ...
    if (my_rank == 0) {
        std::cout << "Compare errors for reconstructed data (first number) and post processed data (second number)" << std::endl;
    }
