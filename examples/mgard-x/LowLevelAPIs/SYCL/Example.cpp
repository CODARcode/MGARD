...
int main() {
  ...
  mgard_x::Hierarchy<3, double, mgard_x::SYCL> hierarchy(shape, config);
  mgard_x::Array<3, double, mgard_x::SYCL> in_array(shape);
  in_array.load(in_array_cpu);
  std::cout << "Done\n";

  std::cout << "Compressing with MGARD-X SYCL backend...";
  double tol = 0.01, s = 0, norm;
  mgard_x::Compressor compressor(hierarchy, config);
  mgard_x::Array<1, unsigned char, mgard_x::SYCL> compressed_array;
  compressor.Compress(in_array, mgard_x::error_bound_type::REL, tol, s, norm,
                      compressed_array, 0);
  mgard_x::DeviceRuntime<mgard_x::SYCL>::SyncQueue(0);
  ...
  std::cout << "Decompressing with MGARD-X SYCL backend...";
  // decompression
  mgard_x::Array<3, double, mgard_x::SYCL> decompressed_array;
