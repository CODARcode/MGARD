...
int main() {
  ...
  mgard_cuda::Array<3, double, CUDA> in_array(shape);
  in_array.loadData(in_array_cpu);
  std::cout << "Done\n";

  std::cout << "Compressing with MGARD-GPU...";
  double tol = 0.01, s = 0;
  mgard_cuda::Array<1, unsigned char, CUDA> compressed_array = mgard_cuda::compress(
  ...
  // decompression
  mgard_cuda::Array<3, double, CUDA> decompressed_array =
