...
void test_vtkm(int argc, char *argv[], std::vector<mgard_x::SIZE> shape, T *original_data, T iso_value,
  ...
  int index = 0;
  for (vtkm::Id cellId = 0; cellId < TriangleCells.GetNumberOfCells(); cellId++) {
    vtkm::Id ids[3];
  ...
  vtkm::cont::ArrayHandle<vtkm::Vec3f> PointArray = PointField.GetData(). template AsArrayHandle<vtkm::cont::ArrayHandle<vtkm::Vec3f>>();
...
int main(int argc, char *argv[]) {
  ...
  // float iso_value = 2e6;
  // std::cout << "test_vtkm\n";
  // test_vtkm(argc, argv, shape, original_data, iso_value, numTriangles_vtkm, Triangles_vtkm, numPoints_vtkm, Points_vtkm);
  std::cout << "test_mine\n";
  test_mine(shape, original_data, iso_value, numTriangles_mine, Triangles_mine, numPoints_mine, Points_mine);

  // if (numTriangles_vtkm != numTriangles_mine) { printf("numTriangles mismatch! %u vs. %u\n", numTriangles_vtkm, Triangles_mine);}
  // bool match = true;
  ...
  // if (numPoints_vtkm != numPoints_mine) { printf("numPoints mismatch! %u vs. %u\n", numPoints_vtkm, numPoints_mine);}
  // match = true;
