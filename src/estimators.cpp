...
//!Compute the bounds relating the `s` indicators (the norms computed from them)
//!to the `s` estimators on a given mesh hierarchy.
//!
//!\param [in] hierarchy Mesh hierarchy on which the estimators and norms are
//!computed.
...
static SandwichBounds s_estimator(
    ...
    return SandwichBounds(
        s_estimator_bounds(hierarchy), std::sqrt(square_estimate)
    );
}
