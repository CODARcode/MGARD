...
inline double interp_1d(double x, double x1, double x2, double q00,
                        double q01) {
...
inline double decode(short x) { return double(double(2 * x + 1.0) / 65535.0); }
...
void qwrite_2D_bin(const int nrow, const int ncol, const int nlevel,
  ...
  // std::cout  << "Pruned : "<< prune_count << " Reduction : " << (double)
...
// void pi_lminus1_l(const int  l, std::vector<double>& v, const
...
double ml2_norm3(const int l, int nr, int nc, int nf, int nrow, int ncol,
  ...
  //    double norm = ( std::inner_product( v.begin(), v.end(), work.begin(),
  //    0.0d)  );
...
// void
// refactor_1D ( const int l_target, std::vector<double>& v,
// std::vector<double>& work,  std::vector<double>& coords, int n, int no)
// {
...
//  void refactor_2D_full(const int nr, const int nc, const int nrow, const int
//  ncol,  const int l_target, double* v, std::vector<double>& work,
//  std::vector<double>& coords_x, std::vector<double>& coords_y,
//  std::vector<double>& row_vec, std::vector<double>& col_vec )
// {
//     //refactor
...
//        // -> change funcs in pi_QL to use _l functions, otherwise distances
//        are wrong!!! pi_Ql(nr, nc, nrow, ncol, l, v, coords_x, coords_y,
//        row_vec, col_vec); //rename!. v@l has I-\Pi_l Q_l+1 u
...
//                 mgard_gen::mass_mult_l(l,  col_vec, coords_y, nr, nrow);
//                 mgard_gen::restriction_l(l+1, col_vec, coords_y, nr, nrow);
//                 mgard_gen::solve_tridiag_M_l(l+1,  col_vec, coords_y, nr,
//                 nrow);
...
void refactor_3D(const int nr, const int nc, const int nf, const int nrow,
  ...
  double copy_slice_time = 0.0;
  double copy_from_slice_time = 0.0;
  ...
  double mass_mult_l_fib_time = 0.0;
  double restriction_l_fib_time = 0.0;
  double solve_tridiag_M_l_fib_time = 0.0;
  ...
  for (int l = 0; l < l_target; ++l) {
    ...
    mass_mult_l_fib_time = 0.0;
    restriction_l_fib_time = 0.0;
    solve_tridiag_M_l_fib_time = 0.0;
    ...
    for (int kfib = 0; kfib < nf; kfib += stride) {
      ...
      for (int irow = 0; irow < nr; ++irow) {
        ...
        elapsed = end - start;
        copy_from_slice_time += elapsed.count();
      }
    ...
    for (int irow = 0; irow < nr; irow += Cstride) {
      ...
      for (int jcol = 0; jcol < nc; jcol += Cstride) {
        ...
        elapsed = end - start;
        mass_mult_l_fib_time += elapsed.count();

        start = std::chrono::high_resolution_clock::now();
        mgard_gen::restriction_l(l + 1, fib_vec, coords_z, nf, nfib);
        end = std::chrono::high_resolution_clock::now();
        elapsed = end - start;
        restriction_l_fib_time += elapsed.count();

        start = std::chrono::high_resolution_clock::now();
        mgard_gen::solve_tridiag_M_l(l + 1, fib_vec, coords_z, nf, nfib);
        end = std::chrono::high_resolution_clock::now();
        elapsed = end - start;
        solve_tridiag_M_l_fib_time += elapsed.count();
    ...
    timing_results << l << ",copy_slice," << copy_slice_time << std::endl;
    timing_results << l << ",copy_from_slice," << copy_from_slice_time << std::endl;

    timing_results << l << ",pi_Ql," << pi_Ql_time << std::endl;
    timing_results << l << ",copy_level_l," << copy_level_l_time << std::endl;
    timing_results << l << ",assign_num_level_l," << assign_num_level_l_time << std::endl;

    timing_results << l << ",mass_mult_l_row," << mass_mult_l_row_time << std::endl;
    timing_results << l << ",restriction_l_row," << restriction_l_row_time << std::endl;
    timing_results << l << ",solve_tridiag_M_l_row," << solve_tridiag_M_l_row_time << std::endl;

    timing_results << l << ",mass_mult_l_col," << mass_mult_l_col_time << std::endl;
    timing_results << l << ",restriction_l_col," << restriction_l_col_time << std::endl;
    timing_results << l << ",solve_tridiag_M_l_col," << solve_tridiag_M_l_col_time << std::endl;

    timing_results << l << ",mass_mult_l_fib," << mass_mult_l_fib_time << std::endl;
    timing_results << l << ",restriction_l_fib," << restriction_l_fib_time << std::endl;
    timing_results << l << ",solve_tridiag_M_l_fib," << solve_tridiag_M_l_fib_time << std::endl;

    timing_results << l << ",add_level_l," << add_level_l_time << std::endl;

    total_time += copy_slice_time;
    total_time += copy_from_slice_time;

    total_time += pi_Ql_time;
    total_time += copy_level_l_time;
    total_time += assign_num_level_l_time;

    total_time += mass_mult_l_row_time;
    total_time += restriction_l_row_time;
    total_time += solve_tridiag_M_l_row_time;

    total_time += mass_mult_l_col_time;
    total_time += restriction_l_col_time;
    total_time += solve_tridiag_M_l_col_time;

    total_time += mass_mult_l_fib_time;
    total_time += restriction_l_fib_time;
    total_time += solve_tridiag_M_l_fib_time;

    total_time += add_level_l_time;
  }
...
void recompose_3D(const int nr, const int nc, const int nf, const int nrow,
  ...
  double prolongate_l_fib_time = 0.0;
  ...
  for (int l = l_target; l > 0; --l) {
    ...
    for (int irow = 0; irow < nr; irow += Pstride) {
      ...
      for (int jcol = 0; jcol < nc; jcol += Pstride) {
        ...
        elapsed = end - start;
        prolongate_l_fib_time += elapsed.count();
    ...
    timing_results << l << ",subtract_level_l," << subtract_level_l_time << std::endl;

    timing_results << l << ",prolongate_l_row," << prolongate_l_row_time << std::endl;
    timing_results << l << ",prolongate_l_col," << prolongate_l_col_time << std::endl;
    timing_results << l << ",prolongate_l_fib," << prolongate_l_fib_time << std::endl;

    timing_results << l << ",assign_num_level_l," << assign_num_level_l_time2 << std::endl;
    timing_results << l << ",subtract_level_l," << subtract_level_l_time2 << std::endl;
    ...
    total_time += subtract_level_l_time;

    total_time += prolongate_l_row_time;
    total_time += prolongate_l_col_time;
    total_time += prolongate_l_fib_time;

    total_time += assign_num_level_l_time2;
    total_time += subtract_level_l_time2;
  }
  timing_results.close();
}
...
void project_non_canon(const int nr, const int nc, const int nf, const int nrow,
  ...
  //  nfib);
  // double norm = mgard_gen::ml2_norm3(0, nr, nc, nf, nrow, ncol, nfib, work,
...
void project_3D(const int nr, const int nc, const int nf, const int nrow,
                ...
                std::vector<double> &work, std::vector<double> &work2d,
                std::vector<double> &coords_x, std::vector<double> &coords_y,
                std::vector<double> &coords_z, std::vector<double> &norm_vec) {
  ...
  // nfib); double norm = mgard_gen::ml2_norm3_ng(0, nr, nc, nf, nrow, ncol,
  ...
  double norm = mgard_gen::ml2_norm3(l_target + 1, nr, nc, nf, nrow, ncol, nfib,
                                     work, coords_x, coords_y, coords_z);
...
double qoi_norm(int nrow, int ncol, int nfib, std::vector<double> &coords_x,
  ...
  std::vector<double> work2d(nrow * ncol), temp(nsize), row_vec(ncol),
  ...
  double norm22 = mgard_gen::ml2_norm3(0, nrow, ncol, nfib, nrow, ncol, nfib,
                                       xi, coords_x, coords_y, coords_z);
  ...
  double norm = mgard_gen::ml2_norm3(0, nrow, ncol, nfib, nrow, ncol, nfib, xi,
                                     coords_x, coords_y, coords_z);
  norm_vec[0] = norm;
  ...
  double norm0 = mgard_gen::ml2_norm3(nlevel, nr, nc, nf, nrow, ncol, nfib, xi,
                                      coords_x, coords_y, coords_z);
