...
UniformRestriction::UniformRestriction(
    const MeshLevel &MESH, const MeshLevel &mesh
):
    LinearOperator(MESH.ndof(), mesh.ndof()),
    MESH(MESH),
    mesh(mesh)
{
}

void UniformRestriction::do_operator_parentheses(
