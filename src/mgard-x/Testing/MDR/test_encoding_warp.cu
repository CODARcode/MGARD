...
template <typename T_data, typename T_bitplane,
          mgard_x::OPTION NumGroupsPerWarpPerIter, mgard_x::SIZE NumWarpsPerTB,
          mgard_x::OPTION BinaryType, mgard_x::OPTION DataEncodingAlgorithm,
          mgard_x::OPTION ErrorCollectingAlgorithm,
          mgard_x::OPTION DataDecodingAlgorithm>
void test(mgard_x::LENGTH n, mgard_x::SIZE encoding_num_bitplanes,
          mgard_x::SIZE decoding_num_bitplanes, int warmup, int repeat) {
  ...
  printf("T_data: %d, T_bitplane: %d, NumGroups: %u, NumWarps: %u, BinaryType: "
         "%d, Encoding: %d, Error: %d, Decoding: %d, n: %u, Total data: %.2e "
         "GB.\n",
         sizeof(T_data) * 8, sizeof(T_bitplane) * 8, NumGroupsPerWarpPerIter,
         NumWarpsPerTB, BinaryType, DataEncodingAlgorithm,
         ErrorCollectingAlgorithm, DataDecodingAlgorithm, n, total_data);
  ...
  // path.erase (path.end()-std::string("test_encoding_warp.cu").length(),
  // path.end()); ofs.open (path + "/encoding_perf_results/" + filename,
  // std::ofstream::out ); if(!ofs) std::cout<<"Writing to file
  // failed"<<std::endl;
  ...
  // CPU
  {
    ...
    auto encoder =
        mgard_x::MDR::GroupedBPEncoder<3, T_data, T_bitplane>(handle);
    t1 = high_resolution_clock::now();
    auto streams = encoder.encode(v, n, exp, encoding_num_bitplanes,
                                  stream_sizes, level_sq_err);
    t2 = high_resolution_clock::now();
    time_span = duration_cast<duration<double>>(t2 - t1);
    std::cout << "CPU Encoding time: " << time_span.count() << " s ("
              << total_data / time_span.count() << "GB/s)\n";
    ofs << total_data / time_span.count() << ",";

    std::vector<uint8_t const *> const_streams;
    for (int i = 0; i < streams.size(); i++)
      const_streams.push_back(streams[i]);

    total_data = (n * decoding_num_bitplanes) / 8 / 1e9;
    t1 = high_resolution_clock::now();
    auto level_decoded_data = encoder.progressive_decode(
        const_streams, n, exp, starting_bitplane, decoding_num_bitplanes, 0);
    t2 = high_resolution_clock::now();
    time_span = duration_cast<duration<double>>(t2 - t1);
    std::cout << "CPU Decoding time: " << time_span.count() << " s ("
              << total_data / time_span.count() << "GB/s)\n";
    ofs << total_data / time_span.count() << ",";
  ...
  // GPU-Warp
  {
    mgard_x::MDR::GroupedWarpEncoder<T_data, T_bitplane, T_error,
                                     ...
                                     BinaryType, DataEncodingAlgorithm,
                                     ErrorCollectingAlgorithm, mgard_x::CUDA>
        encoder;

    mgard_x::MDR::GroupedWarpDecoder<
        T_data, T_bitplane, NumGroupsPerWarpPerIter, NumWarpsPerTB, BinaryType,
        DataDecodingAlgorithm, mgard_x::CUDA>
        decoder;

    mgard_x::Array<2, T_error, mgard_x::CUDA> level_errors_work_array(
        {encoding_num_bitplanes + 1, MGARDX_NUM_SMs});
    ...
    for (int r = 0; r < warmup; r++) {
      encoder.Execute(n, encoding_num_bitplanes, exp, v_subarray,
                      encoded_bitplanes_subarray, level_errors,
                      level_errors_work, 0);
    }
    ...
    std::cout << "GPU Encoding time: " << time_span.count() << " s ("
              << total_data / time_span.count() << "GB/s)\n";
    ofs << total_data / time_span.count() << ",";
    ...
    for (int r = 0; r < warmup; r++) {
      decoder.Execute(n, starting_bitplane, decoding_num_bitplanes, exp,
                      encoded_bitplanes_subarray, signs_subarray, v_subarray,
                      0);
    }
    ...
    t2 = high_resolution_clock::now();
    time_span = duration_cast<duration<double>>(t2 - t1) / repeat;
    std::cout << "GPU Decoding time: " << time_span.count() << " s ("
              << total_data / time_span.count() << "GB/s)\n";
    ofs << total_data / time_span.count() << "\n";
  }
  ...
  if (check_correctness) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      T_data shifted_data =
          ldexp(cur_data, (int)encoding_num_bitplanes - (int)exp);
      T_fp fp_data;
      ...
      if (BinaryType == BINARY) {
        ...
        cur_data2 = ldexp((T_data)mgard_x::negabinary2binary(fp_data),
                          -encoding_num_bitplanes + exp);
...

      {
        ...
        // printf("fp: %u error: \n", fp_data);
        for (int bitplane_idx = 0; bitplane_idx < encoding_num_bitplanes;
             bitplane_idx++) {
          ...
          if (BinaryType == BINARY) {
            ...
            diff =
                (T_error)mgard_x::negabinary2binary(ngb_data & mask) + mantissa;
          }
...
void test_method() {
  ...
  // mgard_x::SIZE N = 512*1024*1024;
  {
    ...
    //       DataDecodingAlgorithm> (N, 32, 10, warmup, repeat);

    // test<float, uint32_t, 32, 2,
    //       BinaryType, DataEncodingAlgorithm, ErrorCollectingAlgorithm,
    //       DataDecodingAlgorithm> (N, 32, 32, warmup, repeat);
    ...
    test<float, uint32_t, 4, 16, BinaryType, DataEncodingAlgorithm,
         ErrorCollectingAlgorithm, DataDecodingAlgorithm>(N, 32, 32, warmup,
                                                          repeat);
    ...
    // for (mgard_x::SIZE num_bitplanes = 1; num_bitplanes <= 32;
    // num_bitplanes++) {
    //   test<float, uint32_t, 4, 16,
    //       BinaryType, DataEncodingAlgorithm, ErrorCollectingAlgorithm,
    //       DataDecodingAlgorithm> (N, num_bitplanes, num_bitplanes, warmup,
    //       repeat);
    // }
  }
}
...
int main() {
  // test_method<BINARY, Warp_Bit_Transpose_Serial_All,
  // Error_Collecting_Serial_All, Warp_Bit_Transpose_Serial_All>();
  // test_method<BINARY, Warp_Bit_Transpose_Parallel_B_Serial_b,
  // Warp_Error_Collecting_Serial_All,
  // Warp_Bit_Transpose_Parallel_B_Serial_b>();
  test_method<BINARY, Warp_Bit_Transpose_Parallel_B_Serial_b,
              Warp_Error_Collecting_Parallel_Bitplanes_Serial_Error,
              Warp_Bit_Transpose_Parallel_B_Serial_b>();
  // test_method<BINARY, Warp_Bit_Transpose_Parallel_B_Serial_b,
  // Warp_Error_Collecting_Serial_Bitplanes_Atomic_Error,
  // Warp_Bit_Transpose_Parallel_B_Serial_b>(); test_method<BINARY,
  // Warp_Bit_Transpose_Parallel_B_Serial_b,
  // Warp_Error_Collecting_Serial_Bitplanes_Reduce_Error,
  // Warp_Bit_Transpose_Parallel_B_Serial_b>(); test_method<BINARY,
  // Warp_Bit_Transpose_Serial_B_Atomic_b, Error_Collecting_Serial_All,
  // Warp_Bit_Transpose_Serial_B_Atomic_b>(); test_method<BINARY,
  // Warp_Bit_Transpose_Serial_B_Reduce_b, Error_Collecting_Serial_All,
  // Warp_Bit_Transpose_Serial_B_Reduce_b>(); test_method<BINARY,
  // Warp_Bit_Transpose_Serial_B_Ballot_b, Error_Collecting_Serial_All,
  // Warp_Bit_Transpose_Serial_B_Ballot_b>();

  // test_method<NEGABINARY, Warp_Bit_Transpose_Serial_All,
  // Error_Collecting_Disable, Warp_Bit_Transpose_Serial_All>();
  // test_method<NEGABINARY, Warp_Bit_Transpose_Parallel_B_Serial_b,
  // Error_Collecting_Disable, Warp_Bit_Transpose_Parallel_B_Serial_b>();
  // test_method<NEGABINARY, Warp_Bit_Transpose_Serial_B_Atomic_b,
  // Error_Collecting_Disable, Warp_Bit_Transpose_Serial_B_Atomic_b>();
  // test_method<NEGABINARY, Warp_Bit_Transpose_Serial_B_Reduce_b,
  // Error_Collecting_Disable, Warp_Bit_Transpose_Serial_B_Reduce_b>();
  // test_method<NEGABINARY, Warp_Bit_Transpose_Serial_B_Ballot_b,
  // Error_Collecting_Disable, Warp_Bit_Transpose_Serial_B_Ballot_b>();
