...
#define KERNELS(D, T)                                        \
template void compress<D, T, CUDA>(std::vector<SIZE> shape, T tol, T s, enum error_bound_type mode,\
...
template void decompress<D, T, CUDA>(std::vector<SIZE> shape, const void *compressed_data,\
