...
template <bool isNP2>
__device__ void loadSharedChunkFromMem(double *s_data,
...
template <bool isNP2>
__device__ void storeSharedChunkToMem(double* g_odata, 
                                      const double* s_data,
                                      int n, 
                                      int ai, int bi, 
                                      int mem_ai, int mem_bi,
...
__device__ void clearFirstElement(double* s_data, 
                                 double *g_blockSums, 
                                 int blockIndex)
{
    if (threadIdx.x == 0)
...
__device__ unsigned int buildSum(double *s_data)
{
...
__device__ void scanRootToLeaves(double *s_data, unsigned int stride)
{
     unsigned int thid = threadIdx.x;
...
template <bool storeSum>
__device__ void postscanBlock(double *data, int blockIndex, double *blockSums)
{
    int stride = buildSum(data);               // build the sum in place up the tree
    clearFirstElement<storeSum>(data, blockSums, 
                               (blockIndex == 0) ? blockIdx.x : blockIndex);
    scanRootToLeaves(data, stride);            // traverse down tree to build the scan 
}

template <bool storeSum, bool isNP2>
__global__ void postscan(double *g_odata, 
                        const double *g_idata, 
                        double *g_blockSums, 
                        int n, 
                        int blockIndex, 
                        int baseIndex)
{
    ...
    postscanBlock<storeSum>(s_data, blockIndex, g_blockSums); 
...
__global__ void uniformMultiply(double *g_data, 
                           double *uniforms, 
                           int n, 
                           int blockOffset, 
                           int baseIndex)
{
    __shared__ double uni;
    if (threadIdx.x == 0)
