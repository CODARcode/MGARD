...
// SINGLE PRECISON version of MGARD API //
...
float *mgard_decompress(float itype_flag,  float& quantizer, unsigned char *data, int data_len, int nrow, int ncol, int nfib)
{
  assert (ncol > 3);
  assert (nrow >= 1);
      ...
      if(nrow > 1 && ncol > 1 && nfib > 1)
        {
          ...
          mgard_decompressed_ptr = mgard::recompose_udq(1.0, nrow, ncol, nfib, data, data_len);
          return mgard_decompressed_ptr;      
        }
      else if (nrow > 1 && ncol > 1)
        {
          assert (nrow > 3);
          assert (ncol > 3);
          mgard_decompressed_ptr = mgard::recompose_udq_2D(1.0, nrow, ncol, data, data_len);
