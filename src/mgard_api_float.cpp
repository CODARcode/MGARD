...
unsigned char *mgard_compress(int itype_flag, float *v, int &out_size, int nrow,
                              int ncol, int nfib, float tol_in)
...
unsigned char *mgard_compress(int itype_flag, float *v, int &out_size, int nrow,
                              int ncol, int nfib, std::vector<float> &coords_x,
...
unsigned char *mgard_compress(int itype_flag, float *v, int &out_size, int nrow,
                              int ncol, int nfib, float tol_in, float s) {
...
// unsigned char *mgard_compress(int itype_flag,  float  *v, int &out_size, int
// nrow, int ncol, int nfib, float tol_in, float (*qoi) (int, int, int,
// std::vector<float>), float s)
...
unsigned char *mgard_compress(int itype_flag, float *v, int &out_size, int nrow,
  ...
  if (nrow > 1 && ncol > 1 && nfib > 1) {
    ...
    float xi_norm = mgard_gen::qoi_norm(nrow, ncol, nfib, coords_x, coords_y,
                                        coords_z, qoi, s);
    tol *= xi_norm;
    mgard_compressed_ptr =
        mgard::refactor_qz(nrow, ncol, nfib, v, out_size, tol, -s);
...
float *mgard_decompress(int itype_flag, float &quantizer, unsigned char *data,
  ...
  if (nrow > 1 && ncol > 1 && nfib > 1) {
    ...
    assert(ncol > 3);
    assert(nfib > 3);
    // passing a dummy float to be able to return a pointer while not confusing
    // the compiler-overloader
    float dummy_float;
    mgard_decompressed_ptr =
        mgard::recompose_udq(dummy_float, nrow, ncol, nfib, data, data_len);
    ...
    // the compiler-overloader
    mgard_decompressed_ptr =
        mgard::recompose_udq_2D(dummy_float, nrow, ncol, data, data_len);
    return mgard_decompressed_ptr;
  } else if (nrow > 1) {
    assert(nrow > 3);
    std::cerr << "MGARD: Not impemented!  Let us know if you need 1D "
...
float *mgard_decompress(int itype_flag, float &quantizer, unsigned char *data,
                        int data_len, int nrow, int ncol, int nfib, float s) {
...
float mgard_compress(int nrow, int ncol, int nfib,
