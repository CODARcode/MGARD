...
unsigned char *refactor_qz(int nrow, int ncol, int nfib, const double *u,
  ...
  double norm = mgard_gen::ml2_norm3(0, nrow, ncol, nfib, nrow, ncol, nfib, v,
...
unsigned char *refactor_qz_2D(int nrow, int ncol, const double *u, int &outsize,
  ...
  // } else {
    ...
    mgard_2d::mgard_gen::refactor_2D(nr, nc, nrow, ncol, l_target, v.data(),
                                     ...
                                     col_vec, csv_prefix);
    // print_matrix(nrow, ncol, v.data(), nrow);
...
void quantize_2D_interleave(const int nrow, const int ncol, double *v,
  ...
  for (int index = 0; index < ncol * nrow; ++index) {
    ...
    int quantum = std::copysign(0.5 + std::abs(v[index] / quantizer), v[index]);
...
void resample_1d_inv2(const double *inbuf, double *outbuf, const int ncol,
                      const int ncol_new) {
  double hx_o = 1.0 / double(ncol - 1);
  double hx = 1.0 / double(ncol_new - 1); // x-spacing
  double hx_ratio = (hx_o / hx);          // ratio of x-spacing resampled/orig
