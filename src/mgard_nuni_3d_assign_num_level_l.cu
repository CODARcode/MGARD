...
__global__ void 
_assign_num_level_l_cuda_cpt(int nf,         int nr,         int nc,
  ...
  for (int z = z0; z * fib_stride < nf; z += blockDim.z * gridDim.z) {
    for (int y = y0; y * row_stride < nr; y += blockDim.y * gridDim.y) {
      for (int x = x0; x * col_stride < nc; x += blockDim.x * gridDim.x) {
        ...
        dwork[get_idx(lddcv1, lddcv2, z, y, x)] = num;
...
mgard_cuda_ret 
assign_num_level_l_cuda_cpt(int nf,         int nr,         int nc,
                      int fib_stride, int row_stride, int col_stride,
                      double * dwork, int lddwork1,   int lddwork2,
                      double num) {
  ...
  _assign_num_level_l_cuda_cpt<<<blockPerGrid, threadsPerBlock>>>(nf,         nr,         nc,
