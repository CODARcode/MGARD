...
//! Function object used to find the local index a midpoint of an edge.
...
#define CREATE_EDGE(a, b)                                                      \
  ...
  ecode = mesh.impl.create_element(moab::MBEDGE, EDGE_CONNECTIVITY, 2, EDGE);  \
  MB_CHK_ERR(ecode);                                                           \
  assert(EDGE == ++most_recent_edge);
  ...
  for (const moab::EntityHandle element : elements) {
    ...
    // paragraph (as of this writing) of the MGARD unstructured paper.
