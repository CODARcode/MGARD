...
{ 
  ...
  if(n1 > 1 && n2 > 1 && n3 > 1) {
      ...
      mgard_compressed_ptr = mgard_cuda::refactor_qz_cuda(handle, data, out_size, tol);
      ...
      assert (n2 > 3);
      mgard_cuda_handle<T> handle(n1, n2, n3);
      mgard_compressed_ptr = mgard_cuda::refactor_qz_2D_cuda(handle, data, out_size, tol);
      return mgard_compressed_ptr;
    } else if (n1 > 1 ) {
      assert (n1 > 3);
...
template unsigned char *mgard_compress_cuda<double>(double * data, int &out_size, int n1, int n2, int n3, double tol);
template unsigned char *mgard_compress_cuda<float>(float * data, int &out_size, int n1, int n2, int n3, float tol);
...
template double *mgard_decompress_cuda<double>(unsigned char * data, int data_len, int n1, int n2, int n3);
template float *mgard_decompress_cuda<float>(unsigned char * data, int data_len, int n1, int n2, int n3);
...
{ 
  ...
  if(n1 > 1 && n2 > 1 && n3 > 1) {
      ...
      mgard_cuda_handle<T> handle(n1, n2, n3, coords_x.data(), coords_y.data(), coords_z.data());
      mgard_compressed_ptr = mgard_cuda::refactor_qz_cuda(handle, data, out_size, tol);
      return mgard_compressed_ptr;
...
template unsigned char *mgard_compress_cuda<double>(double * data, int &out_size, int n1, int n2, int n3, 
                                                    std::vector<double> &coords_x,
                                                    std::vector<double> &coords_y,
                                                    std::vector<double> &coords_z,
                                                    double tol);
template unsigned char *mgard_compress_cuda<float>(float * data, int &out_size, int n1, int n2, int n3, 
...
template unsigned char *mgard_compress_cuda<double>(mgard_cuda_handle<double> & handle, double *v, int &out_size, double tol_in);
template unsigned char *mgard_compress_cuda<float>(mgard_cuda_handle<float> & handle, float *v, int &out_size, float tol_in);
...
template double *mgard_decompress_cuda(mgard_cuda_handle<double> & handle, unsigned char *data, int data_len);
template float *mgard_decompress_cuda(mgard_cuda_handle<float> & handle, unsigned char *data, int data_len);
