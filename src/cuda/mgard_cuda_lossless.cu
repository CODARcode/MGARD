...
void cascaded_compress(mgard_cuda_handle<T, D> &handle, C * input_data, 
  ...
  nvcomp::CascadedCompressor<C> compressor( input_data, intput_count, n_rle, n_de, bitpack);
...
// template void cascaded_compress<double, int>(mgard_cuda_handle<double> &handle, int * input_data, 
//              size_t intput_count, void * &output_data, size_t &output_size, 
//              int n_rle, int n_de, bool bitpack, int queue_idx);
// template void cascaded_compress<float, int>(mgard_cuda_handle<float> &handle, int * input_data, 
//              size_t intput_count, void * &output_data, size_t &output_size, 
//              int n_rle, int n_de, bool bitpack, int queue_idx);
// template void cascaded_compress<double, uint32_t>(mgard_cuda_handle<double> &handle, uint32_t * input_data, 
//              size_t intput_count, void * &output_data, size_t &output_size, 
//              int n_rle, int n_de, bool bitpack, int queue_idx);
// template void cascaded_compress<float, uint32_t>(mgard_cuda_handle<float> &handle, uint32_t * input_data, 
//              size_t intput_count, void * &output_data, size_t &output_size, 
//              int n_rle, int n_de, bool bitpack, int queue_idx);
...
void cascaded_decompress(mgard_cuda_handle<T, D> &handle, void * input_data, size_t input_size, 
  ...
  nvcomp::Decompressor<C> decompressor(input_data, input_size, *(cudaStream_t *)handle.get(queue_idx));
  ...
  decompressor.decompress_async(temp_space, temp_size, output_data, output_count, 
                                *(cudaStream_t *)handle.get(queue_idx));
}

// template void cascaded_decompress<double, int>(mgard_cuda_handle<double> &handle, void * input_data, 
//                 size_t input_size, int * &output_data,  int queue_idx);
// template void cascaded_decompress<float, int>(mgard_cuda_handle<float> &handle, void * input_data, 
//              size_t input_size, int * &output_data, int queue_idx);
// template void cascaded_decompress<double, uint32_t>(mgard_cuda_handle<double> &handle, void * input_data, 
//                 size_t input_size, uint32_t * &output_data,  int queue_idx);
// template void cascaded_decompress<float, uint32_t>(mgard_cuda_handle<float> &handle, void * input_data, 
//              size_t input_size, uint32_t * &output_data, int queue_idx);
...
void lz4_compress(mgard_cuda_handle<T, D> &handle, C * input_data, 
             size_t input_count, void * &output_data, size_t &output_size, size_t chunk_size, int queue_idx) {
...
// template void lz4_compress<double, int>(mgard_cuda_handle<double> &handle, int * input_data, 
//              size_t intput_count, void * &output_data, size_t &output_size, size_t chunk_size, int queue_idx);
// template void lz4_compress<float, int>(mgard_cuda_handle<float> &handle, int * input_data, 
//              size_t intput_count, void * &output_data, size_t &output_size, size_t chunk_size, int queue_idx);
// template void lz4_compress<double, uint32_t>(mgard_cuda_handle<double> &handle, uint32_t * input_data, 
//              size_t intput_count, void * &output_data, size_t &output_size, size_t chunk_size, int queue_idx);
// template void lz4_compress<float, uint32_t>(mgard_cuda_handle<float> &handle, uint32_t * input_data, 
//              size_t intput_count, void * &output_data, size_t &output_size, size_t chunk_size, int queue_idx);
...
#define KERNELS(T, D, C) \
                ...
                mgard_cuda_handle<T, D> &handle, C * input_data,\
                size_t intput_count, void * &output_data, size_t &output_size,\
                int n_rle, int n_de, bool bitpack, int queue_idx);\
                ...
                C * &output_data, int queue_idx);\
                template void lz4_compress<T, D, C>(\
                mgard_cuda_handle<T, D> &handle,\
                ...
                mgard_cuda_handle<T, D> &handle,\
                void * input_data, size_t input_size,\
                C * &output_data, size_t &output_count,\
                int queue_idx);
...
void SeparateOutlierAndPrimary(mgard_cuda_handle<T, D> &handle, 
  ...
  // printf("copy primary\n");
  if (size > 0) { mgard_cuda::cudaMemcpyAsyncHelper(handle, dprimary + pp, dqv + p, size * sizeof(Q), mgard_cuda::D2D, queue_idx); }
  pp += size;
  p += size;
  ...
  for (int i = 0; i < outlier_count - 1; i++) {
    size = 1;
    // printf("copy outlier\n");
    mgard_cuda::cudaMemcpyAsyncHelper(handle, doutlier + op, dqv + p, size * sizeof(S), mgard_cuda::D2D, queue_idx);
    op += size;
    p += size;
...
void CombineOutlierAndPrimary(mgard_cuda_handle<T, D> &handle, 
  ...
  size_t size = outlier_idx[0] - 0;
  // printf("copy primary\n");
  if (size > 0) { mgard_cuda::cudaMemcpyAsyncHelper(handle, dqv + p, dprimary + pp, size * sizeof(Q), mgard_cuda::D2D, queue_idx); }
  pp += size;
  p += size;
  ...
  for (int i = 0; i < outlier_count - 1; i++) {
    size = 1;
    // printf("copy outlier\n");
    mgard_cuda::cudaMemcpyAsyncHelper(handle, dqv + p, doutlier + op, size * sizeof(S), mgard_cuda::D2D, queue_idx);
    op += size;
    p += size;
    size = outlier_idx[i + 1] - outlier_idx[i] - 1;
    // printf("copy primary %d %d %d\n", p, size, outlier_idx[outlier_idx.size() - 1]);
...
void huffman_compress(mgard_cuda_handle<T, D> &handle, S * input_data, size_t input_count, std::vector<size_t>& outlier_idx,
  ...
  HuffmanEncode<T, D, S, Q, H>(handle, input_data, input_count, outlier_idx, out_meta, out_meta_size, out_data, out_data_size, chunk_size, dict_size);
}
...
void huffman_decompress(mgard_cuda_handle<T, D> &handle, 
  ...
  HuffmanDecode<T, D, S, Q, H>(handle, output_data, output_count, in_meta, in_meta_size, in_data, in_data_size);
