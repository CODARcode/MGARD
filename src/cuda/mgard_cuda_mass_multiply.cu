...
template <typename T>
__global__ void
_mass_multiply_1(int nrow,       int ncol,
                 int nr,         int nc,
  ...
  for (int r = r0; r < nr; r += (blockDim.x * gridDim.x) * row_stride) {
    ...
    T h1, h2;
    temp1 = vec[dicol[0]];
    h1 = _get_dist(dcoords_x,  dicol[0], dicol[col_stride]); //dicol[col_stride] - dicol[0];
    ...
    vec[dicol[nc - 1]] = _get_dist(dcoords_x, dicol[nc - col_stride - 1], dicol[nc - 1]) * temp1 +
                        2 * _get_dist(dcoords_x, dicol[nc - col_stride - 1], dicol[nc - 1]) * vec[dicol[nc - 1]];
  }
}
...
mass_multiply_1(mgard_cuda_handle<T> & handle, 
  ...
  _mass_multiply_1<<<blockPerGrid, threadsPerBlock,
...
template void 
mass_multiply_1<double>(mgard_cuda_handle<double> & handle, 
                        int nrow,       int ncol,
                        int nr,         int nc,
                        int row_stride, int col_stride,
                        int * dirow,    int * dicol,
                        ...
                        int queue_idx);
template void 
mass_multiply_1<float>(mgard_cuda_handle<float> & handle,
                       int nrow,       int ncol,
                       int nr,         int nc,
                       int row_stride, int col_stride,
                       int * dirow,    int * dicol,
...
template <typename T>
__global__ void
_mass_multiply_2(int nrow,       int ncol,
                 int nr,         int nc,
                 int row_stride, int col_stride,
                 int * dirow,    int * dicol,
                 T * dcoords_y,
  ...
  for (int c = c0; c < nc; c += (blockDim.x * gridDim.x) * col_stride) {
    ...
    h1 = _get_dist(dcoords_y,  dirow[0], dirow[row_stride]); //dirow[row_stride] - dirow[0];
    ...
    vec[dirow[nr - 1] * lddv] = _get_dist(dcoords_y, dirow[nr - row_stride - 1], dirow[nr - 1]) * temp1 +
                        2 * _get_dist(dcoords_y, dirow[nr - row_stride - 1], dirow[nr - 1]) * vec[dirow[nr - 1] * lddv];
  }
}
...
mass_multiply_2(mgard_cuda_handle<T> & handle, 
  ...
  dim3 threadsPerBlock(tbx, 1);
  dim3 blockPerGrid(gridx, 1); 
  _mass_multiply_2<<<blockPerGrid, threadsPerBlock,
                     0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                               nrow,       ncol,
                                               nr,         nc,
                                               row_stride, col_stride,
...
template void 
mass_multiply_2<double>(mgard_cuda_handle<double> & handle, 
                        int nrow,       int ncol,
                        int nr,         int nc,
                        int row_stride, int col_stride,
                        int * dirow,    int * dicol,
                        ...
                        int queue_idx);
template void 
mass_multiply_2<float>(mgard_cuda_handle<float> & handle, 
                        int nrow,       int ncol,
                        int nr,         int nc,
                        int row_stride, int col_stride,
                        int * dirow,    int * dicol,
...
template <typename T> 
__global__ void
_mass_multiply_1_cpt(int nr,         int nc,
                     int row_stride, int col_stride,
...
mass_multiply_1_cpt(mgard_cuda_handle<T> & handle,
  ...
  _mass_multiply_1_cpt<<<blockPerGrid, threadsPerBlock, 
...
template void 
mass_multiply_1_cpt<double>(mgard_cuda_handle<double> & handle,
                    int nr,         int nc,
                    int row_stride, int col_stride,
                    double * ddist_x,
                    double * dv,    int lddv,
                    int queue_idx);
template void 
mass_multiply_1_cpt<float>(mgard_cuda_handle<float> & handle,
                    int nr,         int nc,
                    int row_stride, int col_stride,
                    float * ddist_x,
                    float * dv,    int lddv,
...
template <typename T>
__global__ void
_mass_multiply_2_cpt(int nr,         int nc,
                     int row_stride, int col_stride,
...
mass_multiply_2_cpt(mgard_cuda_handle<T> & handle, 
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _mass_multiply_2_cpt<<<blockPerGrid, threadsPerBlock, 
                         sm_size, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                                      nr,         nc,
                                                      row_stride, col_stride,
                                                      ddist_y,    ghost_row,
...
template void 
mass_multiply_2_cpt<double>(mgard_cuda_handle<double> & handle, 
                    int nr,         int nc,
                    int row_stride, int col_stride,
                    double * ddist_y,
                    double * dv,    int lddv,
                    int queue_idx);
template void 
mass_multiply_2_cpt<float>(mgard_cuda_handle<float> & handle, 
                    int nr,         int nc,
                    int row_stride, int col_stride,
                    float * ddist_y,
                    float * dv,    int lddv,
