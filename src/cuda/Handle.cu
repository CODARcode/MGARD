...
void Handle<D, T>::init(std::vector<SIZE> shape, std::vector<T *> coords,
  ...
  for (int i = 0; i < shape.size(); i++) {
    ...
    // coord_to_dist(dofs[i][0], this->coords[i].get_dv(), curr_ddist_l[0]);
  ...
  // for (int d = 0; d < D; d++) {
  //   for (int l = 0; l < l_target + 1; l++) {
  //     calc_volume(dofs[d][l], dist_array[d][l].get_dv(), volumes+ldvolumes*(d*(l_target + 1) + (l_target-l)));
  //   }
  // }
  ...
  for (DIM i = 0; i < D; i++) {
    ...
    for (SIZE l = 0; l < l_target+1; l++) {
      ...
      // calc_am_bm(dofs[i][l], dist_array[i][l].get_dv(), curr_am_l[l], curr_bm_l[l]);
