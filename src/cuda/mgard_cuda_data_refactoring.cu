...
void refactor_reo(mgard_cuda_handle<T, D> &handle, T *dv,
                  thrust::device_vector<int> ldvs, int l_target) {
  ...
  if (D <= 3) {
    ...
    for (int l = 0; l < l_target; ++l) {
      ...
      //           handle.ratio[0][l], handle.dw, handle.ldws[0],
      //           handle.ldws[1], dv, ldvs_h[0], ldvs_h[1],
      //           //null, ldvs[0], ldvs[1],
      ...
      if (D >= 2) {
        ...
        //                 handle.dw+get_idx(handle.ldws[0], handle.ldws[1], 0,
        //                 0, 0), handle.ldws[0], handle.ldws[1],
        //                 handle.dw+get_idx(handle.ldws[0], handle.ldws[1], 0,
        //                 handle.dofs[1][l+1], 0), handle.ldws[0],
        //                 handle.ldws[1], handle.dw+get_idx(handle.ldws[0],
        //                 handle.ldws[1], 0, 0, handle.dofs[0][l+1]),
        //                 handle.ldws[0], handle.ldws[1], 0,
        //                 handle.auto_tuning_mr1[handle.arch][handle.precision][range_lp1]);
        ...
        verify_matrix_cuda(
            ...
            handle.dw + get_idx(handle.ldws[0], handle.ldws[1], 0, 0,
                                handle.dofs[0][l + 1]),
            handle.ldws[0], handle.ldws[1], handle.ldws[0],
        ...
        // handle.dofs[0][l+1],
        //                 handle.dw+get_idx(handle.ldws[0], handle.ldws[1], 0,
        //                 0, handle.dofs[0][l+1]), handle.ldws[0],
        //                 handle.ldws[1] ,handle.ldws[0]);
        ...
        //             handle.dw+get_idx(handle.ldws[0], handle.ldws[1], 0, 0,
        //             handle.dofs[0][l+1]), handle.ldws[0], handle.ldws[1], 0,
        //             handle.auto_tuning_ts1[handle.arch][handle.precision][range_lp1]);
      ...
      if (D == 3) {
        ...
        //              handle.dw+get_idx(handle.ldws[0], handle.ldws[1], 0, 0,
        //              handle.dofs[0][l+1]), handle.ldws[0], handle.ldws[1],
        //              handle.dw+get_idx(handle.ldws[0], handle.ldws[1],
        //              handle.dofs[2][l+1], 0, handle.dofs[0][l+1]),
        //              handle.ldws[0], handle.ldws[1],
        //              handle.dw+get_idx(handle.ldws[0], handle.ldws[1], 0,
        //              handle.dofs[1][l+1], handle.dofs[0][l+1]),
        //              handle.ldws[0], handle.ldws[1], 0,
        //              handle.auto_tuning_mr1[handle.arch][handle.precision][range_lp1]);
  ...
  if (D >= 4) {
    ...
    for (int l = 0; l < l_target; ++l) {
      ...
      //   // print_matrix_cuda(handle.dofs[curr_dim_r][l],
      //   handle.dofs[curr_dim_c][l], handle.dofs[curr_dim_f][l],
      //   // handle.dw+i*handle.ldws[0]*handle.ldws[1]*handle.ldws[2],
      //   handle.ldws[0], handle.ldws[1], handle.ldws[0]);
      ...
      gpk_reo<T, D, 3, false, false, 1>(
          handle, handle.shapes_h[l], handle.shapes_d[l],
          handle.shapes_d[l + 1], thrust::raw_pointer_cast(handle.ldbs.data()),
          handle.ldws_d, unprocessed_dims.size(),
          thrust::raw_pointer_cast(unprocessed_dims.data()), curr_dim_r,
          curr_dim_c, curr_dim_f, handle.ratio[curr_dim_r][l],
          ...
          // null, lddv1, lddv2,
          handle.dw +
              get_idx(handle.ldws, gen_idx(handle.D_padded, curr_dim_r,
          ...
          handle.dw +
              get_idx(handle.ldws, gen_idx(handle.D_padded, curr_dim_r,
                                           curr_dim_c, curr_dim_f, 0,
                                           handle.dofs[curr_dim_c][l + 1], 0)),
          lddw1, lddw2,
          // null, lddv1, lddv2,
          handle.dw + get_idx(handle.ldws,
                              gen_idx(handle.D_padded, curr_dim_r, curr_dim_c,
                                      curr_dim_f,
                                      handle.dofs[curr_dim_r][l + 1], 0, 0)),
          lddw1, lddw2,
      ...
      //   print_matrix_cuda(handle.dofs[curr_dim_r][l],
      //   handle.dofs[curr_dim_c][l], handle.dofs[curr_dim_f][l],
      //                     handle.db+i*handle.ldbs[0]*handle.ldbs[1]*handle.ldbs[2],
      //                     handle.ldbs[0], handle.ldbs[1], handle.ldbs[0]);
      // }
      ...
      if (D % 2 == 0) {
        ...
        gpk_reo<T, D, 2, false, true, 2>(
            handle, handle.shapes_h[l], handle.shapes_d[l],
            handle.shapes_d[l + 1],
            thrust::raw_pointer_cast(handle.ldbs.data()), ldvs_d,
            unprocessed_dims.size(),
            thrust::raw_pointer_cast(unprocessed_dims.data()), curr_dim_r,
            curr_dim_c, curr_dim_f, handle.ratio[curr_dim_r][l],
            handle.ratio[curr_dim_c][l], handle.ratio[curr_dim_f][l], handle.db,
      ...
      //   print_matrix_cuda(handle.dofs[2][l], handle.dofs[1][l],
      //   handle.dofs[0][l+1],
      //                     dw_out+i*handle.ldws[0]*handle.ldws[1]*handle.ldws[2],
      //                     handle.ldws[0], handle.ldws[1], handle.ldws[0]);
      // }

      // printf("solve tridiag 1D\n");
      // ipk_1<T, D>(handle, shape, shape_c, handle.ldws, handle.ldws,
      ...
      curr_dim_f = 0, curr_dim_c = 1, curr_dim_r = 2;
      dw_in1 = dw_out;
      dw_in2 = dw_out + get_idx(handle.ldws,
                                gen_idx(D, curr_dim_r, curr_dim_c, curr_dim_f,
                                        0, handle.dofs[curr_dim_c][l + 1], 0));
      dw_out +=
          get_idx(handle.ldws, gen_idx(D, prev_dim_r, prev_dim_c, prev_dim_f, 0,
                                       0, handle.dofs[prev_dim_f][l + 1]));
      prev_dim_f = curr_dim_f;
      prev_dim_c = curr_dim_c;
      prev_dim_r = curr_dim_r;
      lddv1 = 1, lddv2 = 1, lddw1 = 1, lddw2 = 1;
      ...
      lpk_reo_2<T, D>(
          ...
          // handle.dw+get_idx(handle.ldws, gen_idx(D, curr_dim_r, curr_dim_c,
          // curr_dim_f, 0, 0, 0)),
          dw_in1, lddw1, lddw2,
          // handle.dw+get_idx(handle.ldws, gen_idx(D, curr_dim_r, curr_dim_c,
      ...
      ipk_3<T, D>(
          handle, shape, shape_c, handle.ldws, handle.ldws, processed_dims,
    ...
    for (int l = l_target - 1; l >= 0; l--) {
      ...
      if (D >= 1) {
        ...
        lpk_reo_1_3d(
            ...
            ldvs_h[0], ldvs_h[1],
            dv + get_idx(ldvs_h[0], ldvs_h[1], 0, 0, handle.dofs[0][l + 1]),
            ldvs_h[0], ldvs_h[1], handle.dw, handle.ldws[0], handle.ldws[1], 0,
            handle.auto_tuning_mr1[handle.arch][handle.precision][range_lp1]);
        ...
        ipk_1_3d(
            handle, handle.dofs[2][l], handle.dofs[1][l], handle.dofs[0][l + 1],
            handle.am[0][l + 1], handle.bm[0][l + 1], handle.dist[0][l + 1],
            handle.dw, handle.ldws[0], handle.ldws[1], 0,
            handle.auto_tuning_ts1[handle.arch][handle.precision][range_lp1]);
      ...
      if (D >= 2) {
        ...
        lpk_reo_2_3d(
            handle, handle.dofs[2][l], handle.dofs[1][l], handle.dofs[0][l + 1],
            handle.dofs[1][l + 1], handle.dist[1][l], handle.ratio[1][l],
            handle.dw + get_idx(handle.ldws[0], handle.ldws[1], 0, 0, 0),
            handle.ldws[0], handle.ldws[1],
            handle.dw + get_idx(handle.ldws[0], handle.ldws[1], 0,
                                handle.dofs[1][l + 1], 0),
            handle.ldws[0], handle.ldws[1],
      ...
      if (D == 3) {
        ...
        //                 0, handle.dofs[0][l+1]), handle.ldws[0],
        //                 handle.ldws[1], handle.dw+get_idx(handle.ldws[0],
        //                 handle.ldws[1], handle.dofs[2][l+1], 0,
        //                 handle.dofs[0][l+1]), handle.ldws[0], handle.ldws[1],
        //                 handle.dw+get_idx(handle.ldws[0], handle.ldws[1], 0,
        //                 handle.dofs[1][l+1], handle.dofs[0][l+1]),
        //                 handle.ldws[0], handle.ldws[1], 0,
      ...
      gpk_rev_3d(
          handle, handle.dofs[2][l], handle.dofs[1][l], handle.dofs[0][l],
          handle.ratio[2][l], handle.ratio[1][l], handle.ratio[0][l], handle.dw,
          handle.ldws[0], handle.ldws[1], dv, ldvs[0], ldvs[1],
  ...
  if (D >= 4) {
    for (int l = l_target - 1; l >= 0; l--) {
      ...
      gpk_rev<T, D, 3, true, false, 1>(
          ...
          curr_dim_c, curr_dim_f, handle.ratio[curr_dim_r][l],
          handle.ratio[curr_dim_c][l], handle.ratio[curr_dim_f][l], handle.dw,
          handle.ldws[0], handle.ldws[1], dv, lddv1, lddv2,
          dv + get_idx(ldvs, gen_idx(D, curr_dim_r, curr_dim_c, curr_dim_f, 0,
                                     0, handle.dofs[curr_dim_f][l + 1])),
          lddv1, lddv2,
          // null, ldvs[0], ldvs[1],
      ...
      if (D % 2 == 0) {
        ...
        gpk_rev<T, D, 2, false, true, 2>(
            ...
            // null, lddv1, lddv2,
            handle.db +
                get_idx(handle.ldbs, gen_idx(handle.D_padded, curr_dim_r,
                                             curr_dim_c, curr_dim_f, 0, 0,
                                             handle.dofs[curr_dim_f][l + 1])),
            lddb1, lddb2,
            // null, lddv1, lddv2,
...
#define KERNELS(T, D)                                                          \
  template void refactor_reo<T, D>(mgard_cuda_handle<T, D> & handle, T * dv,   \
                                   thrust::device_vector<int> ldvs,            \
                                   int l_target);                              \
  template void recompose_reo<T, D>(mgard_cuda_handle<T, D> & handle, T * dv,  \
                                    thrust::device_vector<int> ldvs,           \
                                    int l_target);
