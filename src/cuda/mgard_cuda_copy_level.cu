...
template <typename T>
__global__ void _copy_level(int nrow, int ncol, int nr, int nc, int row_stride,
  ...
  for (int y = y0; y < nr; y += blockDim.y * gridDim.y * row_stride) {
    for (int x = x0; x < nc; x += blockDim.x * gridDim.x * col_stride) {
      dwork[get_idx(lddwork, irow[y], icol[x])] =
          dv[get_idx(lddv, irow[y], icol[x])];
    }
  }
}

template <typename T>
void copy_level(mgard_cuda_handle<T> &handle, int nrow, int ncol, int nr,
                int nc, int row_stride, int col_stride, int *dirow, int *dicol,
                T *dv, int lddv, T *dwork, int lddwork, int queue_idx) {
  ...
  _copy_level<<<blockPerGrid, threadsPerBlock, 0,
...
template void copy_level<double>(mgard_cuda_handle<double> &handle, int nrow,
...
template void copy_level<float>(mgard_cuda_handle<float> &handle, int nrow,
                                int ncol, int nr, int nc, int row_stride,
                                int col_stride, int *dirow, int *dicol,
...
template <typename T>
__global__ void _copy_level_cpt(int nr, int nc, int row_stride, int col_stride,
...
template <typename T>
void copy_level_cpt(mgard_cuda_handle<T> &handle, int nr, int nc,
                    int row_stride, int col_stride, T *dv, int lddv, T *dwork,
                    int lddwork, int queue_idx) {
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _copy_level_cpt<<<blockPerGrid, threadsPerBlock, 0,
                    *(cudaStream_t *)handle.get(queue_idx)>>>(
...
template void copy_level_cpt<double>(mgard_cuda_handle<double> &handle, int nr,
                                     int nc, int row_stride, int col_stride,
                                     double *dv, int lddv, double *dwork,
                                     int lddwork, int queue_idx);
template void copy_level_cpt<float>(mgard_cuda_handle<float> &handle, int nr,
                                    int nc, int row_stride, int col_stride,
                                    float *dv, int lddv, float *dwork,
                                    int lddwork, int queue_idx);

template <typename T>
__global__ void _copy_level_cpt(int nr, int nc, int nf, int row_stride,
...
template <typename T>
void copy_level_cpt(mgard_cuda_handle<T> &handle, int nr, int nc, int nf,
