...
{
  __host__ __device__
  T operator()(T x, T y) { return max(abs(x),abs(y)); }
};
...
unsigned char *refactor_qz_cuda(mgard_cuda_handle<T, D> &handle, T *u,
  ...
  cudaMemcpyAsyncHelper(handle, ldvs_d, ldvs_h.data(), 
    handle.D_padded * sizeof(int), AUTO, 0);
  ...
  {
    t1 = high_resolution_clock::now();
    thrust::device_vector<T> v_vec(handle.dofs[0][0] * handle.dofs[1][0] * handle.linearized_depth);
    cudaMemcpyAsyncHelper(
        handle, thrust::raw_pointer_cast(v_vec.data()), u, 
        handle.dofs[0][0] * handle.dofs[1][0] * handle.linearized_depth * sizeof(T), AUTO, 0);
    handle.sync_all();
    t2 = high_resolution_clock::now();
    time_span = duration_cast<duration<double>>(t2 - t1);
    ...
    cudaMemcpy3DAsyncHelper(
        handle, dv, ldvs_h[0] * sizeof(T), handle.dofs[0][0] * sizeof(T), handle.dofs[1][0], 
        thrust::raw_pointer_cast(v_vec.data()),
        handle.dofs[0][0] * sizeof(T), handle.dofs[0][0] * sizeof(T), handle.dofs[1][0],
        handle.dofs[0][0] * sizeof(T), handle.dofs[1][0], handle.linearized_depth, AUTO, 0);
    handle.sync_all();
    t2 = high_resolution_clock::now();
    time_span = duration_cast<duration<double>>(t2 - t1);
    // printf("copy time: %.6f s \n", time_span.count());
  ...
  // Decomposition
  t1 = high_resolution_clock::now();
  refactor_reo<T, D>(handle, dv, ldvs_h, handle.l_target);
  handle.sync_all();
  t2 = high_resolution_clock::now();
  time_span = duration_cast<duration<double>>(t2 - t1);
  ...
  /**** refactoring ****/
  // outsize = (handle.linearized_depth * handle.dofs[1][0] * handle.dofs[0][0]) * sizeof(T);
  // unsigned char *buffer = (unsigned char *)malloc(outsize);
  // cudaMemcpy3DAsyncHelper(
  //       handle, 
  //       buffer, handle.dofs[0][0] * sizeof(T), handle.dofs[0][0] * sizeof(T), handle.dofs[1][0],
  //       dv, lddv1 * sizeof(T), handle.dofs[0][0] * sizeof(T), handle.dofs[1][0], 
  //       handle.dofs[0][0] * sizeof(T), handle.dofs[1][0], handle.linearized_depth, D2H, 0);

  // Quantization
  bool huffman = true;
  int dict_size = handle.huff_dict_size, block_size = handle.huff_block_size;
  size_t quantized_count = handle.dofs[0][0] * handle.dofs[1][0] * handle.linearized_depth;
  int *dqv;
  cudaMallocHelper((void **)&dqv,
  ...
  for (int i = 1; i < handle.D_padded; i++) { ldqvs[i] = handle.dofs[i][0]; }
  ...
  for (int d = 0; d < D; d++) {
    ...
    for (int l = 1; l < handle.l_target+2; l++) {
       hshapes[d * (handle.l_target+2)+l] = handle.dofs[d][handle.l_target + 1 - l];
    }
    // printf("hshapes[%d]: ", d);
    // for (int l = 0; l < handle.l_target+2; l++) { printf("%d ", hshapes[d * (handle.l_target+2)+l]); }
    // printf("\n");
  }
  int * dshapes;
  cudaMallocHelper((void **)&dshapes, D * (handle.l_target+2) * sizeof(int));
  cudaMemcpyAsyncHelper(handle, dshapes, hshapes, D * (handle.l_target+2) * sizeof(int), H2D, 0);


  size_t estimate_outlier_count = (double)handle.dofs[0][0] * handle.dofs[1][0] * handle.linearized_depth *1;
  ...
  cudaMallocHelper((void **)&outlier_idx_d, estimate_outlier_count * sizeof(unsigned int));
  size_t zero = 0, outlier_count, * outlier_idx_h;
  cudaMemcpyAsyncHelper(handle, outlier_count_d, &zero, sizeof(size_t), H2D, 0);

  quant_meta<T> m;
  m.norm = norm; m.s = s; m.tol = tol; m.dict_size = dict_size; m.enable_lz4 = handle.enable_lz4;
  m.l_target = handle.l_target;
  
  // cudaMemGetInfo(&free, &total); printf("Mem: %f/%f\n", (double)(total-free)/1e9, (double)total/1e9);

  levelwise_linear_quantize<T, D>(handle, dshapes, handle.l_target, m, 
                                  dv, ldvs_d, 
                                  dqv, thrust::raw_pointer_cast(ldqvs.data()), 
                                  huffman,
                                  handle.shapes_d[0],
                                  outlier_count_d, outlier_idx_d, outliers, 0);

  cudaMemcpyAsyncHelper(handle, &outlier_count, outlier_count_d, sizeof(size_t), D2H, 0);
  ...
  size_t hufmeta_size;
  size_t hufdata_size;
  huffman_compress<T, D, int, uint32_t, uint64_t>(handle, dqv, quantized_count, outlier_idx, 
                    hufmeta, hufmeta_size, hufdata, hufdata_size,
                    block_size, dict_size, 0);
  handle.sync_all();
  t2 = high_resolution_clock::now();
  time_span = duration_cast<duration<double>>(t2 - t1);
  ...
  printf("Overall compression time: %.6f s (%.6f GB/s)\n", time_span.count(), 
    (double)(handle.dofs[0][0] * handle.dofs[1][0] *handle.linearized_depth *sizeof(T))/time_span.count()/1e9);
  ...
  outsize += sizeof(size_t) + outlier_count * sizeof(size_t) + outlier_count * sizeof(int);
  outsize += sizeof(size_t) + hufmeta_size;
  outsize += sizeof(size_t) + hufdata_size;
  

  unsigned char * buffer;
  if (!GPU_ONLY) cudaMallocHostHelper((void**)&buffer, outsize);
  ...
  cudaMemcpyAsyncHelper(handle, buffer_p, outlier_count_d, sizeof(size_t), AUTO, 0);
  buffer_p = buffer_p + sizeof(size_t);
  cudaMemcpyAsyncHelper(handle, buffer_p, outlier_idx_d, outlier_count * sizeof(unsigned int), AUTO, 0);
  buffer_p = buffer_p + outlier_count * sizeof(unsigned int);
  cudaMemcpyAsyncHelper(handle, buffer_p, outliers, outlier_count * sizeof(int), AUTO, 0);
  buffer_p = buffer_p + outlier_count * sizeof(int);

  // memcpy(buffer_p, &hufmeta_size, sizeof(size_t));
  cudaMemcpyAsyncHelper(handle, buffer_p, &hufmeta_size, sizeof(size_t), AUTO, 0);
  ...
  cudaMemcpyAsyncHelper(handle, buffer_p, &hufdata_size, sizeof(size_t), AUTO, 0);
  buffer_p = buffer_p + sizeof(size_t);
  // cudaMemcpyAsyncHelper(handle, buffer_p, lz4_hufmeta, lz4_hufmeta_size, D2H, 0);
  // buffer_p = buffer_p + lz4_hufmeta_size;
...
T *recompose_udq_cuda(mgard_cuda_handle<T, D> &handle, unsigned char *data,
  ...
  cudaMallocHelper((void **)&outlier_idx_d, outlier_count * sizeof(unsigned int));
  cudaMemcpyAsyncHelper(handle, outlier_idx_d, data_p, outlier_count * sizeof(unsigned int), H2D, 0);
  data_p = data_p + outlier_count * sizeof(unsigned int);
  cudaMallocHelper((void **)&outliers, outlier_count * sizeof(int));
  cudaMemcpyAsyncHelper(handle, outliers, data_p, outlier_count * sizeof(int), H2D, 0);
  ...
  if (m.enable_lz4) {
    if (!handle.enable_lz4) printf("Warning: This data was compressed with LZ4, but handler is configed to disable LZ4!\n");
    t1 = high_resolution_clock::now();
  ...
  t1 = high_resolution_clock::now();
  huffman_decompress<T, D, int, uint32_t, uint64_t>(handle, (uint64_t*)hufmeta, hufmeta_size, hufdata, hufdata_size, dqv, outsize, 0);
  handle.sync_all();
  t2 = high_resolution_clock::now();
  time_span = duration_cast<duration<double>>(t2 - t1);
  printf("Parallel Huffman time: %.6f s\n", time_span.count());
  ...
  t1 = high_resolution_clock::now();
  levelwise_linear_dequantize<T, D>(handle, dshapes, handle.l_target,
                                    m,  
                                    dqv, thrust::raw_pointer_cast(ldqvs.data()), 
                                    dv, ldvs_d,
  ...
  //   dv, lddv1 * sizeof(T), handle.dofs[0][0] * sizeof(T), handle.dofs[1][0],
  //     data, handle.dofs[0][0] * sizeof(T), handle.dofs[0][0] * sizeof(T), handle.dofs[1][0],
  //     handle.dofs[0][0] * sizeof(T), handle.dofs[1][0], handle.linearized_depth, H2D, 0);
  ...
  t1 = high_resolution_clock::now();
  recompose_reo<T, D>(handle, dv, ldvs_h, m.l_target);
  handle.sync_all();
  t2 = high_resolution_clock::now();
  time_span = duration_cast<duration<double>>(t2 - t1);
  ...
  T *v;
  cudaMallocHostHelper((void **)&v, handle.dofs[0][0] * handle.dofs[1][0] * handle.linearized_depth * sizeof(T));
  // = (T *)malloc(handle.dofs[0][0] * handle.dofs[1][0] * handle.linearized_depth * sizeof(T));
  


  cudaMemcpy3DAsyncHelper(
      handle, v, handle.dofs[0][0] * sizeof(T), handle.dofs[0][0] * sizeof(T), handle.dofs[1][0],
      dv, lddv1 * sizeof(T), handle.dofs[0][0] * sizeof(T), handle.dofs[1][0],
      handle.dofs[0][0] * sizeof(T), handle.dofs[1][0], handle.linearized_depth, D2H, 0);
...
#define KERNELS(T, D) \
                ...
                T *u, size_t &outsize, T tol, T s);\
                template T *recompose_udq_cuda<T, D>(\
                mgard_cuda_handle<T, D> &handle,\
