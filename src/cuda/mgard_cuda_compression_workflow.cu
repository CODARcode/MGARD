...
unsigned char *refactor_qz_cuda(mgard_cuda_handle<T, D> &handle, T *u,
  ...
  T norm;
  {
    ...
    cudaMemcpyAsyncHelper(
        ...
        handle.dofs[0][0] * handle.dofs[1][0] * handle.linearized_depth * sizeof(T), H2D, 0);
    handle.sync_all();
    t2 = high_resolution_clock::now();
    time_span = duration_cast<duration<double>>(t2 - t1);
    ...
    cudaMemcpy3DAsyncHelper(
        handle, dv, lddv1 * sizeof(T), handle.dofs[0][0] * sizeof(T), handle.dofs[1][0], 
        thrust::raw_pointer_cast(v_vec.data()),
        handle.dofs[0][0] * sizeof(T), handle.dofs[0][0] * sizeof(T), handle.dofs[1][0],
        handle.dofs[0][0] * sizeof(T), handle.dofs[1][0], handle.linearized_depth, D2D, 0);
    handle.sync_all();
    t2 = high_resolution_clock::now();
    time_span = duration_cast<duration<double>>(t2 - t1);
    // printf("copy time: %.6f s \n", time_span.count());
  ...
  cudaMemcpyAsyncHelper(handle, buffer_p, outlier_count_d, sizeof(size_t), D2H, 0);
  buffer_p = buffer_p + sizeof(size_t);
  cudaMemcpyAsyncHelper(handle, buffer_p, outlier_idx_d, outlier_count * sizeof(unsigned int), D2H, 0);
  buffer_p = buffer_p + outlier_count * sizeof(unsigned int);
  cudaMemcpyAsyncHelper(handle, buffer_p, outliers, outlier_count * sizeof(int), D2H, 0);
...
T *recompose_udq_cuda(mgard_cuda_handle<T, D> &handle, unsigned char *data,
  ...
  T *v = (T *)malloc(handle.dofs[0][0] * handle.dofs[1][0] * handle.linearized_depth * sizeof(T));
