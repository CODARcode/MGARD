...
template <typename T>
__global__ void 
_assign_num_level(int nrow,           int ncol,
            int nr,             int nc,
...
template <typename T>
void 
assign_num_level(mgard_cuda_handle<T> & handle, 
           int nrow,           int ncol,
           int nr,             int nc,
           int row_stride,     int col_stride,
           int * dirow,        int * dicol,
  ...
  _assign_num_level<<<blockPerGrid, threadsPerBlock,
                      0, *(cudaStream_t *)handle.get(queue_idx)>>>(nrow,       ncol,
                                          nr,         nc,
...
template void 
assign_num_level<double>(mgard_cuda_handle<double> & handle, 
                   int nrow,           int ncol,
                   int nr,             int nc,
                   int row_stride,     int col_stride,
                   int * dirow,        int * dicol,
                   ...
                   int queue_idx);
template void 
assign_num_level<float>(mgard_cuda_handle<float> & handle, 
                   int nrow,           int ncol,
                   int nr,             int nc,
                   int row_stride,     int col_stride,
                   int * dirow,        int * dicol,
...
template <typename T>
__global__ void 
_assign_num_level_cpt(int nr,             int nc,
                int row_stride,     int col_stride,
                T num,
                T * dv,        int lddv) {
...
template <typename T>
void 
assign_num_level_cpt(mgard_cuda_handle<T> & handle, 
               int nr,             int nc,
               int row_stride,     int col_stride,
               T num,
               T * dv,        int lddv,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _assign_num_level_cpt<<<blockPerGrid, threadsPerBlock,
...
template void 
assign_num_level_cpt<double>(mgard_cuda_handle<double> & handle, 
                       int nr,             int nc,
                       int row_stride,     int col_stride,
                       double num,
                       double * dv,        int lddv, 
                       int queue_idx);
template void 
assign_num_level_cpt<float>(mgard_cuda_handle<float> & handle, 
                       int nr,             int nc,
                       int row_stride,     int col_stride,
                       float num,
                       float * dv,         int lddv, 
...
template <typename T>
__global__ void 
_assign_num_level(int nrow,       int ncol,       int nfib,
...
template <typename T>
__global__ void 
_assign_num_level_cpt(int nr,         int nc,         int nf,
                      int row_stride, int col_stride, int fib_stride, 
                      T num,
                      T * dwork, int lddwork1,   int lddwork2) {
