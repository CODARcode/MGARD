...
template <typename T>
void 
prolongate(mgard_cuda_handle<T> & handle,
      int nrow,           int ncol,
      int nr,             int nc,
      int row_stride,     int col_stride,
      int * dirow,        int * dicol,
  ...
  _prolongate<<<blockPerGrid, threadsPerBlock,
...
template void 
prolongate<double>(mgard_cuda_handle<double> & handle,
              int nrow,           int ncol,
              int nr,             int nc,
              int row_stride,     int col_stride,
              int * dirow,        int * dicol,
              ...
              int queue_idx);
template void 
prolongate<float>(mgard_cuda_handle<float> & handle,
             int nrow,           int ncol,
             int nr,             int nc,
             int row_stride,     int col_stride,
             int * dirow,        int * dicol,
...
template <typename T>
void
prolongate_cpt(mgard_cuda_handle<T> & handle,
               int nr,         int nc,
               int row_stride, int col_stride,
               T * ddist_y, T * ddist_x,
               T * dv,    int lddv,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _prolongate_cpt<<<blockPerGrid, threadsPerBlock, 
...
template void
prolongate_cpt<double>(mgard_cuda_handle<double> & handle,
                       int nr,         int nc,
                       int row_stride, int col_stride,
                       ...
                       int queue_idx);
template void
prolongate_cpt<float>(mgard_cuda_handle<float> & handle,
                       int nr,         int nc,
                       int row_stride, int col_stride,
...
template <typename T>
__global__ void 
_prolongate_last_1(const int nrow, const int ncol,
...
template <typename T>
void 
prolongate_last_1(mgard_cuda_handle<T> & handle, 
  ...
  _prolongate_last_1<<<blockPerGrid, threadsPerBlock, 
                       0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                                      nrow,    ncol,
                                                      nr,      nc,
                                                      dirow,   dicol_p,
...
template void 
prolongate_last_1<double>(mgard_cuda_handle<double> & handle, 
                  const int nrow, const int ncol,
                  const int nr,   const int nc,
                  int * dirow,    int * dicol_p,
                  ...
                  int queue_idx);
template void 
prolongate_last_1<float>(mgard_cuda_handle<float> & handle, 
                  const int nrow, const int ncol,
                  const int nr,   const int nc,
                  int * dirow,    int * dicol_p,
...
template <typename T>
__global__ void 
_prolongate_last_2(const int nrow, const int ncol,
                   const int nr,   const int nc,
                   int * irow_p,   int * icol,
                   T * dist_r,     T * dist_c,
                   T * v,         int ldv) {
...
template <typename T>
void 
prolongate_last_2(mgard_cuda_handle<T> & handle, 
                  const int nrow, const int ncol,
                  const int nr,   const int nc,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _prolongate_last_2<<<blockPerGrid, threadsPerBlock, 
                       0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                                      nrow,    ncol,
                                                      nr,      nc,
                                                      dirow_p, dicol,
...
template void 
prolongate_last_2<double>(mgard_cuda_handle<double> & handle, 
                  const int nrow, const int ncol,
                  const int nr,   const int nc,
                  int * dirow_p,  int * dicol,
                  double * ddist_r,    double * ddist_c,
                  ...
                  int queue_idx);
template void 
prolongate_last_2<float>(mgard_cuda_handle<float> & handle, 
                  const int nrow, const int ncol,
                  const int nr,   const int nc,
                  int * dirow_p,  int * dicol,
                  float * ddist_r,    float * ddist_c,
...
template <typename T>
__global__ void 
_prolongate_last_12(const int nrow, const int ncol,
                    const int nr,   const int nc,
...
template <typename T>
void 
prolongate_last_12(mgard_cuda_handle<T> & handle, 
                   const int nrow, const int ncol,
                   const int nr,    const int nc,
  ...
  _prolongate_last_12<<<blockPerGrid, threadsPerBlock,   
                        0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                                        nrow,    ncol,
                                                        nr,      nc,
                                                        dirow_p, dicol_p,
...
template void 
prolongate_last_12<double>(mgard_cuda_handle<double> & handle, 
                   const int nrow, const int ncol,
                   const int nr,    const int nc,
                   int * dirow_p,   int * dicol_p,
                   double * ddist_r,     double * ddist_c,
                   ...
                   int queue_idx);
template void 
prolongate_last_12<float>(mgard_cuda_handle<float> & handle, 
                   const int nrow, const int ncol,
                   const int nr,    const int nc,
                   int * dirow_p,   int * dicol_p,
                   float * ddist_r,     float * ddist_c,
...
template <typename T>
__global__ void 
_prolongate_cpt(int nr,           int nc,           int nf, 
...
prolongate_cpt(mgard_cuda_handle<T> & handle, 
  ...
  size_t sm_size = ((B_adjust+1) * (B_adjust+1) * (B_adjust+1) + 3 * B_adjust) * sizeof(T);
...
template <typename T>
__global__ void 
_prolongate_last_1(int nrow,        int ncol,         int nfib,
               int nr,          int nc,           int nf, 
...
template <typename T>
__global__ void 
_prolongate_last_2(int nrow,        int ncol,         int nfib,
               int nr,          int nc,           int nf, 
...
template <typename T>
__global__ void 
_prolongate_last_3(int nrow,        int ncol,         int nfib,
               int nr,          int nc,           int nf, 
...
template <typename T>
void 
prolongate_last_3(mgard_cuda_handle<T> & handle, 
              int nrow,        int ncol,         int nfib,
              int nr,          int nc,           int nf, 
  ...
  dim3 threadsPerBlock(tbx, tby, tbz);
  dim3 blockPerGrid(gridx, gridy, gridz);
  _prolongate_last_3<<<blockPerGrid, threadsPerBlock, 
                   0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                          nrow,       ncol,       nfib,
                                          nr,         nc,         nf,
                                          dirow_p,    dicol,      difib,
...
template void 
prolongate_last_3<double>(mgard_cuda_handle<double> & handle, 
              int nrow,        int ncol,         int nfib,
              int nr,          int nc,           int nf, 
              int * dirow_p,    int * dicol,      int * difib,
              double * ddist_r, double * ddist_c, double * ddist_f,
              ...
              int queue_idx);
template void 
prolongate_last_3<float>(mgard_cuda_handle<float> & handle, 
              int nrow,        int ncol,         int nfib,
              int nr,          int nc,           int nf, 
              int * dirow_p,    int * dicol,      int * difib,
              float * ddist_r, float * ddist_c, float * ddist_f,
...
template <typename T>
__global__ void 
_prolongate_last_12(int nrow,        int ncol,         int nfib,
                int nr,          int nc,           int nf, 
...
template <typename T>
__global__ void 
_prolongate_last_13(int nrow,        int ncol,         int nfib,
                int nr,          int nc,           int nf, 
...
template <typename T>
void 
prolongate_last_13(mgard_cuda_handle<T> & handle, 
               int nrow,        int ncol,         int nfib,
               int nr,          int nc,           int nf, 
  ...
  dim3 threadsPerBlock(tbx, tby, tbz);
  dim3 blockPerGrid(gridx, gridy, gridz);
  _prolongate_last_13<<<blockPerGrid, threadsPerBlock, 
                    0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                          nrow,       ncol,       nfib,
                                          nr,         nc,         nf,
...
template void 
prolongate_last_13<double>(mgard_cuda_handle<double> & handle, 
               int nrow,        int ncol,         int nfib,
               int nr,          int nc,           int nf, 
               int * dirow_p,   int * dicol,      int * difib_p,
               double * ddist_r, double * ddist_c, double * ddist_f,
               ...
               int queue_idx);
template void 
prolongate_last_13<float>(mgard_cuda_handle<float> & handle, 
               int nrow,        int ncol,         int nfib,
               int nr,          int nc,           int nf, 
               int * dirow_p,   int * dicol,      int * difib_p,
               float * ddist_r, float * ddist_c, float * ddist_f,
...
template <typename T>
__global__ void 
_prolongate_last_23(int nrow,        int ncol,         int nfib,
                int nr,          int nc,           int nf, 
...
template <typename T>
void 
prolongate_last_23(mgard_cuda_handle<T> & handle, 
               int nrow,        int ncol,         int nfib,
               int nr,          int nc,           int nf, 
  ...
  dim3 threadsPerBlock(tbx, tby, tbz);
  dim3 blockPerGrid(gridx, gridy, gridz);
  _prolongate_last_23<<<blockPerGrid, threadsPerBlock, 
                    0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                          nrow,       ncol,       nfib,
                                          nr,         nc,         nf,
...
template void 
prolongate_last_23<double>(mgard_cuda_handle<double> & handle, 
               int nrow,        int ncol,         int nfib,
               int nr,          int nc,           int nf, 
               int * dirow_p,   int * dicol_p,    int * difib,
               double * ddist_r, double * ddist_c, double * ddist_f,
               ...
               int queue_idx);
template void 
prolongate_last_23<float>(mgard_cuda_handle<float> & handle, 
               int nrow,        int ncol,         int nfib,
               int nr,          int nc,           int nf, 
               int * dirow_p,   int * dicol_p,    int * difib,
               float * ddist_r, float * ddist_c, float * ddist_f,
...
template <typename T>
__global__ void 
_prolongate_last_123(int nrow,        int ncol,         int nfib,
                 int nr,          int nc,           int nf, 
...
template <typename T>
void 
prolongate_last_123(mgard_cuda_handle<T> & handle, 
                int nrow,        int ncol,         int nfib,
                int nr,          int nc,           int nf, 
  ...
  dim3 threadsPerBlock(tbx, tby, tbz);
  dim3 blockPerGrid(gridx, gridy, gridz);
  _prolongate_last_123<<<blockPerGrid, threadsPerBlock, 
                     0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                          nrow,       ncol,       nfib,
                                          nr,         nc,         nf,
...
template void 
prolongate_last_123<double>(mgard_cuda_handle<double> & handle, 
                int nrow,        int ncol,         int nfib,
                int nr,          int nc,           int nf, 
                int * dirow_p,    int * dicol_p,     int * difib_p,
                double * ddist_r, double * ddist_c, double * ddist_f,
                ...
                int queue_idx);
template void 
prolongate_last_123<float>(mgard_cuda_handle<float> & handle, 
                int nrow,        int ncol,         int nfib,
                int nr,          int nc,           int nf, 
                int * dirow_p,    int * dicol_p,     int * difib_p,
                float * ddist_r, float * ddist_c, float * ddist_f,
