...
template <typename T>
__global__ void _solve_tridiag_1(int nrow, int ncol, int nr, int nc,
                                 int row_stride, int col_stride, int *dirow,
                                 int *dicol, T *dcoords_x, T *dcoeff,
                                 int lddcoeff, T *dv, int lddv) {
  ...
  for (int idx = idx0; idx < nr; idx += (blockDim.x * gridDim.x) * row_stride) {
    ...
    am = 2.0 * _get_dist(dcoords_x, dicol[0],
                         dicol[col_stride]); // dicol[col_stride] - dicol[0]
    bm = _get_dist(dcoords_x, dicol[0], dicol[col_stride]) /
         am; // dicol[col_stride] - dicol[0]
    int counter = 1;
    coeff[0] = am;
    ...
    for (int i = nc - 1 - col_stride; i >= 0; i -= col_stride) {
      h2 = _get_dist(dcoords_x, dicol[i], dicol[i + col_stride]);
      vec[dicol[i]] =
          (vec[dicol[i]] - h2 * vec[dicol[i + col_stride]]) / coeff[counter];
      --counter;
    }
  }
}
...
void solve_tridiag_1(mgard_cuda_handle<T> &handle, int nrow, int ncol, int nr,
  ...
  _solve_tridiag_1<<<blockPerGrid, threadsPerBlock, 0,
                     *(cudaStream_t *)handle.get(queue_idx)>>>(
      nrow, ncol, nr, nc, row_stride, col_stride, dirow, dicol, dcoords_x,
      dcoeff, lddcoeff, dv, lddv);
...
template void solve_tridiag_1<float>(mgard_cuda_handle<float> &handle, int nrow,
...
template <typename T>
__global__ void _solve_tridiag_2(int nrow, int ncol, int nr, int nc,
                                 int row_stride, int col_stride, int *dirow,
                                 int *dicol, T *dcoords_y, T *dcoeff,
                                 int lddcoeff, T *dv, int lddv) {
  ...
  for (int idx = idx0; idx < nc; idx += (blockDim.x * gridDim.x) * col_stride) {
    ...
    T *vec = dv + c;
    T *coeff = dcoeff + idx;
    am = 2.0 * _get_dist(dcoords_y, dirow[0],
                         dirow[row_stride]); // dirow[row_stride] - dirow[0]
    bm = _get_dist(dcoords_y, dirow[0], dirow[row_stride]) /
         am; // dirow[row_stride] - dirow[0]
    int counter = 1;
    coeff[0 * lddcoeff] = am;
...
void solve_tridiag_2(mgard_cuda_handle<T> &handle, int nrow, int ncol, int nr,
  ...
  dim3 threadsPerBlock(tb, 1);
  dim3 blockPerGrid(grid, 1);
  _solve_tridiag_2<<<blockPerGrid, threadsPerBlock, 0,
                     *(cudaStream_t *)handle.get(queue_idx)>>>(
...
template void solve_tridiag_2<double>(mgard_cuda_handle<double> &handle,
                                      ...
                                      double *dv, int lddv, int queue_idx);
template void solve_tridiag_2<float>(mgard_cuda_handle<float> &handle, int nrow,
                                     int ncol, int nr, int nc, int row_stride,
                                     int col_stride, int *dirow, int *dicol,
...
template void calc_am_bm<double>(mgard_cuda_handle<double> &handle, int n,
                                 double *ddist, double *am, double *bm,
                                 int queue_idx);
template void calc_am_bm<float>(mgard_cuda_handle<float> &handle, int n,
                                float *ddist, float *am, float *bm,
                                int queue_idx);

template <typename T>
__global__ void _solve_tridiag_forward_1_cpt(int nr, int nc, int row_stride,
                                             int col_stride, T *bm,
                                             int ghost_col, T *dv, int lddv) {
  ...
  for (int r = r0_stride; r < nr; r += gridDim.x * blockDim.x * row_stride) {
    ...
    while (rest_col > blockDim.x - real_ghost_col) {
      ...
      if (c_sm < real_main_col) {
        for (int i = 0; i < rest_row; i++) {
          vec_sm[i * ldsm + c_sm + real_ghost_col] =
              vec[i * row_stride * lddv + c_stride +
                  real_ghost_col * col_stride];
          // printf("c_sm = %d, r0_stride = %d, vec_sm_gh[%d/%d](%d) = %f\n",
          // c_sm, r0_stride, i,rest_row, i * row_stride * lddv + c_stride +
          // real_ghost_col * col_stride, vec_sm[i * ldsm + c_sm +
          // real_ghost_col]);
        }
    ...
    if (c_sm < rest_col) {
      for (int i = 0; i < rest_row; i++) {
        vec_sm[i * ldsm + c_sm + real_ghost_col] =
            vec[i * row_stride * lddv + c_stride + real_ghost_col * col_stride];
      }
      bm_sm[c_sm + real_ghost_col] = bm[c + real_ghost_col];
    }
    __syncthreads();
...
void solve_tridiag_forward_1_cpt(mgard_cuda_handle<T> &handle, int nr, int nc,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _solve_tridiag_forward_1_cpt<<<blockPerGrid, threadsPerBlock, sm_size,
                                 *(cudaStream_t *)handle.get(queue_idx)>>>(
      nr, nc, row_stride, col_stride, bm, ghost_col, dv, lddv);
  gpuErrchk(cudaGetLastError());
#ifdef MGARD_CUDA_DEBUG
  gpuErrchk(cudaDeviceSynchronize());
#endif
...
template void solve_tridiag_forward_1_cpt<double>(
    mgard_cuda_handle<double> &handle, int nr, int nc, int row_stride,
    int col_stride, double *bm, double *dv, int lddv, int queue_idx);
template void solve_tridiag_forward_1_cpt<float>(
    mgard_cuda_handle<float> &handle, int nr, int nc, int row_stride,
    int col_stride, float *bm, float *dv, int lddv, int queue_idx);

template <typename T>
__global__ void _solve_tridiag_backward_1_cpt(int nr, int nc, int row_stride,
                                              int col_stride, T *ddist_x, T *am,
                                              int ghost_col, T *dv, int lddv) {
  /* Global col idx */
  register int r0 = blockIdx.x * blockDim.x;
  register int r0_stride = r0 * row_stride;
  ...
  for (int r = r0_stride; r < nr; r += gridDim.x * blockDim.x * row_stride) {
    ...
    if (c_sm < real_ghost_col) {
      for (int i = 0; i < rest_row; i++) {
        vec_sm[i * ldsm + c_sm] =
            vec[i * row_stride * lddv + (nc - 1) - c_stride];
        // if (r_sm == 0) printf("r0_stride = %d, vec_sm[%d] = %f\n", r0_stride,
        // i, vec_sm[i * ldsm + c_sm]);
      }
    ...
    while (rest_col > blockDim.x - real_ghost_col) {
      ...
      if (c_sm < real_main_col) {
        for (int i = 0; i < rest_row; i++) {
          vec_sm[i * ldsm + c_sm + real_ghost_col] =
              vec[i * row_stride * lddv + (nc - 1) -
                  (c_stride + real_ghost_col * col_stride)];
          // printf("c_sm = %d, r0_stride = %d, vec_sm_gh[%d/%d](%d) = %f\n",
          // c_sm, r0_stride, i,rest_row, i * row_stride * lddv + c_stride +
          // real_ghost_col * col_stride, vec_sm[i * ldsm + c_sm +
          // real_ghost_col]);
        }
        am_sm[c_sm + real_ghost_col] =
            am[(total_col - 1) - (c + real_ghost_col)];
        dist_x_sm[c_sm + real_ghost_col] =
            ddist_x[(total_col - 1) - (c + real_ghost_col)];

        // printf("am_sm[%d+ real_ghost_col] = %f\n",c_sm, am_sm[c_sm+
        // real_ghost_col]); printf("ddist_x[%d] = %f\n",(total_col-1) - (c +
        // real_ghost_col), ddist_x[(total_col-1) - (c + real_ghost_col)]);
        // printf("dist_x_sm[%d] =\n", c_sm + real_ghost_col);
      }
      __syncthreads();
      ...
      if (r_sm < rest_row) {
        vec_sm[r_sm * ldsm + 0] =
            (vec_sm[r_sm * ldsm + 0] - dist_x_sm[0] * prev_vec_sm) / am_sm[0];
        for (int i = 1; i < blockDim.x; i++) {
          vec_sm[r_sm * ldsm + i] =
              (vec_sm[r_sm * ldsm + i] -
               dist_x_sm[i] * vec_sm[r_sm * ldsm + i - 1]) /
              am_sm[i];
        }
        /* Store last v */
        prev_vec_sm = vec_sm[r_sm * ldsm + blockDim.x - 1];
...
void solve_tridiag_backward_1_cpt(mgard_cuda_handle<T> &handle, int nr, int nc,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _solve_tridiag_backward_1_cpt<<<blockPerGrid, threadsPerBlock, sm_size,
                                  *(cudaStream_t *)handle.get(queue_idx)>>>(
      nr, nc, row_stride, col_stride, ddist_x, am, ghost_col, dv, lddv);
  gpuErrchk(cudaGetLastError());
#ifdef MGARD_CUDA_DEBUG
  gpuErrchk(cudaDeviceSynchronize());
#endif
...
template void
solve_tridiag_backward_1_cpt<double>(mgard_cuda_handle<double> &handle, int nr,
                                     int nc, int row_stride, int col_stride,
                                     double *ddist_x, double *am, double *dv,
                                     int lddv, int queue_idx);
template void
solve_tridiag_backward_1_cpt<float>(mgard_cuda_handle<float> &handle, int nr,
                                    int nc, int row_stride, int col_stride,
                                    float *ddist_x, float *am, float *dv,
                                    int lddv, int queue_idx);
...
void solve_tridiag_1_cpt(mgard_cuda_handle<T> &handle, int nr, int nc,
  ...
  calc_am_bm(handle, ceil((float)nc / col_stride), ddist_x, am, bm, queue_idx);

  solve_tridiag_forward_1_cpt(handle, nr, nc, row_stride, col_stride, bm, dv,
                              lddv, queue_idx);

  solve_tridiag_backward_1_cpt(handle, nr, nc, row_stride, col_stride, ddist_x,
                               am, dv, lddv, queue_idx);
}

template void solve_tridiag_1_cpt<double>(mgard_cuda_handle<double> &handle,
                                          int nr, int nc, int row_stride,
                                          int col_stride, double *ddist_x,
                                          double *am, double *bm, double *dv,
                                          int lddv, int queue_idx);
template void solve_tridiag_1_cpt<float>(mgard_cuda_handle<float> &handle,
                                         int nr, int nc, int row_stride,
                                         int col_stride, float *ddist_x,
                                         float *am, float *bm, float *dv,
                                         int lddv, int queue_idx);

template <typename T>
__global__ void _solve_tridiag_forward_2_cpt(int nr, int nc, int row_stride,
                                             int col_stride, T *bm,
                                             int ghost_row, T *dv, int lddv) {
    ...
    while (rest_row > blockDim.x - real_ghost_row) {
      ...
      if (c + threadIdx.x * col_stride < nc) {
        for (int i = 0; i < real_main_row; i++) {
          vec_sm[(i + real_ghost_row) * ldsm] =
              vec[(i + r + real_ghost_row) * row_stride * lddv];
          // if (c + threadIdx.x * col_stride == 0) printf("vec_sm[%d] = %f,
          // vec[%d] = %f\n",
          //                   i + real_ghost_row, vec_sm[(i + real_ghost_row) *
          //                   ldsm], (i + r + real_ghost_row) * row_stride *
          //                   lddv, vec[(i + r + real_ghost_row) * row_stride *
          //                   lddv]);
        }
      }
      ...
      if (c + threadIdx.x * col_stride < nc) {
        /* Copy next ghost to main */
        real_ghost_row =
            min(ghost_row, real_main_row - (blockDim.x - ghost_row));
        for (int i = 0; i < real_ghost_row; i++) {
...
void solve_tridiag_forward_2_cpt(mgard_cuda_handle<T> &handle, int nr, int nc,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _solve_tridiag_forward_2_cpt<<<blockPerGrid, threadsPerBlock, sm_size,
                                 *(cudaStream_t *)handle.get(queue_idx)>>>(
      nr, nc, row_stride, col_stride, bm, ghost_row, dv, lddv);
  gpuErrchk(cudaGetLastError());
#ifdef MGARD_CUDA_DEBUG
  gpuErrchk(cudaDeviceSynchronize());
#endif
...
template void solve_tridiag_forward_2_cpt<double>(
    mgard_cuda_handle<double> &handle, int nr, int nc, int row_stride,
    int col_stride, double *bm, double *dv, int lddv, int queue_idx);
template void solve_tridiag_forward_2_cpt<float>(
    mgard_cuda_handle<float> &handle, int nr, int nc, int row_stride,
    int col_stride, float *bm, float *dv, int lddv, int queue_idx);

template <typename T>
__global__ void _solve_tridiag_backward_2_cpt(int nr, int nc, int row_stride,
                                              int col_stride, T *ddist_x, T *am,
                                              int ghost_row, T *dv, int lddv) {
  /* Global idx */
  register int c0 = blockIdx.x * blockDim.x;
  ...
  for (int c = c0_stride; c < nc; c += gridDim.x * blockDim.x * col_stride) {
    ...
    while (rest_row > blockDim.x - real_ghost_row) {
      ...
      if (c + threadIdx.x * col_stride < nc) {
        for (int i = 0; i < real_main_row; i++) {
          vec_sm[(i + real_ghost_row) * ldsm] =
              vec[((nr - 1) - (i + r + real_ghost_row) * row_stride) * lddv];
          // printf("c_sm = %d, r0_stride = %d, vec_sm_gh[%d/%d](%d) = %f\n",
          // c_sm, r0_stride, i,rest_row, i * row_stride * lddv + c_stride +
          // real_ghost_col * col_stride, vec_sm[i * ldsm + c_sm +
          // real_ghost_col]);
      ...
      if (c_sm < real_main_row) {
        am_sm[c_sm + real_ghost_row] =
            am[(total_row - 1) - (r + c_sm + real_ghost_row)];
        dist_x_sm[c_sm + real_ghost_row] =
            ddist_x[(total_row - 1) - (r + c_sm + real_ghost_row)];
      ...
      if (c + threadIdx.x * col_stride < nc) {
        ...
        vec_sm[0 * ldsm] =
            (vec_sm[0 * ldsm] - dist_x_sm[0] * prev_vec_sm) / am_sm[0];
        // printf("after vec: %f, am: %f\n", vec_sm[0 * ldsm], am_sm[0]);
        for (int i = 1; i < blockDim.x; i++) {
          vec_sm[i * ldsm] =
              (vec_sm[i * ldsm] - dist_x_sm[i] * vec_sm[(i - 1) * ldsm]) /
              am_sm[i];
        }
        /* Store last v */
        prev_vec_sm = vec_sm[(blockDim.x - 1) * ldsm];
    ...
    if (c + threadIdx.x * col_stride < nc) {
      ...
      if (real_ghost_row + total_row == 1) {
        ...
        // if (c_sm==0) printf("compute: vec_sm[0 * ldsm] (%f) / am_sm[0] (%f) =
        // %f\n", vec_sm[0 * ldsm], am_sm[0], (vec_sm[0 * ldsm] - dist_x_sm[0] *
        // prev_vec_sm) / am_sm[0]);
        vec_sm[0 * ldsm] =
            (vec_sm[0 * ldsm] - dist_x_sm[0] * prev_vec_sm) / am_sm[0];
...
void solve_tridiag_backward_2_cpt(mgard_cuda_handle<T> &handle, int nr, int nc,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _solve_tridiag_backward_2_cpt<<<blockPerGrid, threadsPerBlock, sm_size,
                                  *(cudaStream_t *)handle.get(queue_idx)>>>(
      nr, nc, row_stride, col_stride, ddist_y, am, ghost_row, dv, lddv);
  gpuErrchk(cudaGetLastError());
#ifdef MGARD_CUDA_DEBUG
  gpuErrchk(cudaDeviceSynchronize());
#endif
...
template void
solve_tridiag_backward_2_cpt<double>(mgard_cuda_handle<double> &handle, int nr,
                                     int nc, int row_stride, int col_stride,
                                     double *ddist_y, double *am, double *dv,
                                     int lddv, int queue_idx);
template void
solve_tridiag_backward_2_cpt<float>(mgard_cuda_handle<float> &handle, int nr,
                                    int nc, int row_stride, int col_stride,
                                    float *ddist_y, float *am, float *dv,
                                    int lddv, int queue_idx);
...
void solve_tridiag_2_cpt(mgard_cuda_handle<T> &handle, int nr, int nc,
  ...
  calc_am_bm(handle, ceil((float)nr / row_stride), ddist_y, am, bm, queue_idx);

  solve_tridiag_forward_2_cpt(handle, nr, nc, row_stride, col_stride, bm, dv,
                              lddv, queue_idx);

  solve_tridiag_backward_2_cpt(handle, nr, nc, row_stride, col_stride, ddist_y,
                               am, dv, lddv, queue_idx);
}

template void solve_tridiag_2_cpt<double>(mgard_cuda_handle<double> &handle,
                                          int nr, int nc, int row_stride,
                                          int col_stride, double *ddist_y,
                                          double *am, double *bm, double *dv,
                                          int lddv, int queue_idx);
template void solve_tridiag_2_cpt<float>(mgard_cuda_handle<float> &handle,
                                         int nr, int nc, int row_stride,
