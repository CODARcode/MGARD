...
#include "cuda/DataRefactoring_AMR.h"
  ...
  if (D <= 3) {
    ...
    for (int l = 0; l < l_target; ++l) {
      ...
      if (D >= 1) {
        ...
        bool retrieve = false;
        int block_size = 4;
        int nblock = ceil((float)handle.dofs[0][l + 1]/block_size);

        printf("%d %d %f %d\n", handle.dofs[0][l + 1], block_size, (float)handle.dofs[0][l + 1]/block_size,
                              (int)ceil((float)handle.dofs[0][l + 1]/block_size));


        T * dfv;
        size_t dfv_pitch;
        cudaMalloc3DHelper((void **)&(dfv), &dfv_pitch,
                           handle.dofs[1][l] * sizeof(T), nblock,
                           handle.dofs[2][l]);
        int lddfv1 = dfv_pitch / sizeof(T);
        int lddfv2 = nblock;

        T * dbv;
        size_t dbv_pitch;
        cudaMalloc3DHelper((void **)&(dbv), &dbv_pitch,
                          handle.dofs[1][l] * sizeof(T), nblock,
                          handle.dofs[2][l]);
        int lddbv1 = dbv_pitch / sizeof(T);
        int lddbv2 = nblock;
        ...
        ipk_1_3d_amr(
            ...
            handle.dw, handle.ldws_h[0], handle.ldws_h[1], retrieve, block_size, 
            dfv, lddfv1, lddfv2, dbv, lddbv1, lddbv2,
            0,
            handle.auto_tuning_ts1[handle.arch][handle.precision][range_lp1]);
        ...
        print_matrix_cuda(handle.dofs[2][l], handle.dofs[1][l],
        handle.dofs[0][l+1],handle.dw, handle.ldws_h[0], handle.ldws_h[1] ,handle.ldws_h[0]);


        
        printf("dfv\n");
        print_matrix_cuda(handle.dofs[2][l], nblock,
        handle.dofs[1][l],dfv, lddfv1, lddfv2 ,lddfv1);

        printf("dbv\n");
        print_matrix_cuda(handle.dofs[2][l], nblock,
        handle.dofs[1][l],dbv, lddbv1, lddbv2 ,lddbv1);
...
#define KERNELS(D, T)                                                          \
  template void decompose_amr<D, T>(Handle<D, T> & handle, T * dv,                 \
                                std::vector<int> ldvs, int l_target);          \
  template void recompose_amr<D, T>(Handle<D, T> & handle, T * dv,                 \
                                std::vector<int> ldvs, int l_target);
