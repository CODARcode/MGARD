...
template <typename T>
__global__ void 
_linear_quantize(int nrow,    int ncol,
                 T quantizer, 
                 T * dv, int lddv, 
...
template <typename T>
void 
linear_quantize(mgard_cuda_handle<T> & handle, 
                int nrow,    int ncol, 
                T norm, T tol,
                T * dv,  int lddv, 
                int * dwork,  int lddwork, 
  ...
  cudaMemcpyAsyncHelper(handle, dwork, &quantizer, sizeof(double), H2D,
                        queue_idx);
  _linear_quantize<<<blockPerGrid, threadsPerBlock,
...
template void 
linear_quantize<double>(mgard_cuda_handle<double> & handle, 
            int nrow,    int ncol, 
            ...
            int queue_idx);
template void 
linear_quantize<float>(mgard_cuda_handle<float> & handle, 
            int nrow,    int ncol, 
...
template <typename T>
__global__ void 
_linear_dequantize(int nrow,    int ncol, 
               T quantizer,
               T * dv, int lddv, 
               int * dwork, int lddwork) {
  int x0 = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  for (int y = y0; y < nrow; y += blockDim.y * gridDim.y) {
    for (int x = x0; x < ncol; x += blockDim.x * gridDim.x) {
        dv[get_idx(lddv, y, x)] = quantizer * (T)(dwork[get_idx(lddwork, y, x) + size_ratio]);
    }
  }
}
...
template <typename T>
void  
linear_dequantize(mgard_cuda_handle<T> & handle,
              int nrow,   int ncol, 
              T * dv, int lddv, 
              int * dwork, int lddwork,
              int queue_idx) {
  ...
  cudaMemcpyAsyncHelper(handle, &quantizer, dwork, sizeof(double), D2H,
                        queue_idx);
  _linear_dequantize<<<blockPerGrid, threadsPerBlock,
                   0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                              nrow,  ncol, 
                                              quantizer,
                                              dv,    lddv, 
...
template void  
linear_dequantize<double>(mgard_cuda_handle<double> & handle,
              int nrow,   int ncol, 
              double * dv, int lddv, 
              int * dwork, int lddwork,
              int queue_idx);
template void  
linear_dequantize<float>(mgard_cuda_handle<float> & handle,
              int nrow,   int ncol, 
              float * dv, int lddv, 
              int * dwork, int lddwork,
              int queue_idx);
...
template <typename T>
__global__ void 
_linear_quantize(int nrow,    int ncol, int nfib,
          T quantizer, 
          T * v, int ldv1, int ldv2,
          int * work, int ldwork1, int ldwork2) {
  ...
  for (int z = z0; z < nrow; z += blockDim.z * gridDim.z) {
    for (int y = y0; y < ncol; y += blockDim.y * gridDim.y) {
      for (int x = x0; x < nfib; x += blockDim.x * gridDim.x) {
        ...
        work[get_idx(ldwork1, ldwork2, z, y, x) + size_ratio] = copysign(0.5 + fabs(t / quantizer), t);
      }
    }
  }
}
...
template <typename T>
__global__ void 
_linear_dequantize(int nrow,    int ncol, int nfib,
               T quantizer,
               T * v, int ldv1, int ldv2, 
               int * work, int ldwork1, int ldwork2) {
  int x0 = blockIdx.x * blockDim.x + threadIdx.x;
  ...
  for (int z = z0; z < nrow; z += blockDim.z * gridDim.z) {
    for (int y = y0; y < ncol; y += blockDim.y * gridDim.y) {
      for (int x = x0; x < nfib; x += blockDim.x * gridDim.x) {
        v[get_idx(ldv1, ldv2, z, y, x)] = quantizer * (T)(work[get_idx(ldwork1, ldwork2, z, y, x) + size_ratio]);
      }
    }
  }
}
