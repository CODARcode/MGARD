...
refactor_3D_cuda_cpt(mgard_cuda_handle<T> & handle, T * dv, int lddv1, int lddv2) {
  ...
  for (int l = 0; l < handle.l_target; ++l) {
    ...
    pi_Ql_cpt(handle,
              handle.nr_l[l], handle.nc_l[l], handle.nf_l[l],
              1, 1, 1,
    ...
    for (int f = 0; f < handle.nf; f += stride) {
      ...
      pow2p1_to_cpt(handle,
                    ...
                    stride * handle.lddwork1, stride * handle.lddwork1,
                    slice, ldslice,
...
template void refactor_3D_cuda_cpt<double>(mgard_cuda_handle<double> & handle, double * dv, int lddv1, int lddv2);
template void refactor_3D_cuda_cpt<float>(mgard_cuda_handle<float> & handle, float * dv, int lddv1, int lddv2);
}