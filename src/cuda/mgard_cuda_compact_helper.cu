...
template <typename T>
void 
org_to_pow2p1(mgard_cuda_handle<T> & handle,
...
template void 
org_to_pow2p1<double>(mgard_cuda_handle<double> & handle,
              int nrow,     int ncol,    int nfib,
              int nr,      int nc,       int nf,
              int * dirow,  int * dicol, int * difib, 
              ...
              int queue_idx);
template void 
org_to_pow2p1<float>(mgard_cuda_handle<float> & handle,
              int nrow,     int ncol,    int nfib,
              int nr,      int nc,       int nf,
              int * dirow,  int * dicol, int * difib, 
...
template <typename T>
void 
pow2p1_to_org(mgard_cuda_handle<T> & handle, 
              int nrow,     int ncol,    int nfib,   
              int nr,       int nc,      int nf,  
              int * dirow,  int * dicol, int * difib, 
              T * dcv, int lddcv1,  int lddcv2,
...
template void 
pow2p1_to_org<double>(mgard_cuda_handle<double> & handle, 
              int nrow,     int ncol,    int nfib,   
              int nr,       int nc,      int nf,  
              int * dirow,  int * dicol, int * difib, 
              double * dcv, int lddcv1,  int lddcv2,
              ...
              int queue_idx);
template void 
pow2p1_to_org<float>(mgard_cuda_handle<float> & handle, 
              int nrow,     int ncol,    int nfib,   
              int nr,       int nc,      int nf,  
              int * dirow,  int * dicol, int * difib, 
              float * dcv, int lddcv1,  int lddcv2,
...
template <typename T>
void 
pow2p1_to_cpt(mgard_cuda_handle<T> & handle, 
              int nrow,       int ncol,       int nfib, 
              int row_stride, int col_stride, int fib_stride, 
              T * dv,    int lddv1,      int lddv2,
              T * dcv,   int lddcv1,     int lddcv2,
...
template void 
pow2p1_to_cpt<double>(mgard_cuda_handle<double> & handle, 
              int nrow,       int ncol,       int nfib, 
              int row_stride, int col_stride, int fib_stride, 
              double * dv,    int lddv1,      int lddv2,
              double * dcv,   int lddcv1,     int lddcv2,
              int queue_idx);
template void 
pow2p1_to_cpt<float>(mgard_cuda_handle<float> & handle, 
              int nrow,       int ncol,       int nfib, 
              int row_stride, int col_stride, int fib_stride, 
              float * dv,    int lddv1,      int lddv2,
              float * dcv,   int lddcv1,     int lddcv2,
...
template <typename T>
__global__ void 
_pow2p1_to_cpt_num_assign(int nrow,           int ncol, 
                          int row_stride,      int col_stride,
                          T val,
                          T * dv,         int lddv, 
                          T * dcv,        int lddcv) {
  ...
  for (int y = y0; y * row_stride < nrow; y += blockDim.y * gridDim.y) {
    for (int x = x0; x * col_stride < ncol; x += blockDim.x * gridDim.x) {
      int x_strided = x * col_stride;
      int y_strided = y * row_stride;
      if (y % 2 == 0 && x % 2 == 0) dcv[get_idx(lddcv, y, x)] = val;
      else dcv[get_idx(lddcv, y, x)] = dv[get_idx(lddv, y_strided, x_strided)];
    }
  }
}
...
template <typename T>
void 
pow2p1_to_cpt_num_assign(mgard_cuda_handle<T> & handle, 
                        int nrow,      int ncol, 
                        int row_stride, int col_stride,
                        T val,
                        T * dv,    int lddv, 
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _pow2p1_to_cpt_num_assign<<<blockPerGrid, threadsPerBlock,
                  0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                              nrow,      ncol,
                              row_stride, col_stride,
                              val,
...
template void 
pow2p1_to_cpt_num_assign<double>(mgard_cuda_handle<double> & handle, 
                        int nrow,      int ncol, 
                        int row_stride, int col_stride,
                        double val,
                        double * dv,    int lddv, 
                        ...
                        int queue_idx);
template void 
pow2p1_to_cpt_num_assign<float>(mgard_cuda_handle<float> & handle, 
                        int nrow,      int ncol, 
                        int row_stride, int col_stride,
                        float val,
                        float * dv,    int lddv, 
...
template <typename T>
void
cpt_to_pow2p1(mgard_cuda_handle<T> & handle, 
              int nrow,      int ncol,       int nfib, 
              int row_stride, int col_stride, int fib_stride, 
              T * dcv,   int lddcv1,     int lddcv2,
              T * dv,    int lddv1,      int lddv2, 
...
template void
cpt_to_pow2p1<double>(mgard_cuda_handle<double> & handle, 
              int nrow,      int ncol,       int nfib, 
              int row_stride, int col_stride, int fib_stride, 
              double * dcv,   int lddcv1,     int lddcv2,
              double * dv,    int lddv1,      int lddv2, 
              int queue_idx);
template void
cpt_to_pow2p1<float>(mgard_cuda_handle<float> & handle, 
              int nrow,      int ncol,       int nfib, 
              int row_stride, int col_stride, int fib_stride, 
              float * dcv,   int lddcv1,     int lddcv2,
              float * dv,    int lddv1,      int lddv2, 
...
template <typename T>
__global__ void 
_cpt_to_pow2p1_add(int nrow,     int ncol,
  ...
  for (int y = y0; y * row_stride2 < nrow; y += blockDim.y * gridDim.y) {
    for (int x = x0; x * col_stride2 < ncol; x += blockDim.x * gridDim.x) {
      ...
      dv[get_idx(lddv, y_strided2, x_strided2)] += dcv[get_idx(lddcv, y_strided1, x_strided1)];
    }
  }
}
...
template <typename T>
void
cpt_to_pow2p1_add(mgard_cuda_handle<T> & handle, 
                  int nrow, int ncol, 
                  int row_stride1, int col_stride1,
                  int row_stride2, int col_stride2, 
                  T * dcv, int lddcv,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _cpt_to_pow2p1_add<<<blockPerGrid, threadsPerBlock,
                   0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                nrow, ncol,
                                row_stride1, col_stride1, 
...
template void
cpt_to_pow2p1_add<double>(mgard_cuda_handle<double> & handle, 
                  int nrow, int ncol, 
                  int row_stride1, int col_stride1,
                  int row_stride2, int col_stride2, 
                  double * dcv, int lddcv,
                  ...
                  int queue_idx);
template void
cpt_to_pow2p1_add<float>(mgard_cuda_handle<float> & handle, 
                  int nrow, int ncol, 
                  int row_stride1, int col_stride1,
                  int row_stride2, int col_stride2, 
                  float * dcv, int lddcv,
...
template <typename T>
__global__ void 
_cpt_to_pow2p1_subtract(int nrow,     int ncol,
                   int row_stride1, int col_stride1,
                   int row_stride2, int col_stride2,
                   T * dcv,   int lddcv,
                   T * dv,    int lddv) {
...
template <typename T>
void
cpt_to_pow2p1_subtract(mgard_cuda_handle<T> & handle, 
                      int nrow, int ncol, 
                      int row_stride1, int col_stride1,
                      int row_stride2, int col_stride2, 
                      T * dcv, int lddcv,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _cpt_to_pow2p1_subtract<<<blockPerGrid, threadsPerBlock,
                   0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                nrow, ncol,
                                row_stride1, col_stride1, 
                                row_stride2, col_stride2,
...
template void
cpt_to_pow2p1_subtract<double>(mgard_cuda_handle<double> & handle, 
                      int nrow, int ncol, 
                      int row_stride1, int col_stride1,
                      int row_stride2, int col_stride2, 
                      double * dcv, int lddcv,
                      ...
                      int queue_idx);
template void
cpt_to_pow2p1_subtract<float>(mgard_cuda_handle<float> & handle, 
                      int nrow, int ncol, 
                      int row_stride1, int col_stride1,
                      int row_stride2, int col_stride2, 
                      float * dcv, int lddcv,
...
template <typename T>
void
calc_cpt_dist(mgard_cuda_handle<T> & handle, 
              int nrow, int row_stride, 
              T * dcoord, T * ddist,
              int queue_idx) {
...
template void
calc_cpt_dist<double>(mgard_cuda_handle<double> & handle, 
              int nrow, int row_stride, 
              double * dcoord, double * ddist,
              int queue_idx);
template void
calc_cpt_dist<float>(mgard_cuda_handle<float> & handle, 
              int nrow, int row_stride, 
