...
template <typename T>
__global__ void _add_level(int nrow, int ncol, int nr, int nc, int row_stride,
                           int col_stride, int *irow, int *icol, T *dv,
                           int lddv, T *dwork, int lddwork) {
...
template <typename T>
void add_level(mgard_cuda_handle<T> &handle, int nrow, int ncol, int nr, int nc,
               int row_stride, int col_stride, int *dirow, int *dicol, T *dv,
               int lddv, T *dwork, int lddwork, int queue_idx) {
  ...
  _add_level<<<blockPerGrid, threadsPerBlock, 0,
...
template void add_level<double>(mgard_cuda_handle<double> &handle, int nrow,
                                int ncol, int nr, int nc, int row_stride,
                                int col_stride, int *dirow, int *dicol,
                                double *dv, int lddv, double *dwork,
                                int lddwork, int queue_idx);
template void add_level<float>(mgard_cuda_handle<float> &handle, int nrow,
                               int ncol, int nr, int nc, int row_stride,
                               int col_stride, int *dirow, int *dicol,
                               float *dv, int lddv, float *dwork, int lddwork,
                               int queue_idx);

template <typename T>
__global__ void _add_level_cpt(int nr, int nc, int row_stride, int col_stride,
                               T *dv, int lddv, T *dwork, int lddwork) {
  int idx_x = (blockIdx.x * blockDim.x + threadIdx.x) * col_stride;
  int idx_y = (blockIdx.y * blockDim.y + threadIdx.y) * row_stride;
  for (int y = idx_y; y < nr; y += blockDim.y * gridDim.y * row_stride) {
    for (int x = idx_x; x < nc; x += blockDim.x * gridDim.x * col_stride) {
...
template <typename T>
void add_level_cpt(mgard_cuda_handle<T> &handle, int nr, int nc, int row_stride,
                   int col_stride, T *dv, int lddv, T *dwork, int lddwork,
                   int queue_idx) {
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _add_level_cpt<<<blockPerGrid, threadsPerBlock, 0,
                   *(cudaStream_t *)handle.get(queue_idx)>>>(
      nr, nc, row_stride, col_stride, dv, lddv, dwork, lddwork);
  gpuErrchk(cudaGetLastError());
#ifdef MGARD_CUDA_DEBUG
  gpuErrchk(cudaDeviceSynchronize());
#endif
...
template void add_level_cpt<double>(mgard_cuda_handle<double> &handle, int nr,
                                    int nc, int row_stride, int col_stride,
                                    double *dv, int lddv, double *dwork,
                                    int lddwork, int queue_idx);
template void add_level_cpt<float>(mgard_cuda_handle<float> &handle, int nr,
                                   int nc, int row_stride, int col_stride,
                                   float *dv, int lddv, float *dwork,
                                   int lddwork, int queue_idx);

template <typename T>
__global__ void _add_level(int nrow, int ncol, int nfib, int nr, int nc, int nf,
                           int row_stride, int col_stride, int fib_stride,
                           int *irow, int *icol, int *ifib, T *dv, int lddv1,
                           int lddv2, T *dwork, int lddwork1, int lddwork2) {
  ...
  for (int z = z0; z * row_stride < nr;
       z += blockDim.z * gridDim.z * row_stride) {
    for (int y = y0; y * col_stride < nc;
         y += blockDim.y * gridDim.y * col_stride) {
      for (int x = x0; x * fib_stride < nf;
           x += blockDim.x * gridDim.x * fib_stride) {
...
template <typename T>
void add_level(mgard_cuda_handle<T> &handle, int nrow, int ncol, int nfib,
...
template void add_level<double>(mgard_cuda_handle<double> &handle, int nrow,
                                ...
                                int row_stride, int col_stride, int fib_stride,
                                int *dirow, int *dicol, int *difib, double *dv,
                                int lddv1, int lddv2, double *dwork,
                                int lddwork1, int lddwork2, int queue_idx);
template void add_level<float>(mgard_cuda_handle<float> &handle, int nrow,
                               int ncol, int nfib, int nr, int nc, int nf,
                               int row_stride, int col_stride, int fib_stride,
                               int *dirow, int *dicol, int *difib, float *dv,
                               int lddv1, int lddv2, float *dwork, int lddwork1,
                               int lddwork2, int queue_idx);

template <typename T>
__global__ void _add_level_cpt(int nr, int nc, int nf, int row_stride,
                               int col_stride, int fib_stride, T *dv, int lddv1,
                               int lddv2, T *dwork, int lddwork1,
                               int lddwork2) {
...
template <typename T>
void add_level_cpt(mgard_cuda_handle<T> &handle, int nr, int nc, int nf,
...
template void add_level_cpt<float>(mgard_cuda_handle<float> &handle, int nr,
                                   ...
                                   int lddv1, int lddv2, float *dwork,
                                   int lddwork1, int lddwork2, int queue_idx);
} // namespace mgard_cuda