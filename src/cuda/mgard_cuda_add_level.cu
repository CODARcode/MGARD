...
template <typename T>
void
add_level(mgard_cuda_handle<T> & handle,
          int nrow,       int ncol, 
          int nr,         int nc,
          int row_stride, int col_stride,
          int * dirow,    int * dicol,
  ...
  _add_level<<<blockPerGrid, threadsPerBlock, 
...
template void
add_level<double>(mgard_cuda_handle<double> & handle, 
                  int nrow,       int ncol, 
                  int nr,         int nc,
                  int row_stride, int col_stride,
                  int * dirow,    int * dicol,
                  ...
                  int queue_idx);
template void
add_level<float>(mgard_cuda_handle<float> & handle, 
                  int nrow,       int ncol, 
                  int nr,         int nc,
                  int row_stride, int col_stride,
                  int * dirow,    int * dicol,
...
template <typename T>
void
add_level_cpt(mgard_cuda_handle<T> & handle,
              int nr,         int nc,
              int row_stride, int col_stride,
              T * dv,    int lddv, 
              T * dwork, int lddwork,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _add_level_cpt<<<blockPerGrid, threadsPerBlock,
                   0, *(cudaStream_t *)handle.get(queue_idx)>>>(nr,         nc,
                                             row_stride, col_stride, 
                                             dv,         lddv,
                                             dwork,      lddwork);
  gpuErrchk(cudaGetLastError()); 
...
template void
add_level_cpt<double>(mgard_cuda_handle<double> & handle, 
                      int nr,         int nc,
                      int row_stride, int col_stride,
                      double * dv,    int lddv, 
                      double * dwork, int lddwork,
                      int queue_idx);
template void
add_level_cpt<float>(mgard_cuda_handle<float> & handle, 
                      int nr,         int nc,
                      int row_stride, int col_stride,
                      float * dv,     int lddv, 
                      float * dwork,  int lddwork,
...
template <typename T>
__global__ void 
_add_level(int nrow,       int ncol, int nfib,
...
template <typename T>
__global__ void  
_add_level_cpt(int nr,         int nc,         int nf,
               int row_stride, int col_stride, int fib_stride,
               T * dv,    int lddv1,      int lddv2,
               T * dwork, int lddwork1,   int lddwork2) {
