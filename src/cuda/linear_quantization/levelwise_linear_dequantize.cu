...
#define KERNELS(T, D) \
        ...
        mgard_cuda_handle<T, D> &handle, int * shapes, int l_target,\
        quant_meta<T> m,\
        int *dv, int * ldvs, T *dwork, int * ldws,\
        size_t outlier_count, unsigned int * outlier_idx, int * outliers, int queue_idx);
