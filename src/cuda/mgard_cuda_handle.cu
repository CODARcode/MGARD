...
template <typename T, int D>
void mgard_cuda_handle<T, D>::create_queues() {
  num_of_queues = 16;
...
template <typename T, int D>
void mgard_cuda_handle<T, D>::destory_queues() {
...
template <typename T, int D>
std::vector<T*> mgard_cuda_handle<T, D>::create_uniform_coords(std::vector<size_t> shape) {
