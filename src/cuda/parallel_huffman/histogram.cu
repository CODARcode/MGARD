...
__global__ void naiveHistogram(int input_data[], int output[], int N,
...
__global__ void p2013Histogram(T *input_data, Q *output, size_t N, int bins,
                               int R) {
  ...
  const unsigned int begin =
      (N / warps_block) * warpid + WARP_SIZE * blockIdx.x + lane;
  ...
  for (unsigned int pos = threadIdx.x; pos < (bins + 1) * R; pos += blockDim.x)
    Hs[pos] = 0;
...
template __global__ void p2013Histogram<uint8__t, unsigned int>(
    uint8__t *input_data, unsigned int *output, size_t N, int bins, int R);
template __global__ void p2013Histogram<uint16_t, unsigned int>(
    uint16_t *input_data, unsigned int *output, size_t N, int bins, int R);
template __global__ void p2013Histogram<uint32_t, unsigned int>(
    uint32_t *input_data, unsigned int *output, size_t N, int bins, int R);
