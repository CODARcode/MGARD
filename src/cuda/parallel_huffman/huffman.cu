...
__global__ void prototype::GPU_Histogram(T *input_data, Q *output, size_t N,
...
template <typename T, typename Q>
__global__ void prototype::EncodeFixedLen(T *data, Q *hcoded, size_t data_len,
                                          Q *codebook) {
...
__host__ __device__ node_list new_node(HuffmanTree *huffmanTree, size_t freq,
                                       uint32_t c, node_list a, node_list b) {
...
__host__ __device__ node_list qremove(HuffmanTree *ht) {
  ...
  while ((l = (i << 1)) < ht->qend) { // l=(i*2)
    if (l + 1 < ht->qend && ht->qq[l + 1]->freq < ht->qq[l]->freq)
      l++;
    ht->qq[i] = ht->qq[l], i = l;
  }
 ...
 * */
__host__ __device__ void build_code(HuffmanTree *ht, node_list n, int len,
...
template <typename T>
__device__ __forceinline__ void push_v2(internal_stack_t *s, node_list n,
                                        T path, T len) {
...
template <typename T>
__device__ __forceinline__ node_list pop_v2(internal_stack_t *s,
                                            T *path_to_restore,
                                            T *length_to_restore) {
  node_list n;
...
template <typename H>
__global__ void InitHuffTreeAndGetCodebook(
    int stateNum, unsigned int *freq,
    H *codebook) { // length known as huffmanTree->allNodes
  if (threadIdx.x != 0)
    return;
  global_gpuTree = createHuffmanTreeGPU(stateNum);
  for (size_t i = 0; i < global_gpuTree->allNodes; i++)
    if (freq[i])
      qinsert(global_gpuTree, new_node(global_gpuTree, freq[i], i, 0, 0));
  while (global_gpuTree->qend > 2)
    qinsert(global_gpuTree,
            new_node(global_gpuTree, 0, 0, qremove(global_gpuTree),
                     qremove(global_gpuTree)));
  InOrderTraverse_v2<H>(global_gpuTree, codebook);
}

// TODO `unsigned int` seems trivial to pick up
template __global__ void
InitHuffTreeAndGetCodebook<uint32_t>(int stateNum, unsigned int *freq,
                                     uint32_t *codebook);
template __global__ void
InitHuffTreeAndGetCodebook<uint64_t>(int stateNum, unsigned int *freq,
                                     uint64_t *codebook);
