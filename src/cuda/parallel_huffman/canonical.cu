...
__global__ void GPU::GetCanonicalCode(uint8_t *singleton, int DICT_SIZE) {
  ...
  auto metadata =
      reinterpret_cast<int *>(singleton + sizeof(H) * (3 * DICT_SIZE));
  auto keys = reinterpret_cast<Q *>(singleton + sizeof(H) * (3 * DICT_SIZE) +
                                    sizeof(int) * (4 * type_bw));
  H *i_cb = codebooks;
  ...
  if (gid == 0) {
    ...
    for (int i = 1; i < type_bw; i++)
      entry[i] += entry[i - 1];
  }
  g.sync();

  if (gid < type_bw)
    iter_by_[gid] = entry[gid];
  __syncthreads();
  // atomicMax(&max_bw, bw);
  ...
  if (canonical[gid] == ~((H)0x0u))
    return;
  o_cb[keys[gid]] = canonical[gid];
}

template __global__ void
GPU::GetCanonicalCode<uint32_t, uint8_t>(uint8_t *singleton, int DICT_SIZE);
template __global__ void
GPU::GetCanonicalCode<uint32_t, uint16_t>(uint8_t *singleton, int DICT_SIZE);
template __global__ void
GPU::GetCanonicalCode<uint32_t, uint32_t>(uint8_t *singleton, int DICT_SIZE);
template __global__ void
GPU::GetCanonicalCode<uint64_t, uint8_t>(uint8_t *singleton, int DICT_SIZE);
template __global__ void
GPU::GetCanonicalCode<uint64_t, uint16_t>(uint8_t *singleton, int DICT_SIZE);
template __global__ void
GPU::GetCanonicalCode<uint64_t, uint32_t>(uint8_t *singleton, int DICT_SIZE);
