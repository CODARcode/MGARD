...
template <typename T>
__global__ void
_restriction_1(int nrow,       int ncol,
               int nr,         int nc,
  ...
  for (int r = r0; r < nr; r += (blockDim.x * gridDim.x) * row_stride) {
    ...
    T h1 = _get_dist(dcoords_x, dicol[0], dicol[col_stride]); //dicol[col_Pstride] - dicol[0];
    T h2 = _get_dist(dcoords_x, dicol[col_stride], dicol[col_Cstride]); //dicol[col_stride] - dicol[col_Pstride];
    T hsum = h1 + h2;
...
template <typename T>
void 
restriction_1(mgard_cuda_handle<T> & handle, 
              int nrow,       int ncol,
              int nr,         int nc,
              int row_stride, int col_stride,
              int * dirow,    int * dicol,
  ...
  _restriction_1<<<blockPerGrid, threadsPerBlock,
...
template void 
restriction_1<double>(mgard_cuda_handle<double> & handle, 
              int nrow,       int ncol,
              int nr,         int nc,
              int row_stride, int col_stride,
              int * dirow,    int * dicol,
              ...
              int queue_idx);
template void 
restriction_1<float>(mgard_cuda_handle<float> & handle, 
              int nrow,       int ncol,
              int nr,         int nc,
              int row_stride, int col_stride,
              int * dirow,    int * dicol,
...
template <typename T>
__global__ void
_restriction_2(int nrow,       int ncol,
               int nr,         int nc,
               int row_stride, int col_stride,
               int * dirow,    int * dicol,
               T * dcoords_y,
  ...
  for (int c = c0; c < nc; c += (blockDim.x * gridDim.x) * col_stride) {
    T * vec = dv + dicol[c];
    T h1 = _get_dist(dcoords_y, dirow[0], dirow[row_stride]); //dirow[row_Pstride] - dirow[0];
    T h2 = _get_dist(dcoords_y, dirow[row_stride], dirow[row_Cstride]); //dirow[row_stride] - dirow[row_Pstride];
    T hsum = h1 + h2;
...
template <typename T>
void 
restriction_2(mgard_cuda_handle<T> & handle,
              int nrow,       int ncol,
              int nr,         int nc,
              int row_stride, int col_stride,
              int * dirow,    int * dicol,
  ...
  dim3 threadsPerBlock(tbx, 1);
  dim3 blockPerGrid(gridx, 1);
  _restriction_2<<<blockPerGrid, threadsPerBlock,
                   0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                        nrow,       ncol,
                                        nr,         nc,
                                        row_stride, col_stride,
...
template void 
restriction_2<double>(mgard_cuda_handle<double> & handle,
              int nrow,       int ncol,
              int nr,         int nc,
              int row_stride, int col_stride,
              int * dirow,    int * dicol,
              ...
              int queue_idx);
template void 
restriction_2<float>(mgard_cuda_handle<float> & handle,
              int nrow,       int ncol,
              int nr,         int nc,
              int row_stride, int col_stride,
              int * dirow,    int * dicol,
...
template <typename T>
__global__ void
_restriction_1_cpt(int nr,         int nc,
                   int row_stride, int col_stride,
                   T * ddist_x, int ghost_col,
  ...
  for (int r = r0; r < nr; r += gridDim.y * blockDim.y * row_stride) {
    ...
    while (rest_col > blockDim.x - real_ghost_col) {
      ...
      if (c0_sm == 0) {
        ...
        prev_h1 = dist_x_sm[blockDim.x - 2]; //_get_dist(dcoords_x_sm, blockDim.x - 2, blockDim.x - 1);
        prev_h2 = dist_x_sm[blockDim.x - 1]; //_get_dist(dcoords_x_sm, blockDim.x - 1, blockDim.x);
      }
      ...
      if (c0_sm < real_ghost_col + rest_col) {
        if (c0_sm % 2 == 0) {
          if (c0_sm == 0) {
            ...
            h2 = dist_x_sm[c0_sm+1]; //_get_dist(dcoords_x_sm, c0_sm+1, c0_sm+2);
            result += h2 * vec_sm[c0_sm+1] / (h1+h2);
          } else if (c0_sm == real_ghost_col + rest_col - 1) {
            result = vec_sm[c0_sm];
            h1 = dist_x_sm[c0_sm-2]; //_get_dist(dcoords_x_sm, c0_sm-2, c0_sm-1);;
            h2 = dist_x_sm[c0_sm-1]; //_get_dist(dcoords_x_sm, c0_sm-1, c0_sm);;
...
template <typename T>
void 
restriction_1_cpt(mgard_cuda_handle<T> & handle, 
                  int nr,         int nc,
                  int row_stride, int col_stride,
                  T * ddist_x, 
                  T * dv,    int lddv,
  ...
  _restriction_1_cpt<<<blockPerGrid, threadsPerBlock, 
...
template void 
restriction_1_cpt<double>(mgard_cuda_handle<double> & handle, 
                  int nr,         int nc,
                  int row_stride, int col_stride,
                  double * ddist_x, 
                  double * dv,    int lddv,
                  int queue_idx);
template void 
restriction_1_cpt<float>(mgard_cuda_handle<float> & handle, 
                  int nr,         int nc,
                  int row_stride, int col_stride,
                  float * ddist_x, 
                  float * dv,    int lddv,
...
template <typename T>
__global__ void
_restriction_2_cpt(int nr,         int nc,
                   int row_stride, int col_stride,
                   T * ddist_y,    int ghost_row,
                   T * __restrict__ dv,    int lddv) {
  ...
  //     //load main column
      ...
      if (r0_sm == 0) {
        ...
        prev_h1 = dist_y_sm[blockDim.y - 2]; //_get_dist(dcoords_x_sm, blockDim.x - 2, blockDim.x - 1);
        prev_h2 = dist_y_sm[blockDim.y - 1]; //_get_dist(dcoords_x_sm, blockDim.x - 1, blockDim.x);
      }
      __syncthreads();
      // advance c0
      r0 += blockDim.y;
      ...
      if (r0_sm < real_ghost_row + rest_row) {
        if (r0_sm % 2 == 0) {
          if (r0_sm == 0) {
            ...
            h2 = dist_y_sm[r0_sm+1]; //_get_dist(dcoords_x_sm, c0_sm+1, c0_sm+2);
            result += h2 * vec_sm[(r0_sm+1) * ldsm] / (h1+h2);
          } else if (r0_sm == real_ghost_row + rest_row - 1) {
            result = vec_sm[r0_sm * ldsm];
            h1 = dist_y_sm[r0_sm-2]; //_get_dist(dcoords_x_sm, c0_sm-2, c0_sm-1);;
            h2 = dist_y_sm[r0_sm-1]; //_get_dist(dcoords_x_sm, c0_sm-1, c0_sm);;
...
template <typename T>
void 
restriction_2_cpt(mgard_cuda_handle<T> & handle, 
                  int nr,         int nc,
                  int row_stride, int col_stride,
                  T * ddist_y,
                  T * dv,    int lddv,
  ...
  dim3 threadsPerBlock(tbx, tby);
  dim3 blockPerGrid(gridx, gridy);
  _restriction_2_cpt<<<blockPerGrid, threadsPerBlock, 
                       sm_size, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                                  nr,         nc,
                                                  row_stride, col_stride,
                                                  ddist_y,    ghost_row,
...
template void 
restriction_2_cpt<double>(mgard_cuda_handle<double> & handle, 
                  int nr,         int nc,
                  int row_stride, int col_stride,
                  double * ddist_y,
                  double * dv,    int lddv,
                  int queue_idx);
template void 
restriction_2_cpt<float>(mgard_cuda_handle<float> & handle, 
                  int nr,         int nc,
                  int row_stride, int col_stride,
                  float * ddist_y,
                  float * dv,    int lddv,
...
template <typename T>
__global__ void
_restriction_first_1(int nrow,       int ncol,
                     int nr,         int nc,
                     int row_stride, int col_stride, 
                     int * irow,     int * icol_p,
                     T * dist_c, 
...
template <typename T>
void 
restriction_first_1(mgard_cuda_handle<T> & handle, 
                    int nrow,       int ncol,
                    int nr,         int nc,
                    int row_stride, int col_stride, 
  ...
  _restriction_first_1<<<blockPerGrid, threadsPerBlock, 
                         0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                                    nrow,       ncol,
                                                    nr,         nc,
                                                    row_stride, col_stride,
...
template void 
restriction_first_1<double>(mgard_cuda_handle<double> & handle, 
                    int nrow,       int ncol,
                    int nr,         int nc,
                    int row_stride, int col_stride, 
                    int * dirow,     int * dicol_p,
                    ...
                    int queue_idx);
template void 
restriction_first_1<float>(mgard_cuda_handle<float> & handle, 
                    int nrow,       int ncol,
                    int nr,         int nc,
                    int row_stride, int col_stride, 
                    int * dirow,     int * dicol_p,
...
template <typename T>
__global__ void
_restriction_first_2(int nrow,       int ncol,
                     int nr,         int nc,
                     int row_stride, int col_stride,
                     int * irow_p,   int * icol,
                     T * dist_r,
...
template <typename T>
void 
restriction_first_2(mgard_cuda_handle<T> & handle,
                    int nrow,       int ncol,
                    int nr,         int nc,
                    int row_stride, int col_stride,
                    int * irow_p,    int * icol,
  ...
  dim3 threadsPerBlock(tb, 1);
  dim3 blockPerGrid(grid, 1);
  _restriction_first_2<<<blockPerGrid, threadsPerBlock, 
                         0, *(cudaStream_t *)handle.get(queue_idx)>>>(
                                                    nrow,       ncol,
                                                    nr,         nc,
                                                    row_stride, col_stride,
...
template void 
restriction_first_2<double>(mgard_cuda_handle<double> & handle,
                    int nrow,       int ncol,
                    int nr,         int nc,
                    int row_stride, int col_stride,
                    int * irow_p,    int * icol,
                    ...
                    int queue_idx);
template void 
restriction_first_2<float>(mgard_cuda_handle<float> & handle,
                    int nrow,       int ncol,
                    int nr,         int nc,
                    int row_stride, int col_stride,
                    int * irow_p,    int * icol,
