...
int  parse_cmdl(int argc, char**argv, bool& inf_flag, bool& qoi_flag, int& nrow, int& ncol, int& nfib, DOUBLE& tol, DOUBLE& s, std::string& in_file, std::string& coord_file, std::string& shared_obj, std::string& function_handle)
  {
    if ( argc >= 7 )
      {
  ...
  if ( itype == 0 ) //double
    {
      ...
      if(inf_flag)
	{
	  //compressed_data = mgard_compress(0, v.data(), out_size,  nrow,  ncol,  nfib, tol);
    compressed_data = mgard_compress_cuda(0, v.data(), out_size,  nrow,  ncol,  nfib, tol);
	}
      else
	{
	  if(qoi_flag)
      ...
      if(inf_flag)
	{
	  dtest = mgard_decompress(itype, dummy, compressed_data, out_size,  nrow,  ncol,  nfib);
	}
      else
	{
	  //dtest = mgard_decompress(itype, dummy, compressed_data, out_size,  nrow,  ncol,  nfib, s);
    dtest = mgard_decompress_cuda(itype, dummy, compressed_data, out_size,  nrow,  ncol,  nfib);
	}
  ...
  else if ( itype == 1 ) //float
    {
      parse_cmdl(argc, argv, inf_flag, qoi_flag, nrow, ncol, nfib, tolf, sf, in_file, coord_file, shared_obj, function_handle);
       ...
       std::vector<float> v(nrow*ncol*nfib), coords_x(ncol), coords_y(nrow), coords_z(nfib);
      ...
      if(inf_flag)
      	{
      	  compressed_data = mgard_compress(itype, v.data(), out_size,  nrow,  ncol,  nfib, tolf);
      	}
      else
      	{
