...
refactor_2D_cuda_compact_l2_sm(const int l_target,
 ...
 // std::cout <<"test8\n"; 
  ...
  for (int l = 0; l < l_target; ++l) {
    ...
    std::cout << "restriction_l_row_cuda_mem_throughput (" << nr_l[0] << ", " << nc_l[l] << "): " << mem_throughput << "GB/s. \n";
    ...
    // double * work = new double[nr_l[0] * nc_l[l+1]];
    // for (int i = 0; i < nr_l[0]; i++) {
    ...
    //                    nc_l[l+1] * sizeof(double), nr_l[0], H2D);

    // // print_matrix_cuda(nr_l[0],    nc_l[l+1], dcwork,     lddcwork);
    // cudaMemcpy2DHelper(dcwork2, lddcwork2 * sizeof(double),
    //                    dcwork,  lddcwork * sizeof(double),
    //                    nc_l[l+1] * sizeof(double), nr_l[0], D2D);
    ...
    //                                  dccoords_x_l[l+1]);
    // print_matrix_cuda(nr_l[0],    nc_l[l+1], dcwork,     lddcwork);
    ret = solve_tridiag_M_l_row_cuda_sm(nr_l[0],    nc_l[l+1],
                                  nr_l[0],    nc_l[l+1],
                                  row_stride, col_stride,
                                  dcirow_l[0],dcicol_l[l+1],
                                  dcwork,     lddcwork,
    ...
    mem_throughput = (data_size/ret.time)/1e9;
    std::cout << "solve_tridiag_M_l_row_cuda_mem_throughput (" << nr_l[0] << ", " << nc_l[l+1] << "): " << mem_throughput << "GB/s. \n";
    ...
    if (nrow > 1) // do this if we have an 2-dimensional array
    {
      ...
      //                            dccoords_y_l[l]);
      // print_matrix_cuda(nr_l[l],    nc_l[l+1], dcwork,     lddcwork);
      ret = mass_mult_l_col_cuda_sm(nr_l[l],     nc_l[l+1],
                                   nr_l[l],     nc_l[l+1],
                                   row_stride,  col_stride,
                                   dcirow_l[l], dcicol_l[l+1],
                                   dcwork,     lddcwork,
      ...
      //                              dccoords_y_l[l]);
      // print_matrix_cuda(nr_l[l],    nc_l[l+1], dcwork,     lddcwork);
      ret = restriction_l_col_cuda_sm(nr_l[l],         nc_l[l+1],
                                   nr_l[l],         nc_l[l+1],
                                   row_stride, col_stride,
                                   dcirow_l[l],     dcicol_l[l+1],
                                   dcwork, lddcwork,
      ...
      // double * work = new double[nr_l[l+1] * nc_l[l+1]];
      // for (int i = 0; i < nr_l[l+1]; i++) {
      //   for (int j = 0; j < nc_l[l+1]; j++) {
      //     work[i * nc_l[l+1] + j] = i * nc_l[l+1] + j;
      //   }
      // }
      ...
      // cudaMemcpy2DHelper(dcwork, lddcwork * sizeof(double),
      //                    work,  nc_l[l+1] * sizeof(double),
      //                    nc_l[l+1] * sizeof(double), nr_l[l+1], H2D);

      // if (l == 6) print_matrix_cuda(nr_l[l+1],    nc_l[l+1], dcwork,     lddcwork);
      // cudaMemcpy2DHelper(dcwork2, lddcwork2 * sizeof(double),
      //                    dcwork,  lddcwork * sizeof(double),
      //                    nc_l[l+1] * sizeof(double), nr_l[l+1], D2D);
      ...
      ret = solve_tridiag_M_l_col_cuda_sm(nr_l[l+1],     nc_l[l+1],
                                          ...
                                          dccoords_y_l[l+1],
                                          16, 16);
      // if (l == 6) print_matrix_cuda(nr_l[l+1],    nc_l[l+1], dcwork2,     lddcwork2);
      // compare_matrix_cuda(nr_l[l+1], nc_l[l+1],
      //                     dcwork,    lddcwork,
      //                     dcwork2,   lddcwork2);
