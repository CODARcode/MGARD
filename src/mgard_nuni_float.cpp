...
inline float decode(short x) { return float(float(2 * x + 1.0) / 65535.0); }
...
void qwrite_2D_bin(const int nrow, const int ncol, const int nlevel,
  ...
  // std::cout  << "Pruned : "<< prune_count << " Reduction : " << (float)
...
// void pi_lminus1_l(const int  l, std::vector<float>& v, const
  ...
  if (nrow > 1) {
    ...
    for (int irow = stride; irow < nr; irow += Cstride) {
      ...
      for (int jcol = stride; jcol < nc; jcol += Cstride) {
        ...
        float x1 = 0.0; // relative coordinate axis centered at irow - Cstride,
                        // jcol - Cstride
        float y1 = 0.0;
    ...
    for (int irow = 0; irow < nr - 1; irow += stride) {
      ...
      for (int jcol = 0; jcol < nc - 1; jcol += stride) {
            ...
            (jrP != jr + 1)) // we skipped both a row and a column
        {
          ...
          float x1 = 0.0; // relative coordinate axis centered at irow -
...
float ml2_norm3(const int l, int nr, int nc, int nf, int nrow, int ncol,
  ...
  //    float norm = ( std::inner_product( v.begin(), v.end(), work.begin(),
  //    0.0d)  );
...
// void
// refactor_1D ( const int l_target, std::vector<float>& v,  std::vector<float>&
// work,  std::vector<float>& coords, int n, int no)
// {
...
//  ncol,  const int l_target, float* v, std::vector<float>& work,
//  std::vector<float>& coords_x, std::vector<float>& coords_y,
//  std::vector<float>& row_vec, std::vector<float>& col_vec )
// {
//     //refactor
...
void project_non_canon(const int nr, const int nc, const int nf, const int nrow,
  ...
  //  nfib);
  // float norm = mgard_gen::ml2_norm3(0, nr, nc, nf, nrow, ncol, nfib, work,
...
void project_3D(const int nr, const int nc, const int nf, const int nrow,
                ...
                std::vector<float> &work, std::vector<float> &work2d,
                std::vector<float> &coords_x, std::vector<float> &coords_y,
                std::vector<float> &coords_z, std::vector<float> &norm_vec) {
  ...
  // nfib); float norm = mgard_gen::ml2_norm3_ng(0, nr, nc, nf, nrow, ncol,
  ...
  float norm = mgard_gen::ml2_norm3(l_target + 1, nr, nc, nf, nrow, ncol, nfib,
                                    work, coords_x, coords_y, coords_z);
...
float qoi_norm(int nrow, int ncol, int nfib, std::vector<float> &coords_x,
  ...
  std::vector<float> work2d(nrow * ncol), temp(nsize), row_vec(ncol),
  ...
  float norm22 = mgard_gen::ml2_norm3(0, nrow, ncol, nfib, nrow, ncol, nfib, xi,
                                      coords_x, coords_y, coords_z);
  ...
  float norm = mgard_gen::ml2_norm3(0, nrow, ncol, nfib, nrow, ncol, nfib, xi,
                                    coords_x, coords_y, coords_z);
  norm_vec[0] = norm;
  ...
  float norm0 = mgard_gen::ml2_norm3(nlevel, nr, nc, nf, nrow, ncol, nfib, xi,
                                     coords_x, coords_y, coords_z);
